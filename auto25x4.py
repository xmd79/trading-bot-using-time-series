...
====================================================================
NEW SIGNAL LOGIC:
  - Mandatory: Momentum (1m) and Volume (1m) MUST be true for the direction.
...
def compute_signals(buffer_5m, buffer_1m, live_price):
    ...
    # ══════════════════════════════════════════════════════════════════
    # NEW LOGIC: 3 out of 5 conditions, BUT Mom & Vol are MANDATORY
    # ══════════════════════════════════════════════════════════════════
    ...
    # MANDATORY: Mom + Vol must be true. 
    # Since they count as 2, we only need >= 1 more from the other 3 to reach 3/5.
    is_long = (cond_mom_long and cond_vol_long and long_true_count >= 3)
    is_short = (cond_mom_short and cond_vol_short and short_true_count >= 3)
...
def write_trade_to_journal(trade_result):
    ...
    line = (f"{'='*60}\nMODE: {mode}\nTYPE: {trade_result.get('type', 'N/A').upper()}\n"
            ...
            f"LEVERAGE: {LEVERAGE}x\nSTRATEGY: 3/5 Cond (Mom+Vol Mandatory)\n{'='*60}\n\n")
    try:
...
def print_conditions(sig):
    ...
    print(f"  ═══ LONG:{long_true}/5 SHORT:{short_true}/5 (Rule: Mom+Vol Mandatory + >=3 Total) -> LONG:{sig['is_long']} SHORT:{sig['is_short']}")
    ...
    while True:
        try:
            ...
            if not pos["is_open"] and not saved_state:
                ...
                if sig["is_long"]:
                    print(f"\n  *** 3/5 CONDITIONS MET (Mom+Vol MANDATORY) -> LONG ***")
                    ...
                    print(f"\n  *** 3/5 CONDITIONS MET (Mom+Vol MANDATORY) -> SHORT ***")
                    if has_sufficient_balance:
