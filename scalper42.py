...
def predict_reversals(candles, forecast_minutes=12):
    ...
    minor_reversal_target = predictions[-1] * 1.02  
    major_reversal_target = predictions[-1] * 1.05  
                ...
                if true_count >= 6:
                    ...
                    if timeframe == "1m":
                        ...
                        if true_count >= 6:
                            ...
                            with open(signal_file, "a") as f:
                                f.write(f"{timestamp} - SIGNAL: DIP found on 1m at {current_close:.2f}\n")
                            print(f"MTF DIP found on both 5m && 1m tfs at {current_close:.2f} - Recorded to {signal_file}")
