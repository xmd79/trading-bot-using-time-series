...
def remove_nans_and_zeros(array):
...
def scale_to_sine(timeframe):
    ...
    # Toroidal symmetrical circuit calculations
    phi = (1 + np.sqrt(5)) / 2
    pi = np.pi
    threshold_angle = 45  # 45 degree threshold

    # Normalize sine wave within range [-1, 1]
    norm_sine_wave = 2 * (sine_wave - sine_wave_min) / (sine_wave_max - sine_wave_min) - 1
    
    # Calculate complementary angles in radians
    complementary_angle = np.radians(threshold_angle)
    sin_complementary = np.sin(complementary_angle)
    
    # Cyclical adjustments
    cyclic_up = np.where(norm_sine_wave > sin_complementary, 1, 0)
    cyclic_down = np.where(norm_sine_wave < -sin_complementary, -1, 0)
    cyclic_pattern = cyclic_up + cyclic_down
    
    # Phi/pi and pi/phi symmetry adjustments
    phi_pi_symmetry = norm_sine_wave * (phi / pi)
    pi_phi_symmetry = norm_sine_wave * (pi / phi)
    
    # Combine all adjustments
    extended_sine_wave = norm_sine_wave + phi_pi_symmetry + pi_phi_symmetry + cyclic_pattern
    
    # Calculate new min and max distances
    extended_sine_min = np.min(extended_sine_wave)
    extended_sine_max = np.max(extended_sine_wave)
    
    # Calculate % distances for extended sine wave
    extended_dist_min = ((current_sine - extended_sine_min) / (extended_sine_max - extended_sine_min)) * 100
    extended_dist_max = ((extended_sine_max - current_sine) / (extended_sine_max - extended_sine_min)) * 100
    
    return extended_dist_min, extended_dist_max, current_sine, sine_wave, norm_sine_wave, complementary_angle, sin_complementary, cyclic_pattern, phi_pi_symmetry, pi_phi_symmetry, extended_sine_wave

# Example usage (assuming candle_map and timeframe are defined)
dist_min, dist_max, current_sine, sine_wave, norm_sine_wave, complementary_angle, sin_complementary, cyclic_pattern, phi_pi_symmetry, pi_phi_symmetry, extended_sine_wave = scale_to_sine(timeframe)

# Debug prints outside the function
print("Original Sine Wave:", sine_wave)
print("Normalized Sine Wave:", norm_sine_wave)
print("Complementary Angle (radians):", complementary_angle)
print("Sin of Complementary Angle:", sin_complementary)
print("Cyclic Pattern:", cyclic_pattern)
print("Phi/Pi Symmetry:", phi_pi_symmetry)
print("Pi/Phi Symmetry:", pi_phi_symmetry)
print("Extended Sine Wave:", extended_sine_wave)
print("Min Distance:", dist_min)
print("Max Distance:", dist_max)
print("Current Sine:", current_sine)
