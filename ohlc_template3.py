...
def analyze_market_data_wave():
    ...
    transforms data into a stationary wave, and plots the results.
    """
    # Define timeframes to fetch
    timeframes = [client.KLINE_INTERVAL_1HOUR]  # Use 1-hour candles for simplicity
    ...
    # Keep top 5 frequencies for reconstruction (to form a wave-like series)
    power = np.abs(yf) ** 2
    ...
    plt.tight_layout()
    plt.savefig('market_data_wave_analysis.png')
    plt.close()
    print("Plots saved as 'market_data_wave_analysis.png'")
