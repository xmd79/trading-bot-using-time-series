...
def calculate_dmi(candles, timeframe, period=DMI_PERIOD):
    ...
    try:
        ...
        return latest_plus_di, latest_minus_di, latest_adx
    except Exception as e:
...
def calculate_quantity(balance, price):
    if price <= Decimal('0') or balance < MINIMUM_BALANCE:
        logging.info(f"Insufficient balance ({balance:.25f} USDC) or invalid price ({price:.25f}) for trading.")
        print(f"Info: Insufficient balance {balance:.25f} USDC or invalid price {price:.25f} for trading.")
        return Decimal('0.0')
    ...
    try:
        ...
        while True:
            ...
            for timeframe in TIMEFRAMES:
                ...
                # Set volume conditions for all timeframes
                ...
                if timeframe == "1m":
                    ...
                    if plus_di is not None and minus_di is not None and adx is not None:
                        ...
                        # For SHORT: -DI > +DI and ADX > ADX_THRESHOLD (strong downtrend)
                        conditions_short["dmi_bearish_1m"] = (minus_di > plus_di) and (adx > ADX_THRESHOLD)
                ...
                # Print volume conditions for all timeframes
            ...
            # Generate signal if at least SIGNAL_CONDITION_THRESHOLD conditions are true
            long_signal = long_true_count >= SIGNAL_CONDITION_THRESHOLD
            short_signal = short_true_count >= SIGNAL_CONDITION_THRESHOLD
            ...
            if signal in ["LONG", "SHORT"] and position["side"] == "NONE":
                ...
                if quantity > Decimal('0'):
                    ...
                    # Not enough balance, send an info message
                    info_message = (
                        f"Signal Detected: {signal}\n"
                        f"Time: {current_local_time_str}\n"
                        f"Conditions Met: {condition_count}/{total_conditions}\n"
                        f"True Conditions:\n" + "\n".join(true_conditions) + "\n\n"
                        f"Insufficient balance to trade.\n"
                        f"Required: {MINIMUM_BALANCE} USDC, Available: {usdc_balance:.25f} USDC\n"
                    )
                    telegram_loop.run_until_complete(send_telegram_message(info_message))
                    logging.info(f"Insufficient balance to trade {signal} signal. Required: {MINIMUM_BALANCE} USDC, Available: {usdc_balance:.25f} USDC")
                    print(f"Info: Insufficient balance to trade {signal} signal. Required: {MINIMUM_BALANCE} USDC, Available: {usdc_balance:.25f} USDC")
