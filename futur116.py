    ...
    for attempt in range(retries):
        try:
            ...
            message = (
                ...
                f"Conditions Met: {condition_count}/{total_conditions}\n"
                f"True Conditions:\n" + "\n".join(true_conditions) + "\n\n"
            ...
            for tf in TIMEFRAMES:
                ...
                message += (
                    ...
                    f"Wavelet Dominant Cycle: {details.get('wavelet_dominant_cycle', 0)}\n"
                    f"Wavelet Cycle Direction: {details.get('wavelet_cycle_direction', 'N/A')}\n"
                    ...
                    f"Volume Bullish ({tf}): {details.get(f'volume_bullish_{tf}', False)}\n"
                    f"Volume Bearish ({tf}): {details.get(f'volume_bearish_{tf}', False)}\n"
                )
                
                # Add DMI information for 1m timeframe
                if tf == "1m":
                    message += (
                        f"DMI +DI: {details.get('plus_di', 0):.4f}\n"
                        f"DMI -DI: {details.get('minus_di', 0):.4f}\n"
                        f"DMI ADX: {details.get('adx', 0):.4f}\n"
                        f"DMI Bullish: {details.get('dmi_bullish', False)}\n"
                        f"DMI Bearish: {details.get('dmi_bearish', False)}\n"
                    )
                
                # Add LRC information for all timeframes
                message += (
                    f"LRC Regression: {details.get('lrc_regression', 0):.2f}\n"
                    f"LRC Upper: {details.get('lrc_upper', 0):.2f}\n"
                    f"LRC Lower: {details.get('lrc_lower', 0):.2f}\n"
                    f"LRC Forecast: {details.get('lrc_forecast', 0):.2f}\n"
                    f"LRC Above Upper: {details.get('lrc_above_upper', False)}\n"
                    f"LRC Below Lower: {details.get('lrc_below_lower', False)}\n"
                )
                
                # Add new conditions
                message += (
                    f"DIP Reversal: {details.get('dip_reversal', False)}\n"
                    f"TOP Reversal: {details.get('top_reversal', False)}\n"
                    f"Below Middle: {details.get('below_middle', False)}\n"
                    f"Above Middle: {details.get('above_middle', False)}\n"
                )
                
                # Add LRC forecast conditions
                message += (
                    f"LRC Forecast Above Current ({tf}): {details.get(f'lrc_forecast_above_current_{tf}', False)}\n"
                    f"LRC Forecast Below Current ({tf}): {details.get(f'lrc_forecast_below_current_{tf}', False)}\n"
                )
    ...
    try:
        ...
        while True:
            ...
            required_long_conditions = [
                ...
                "below_middle_3m",
                "below_middle_5m",
                # Volume condition only for 1m
                "volume_bullish_1m",
                # LRC forecast conditions for all timeframes
                "lrc_forecast_above_current_1m",
                "lrc_forecast_above_current_3m",
                "lrc_forecast_above_current_5m",
            ]
            ...
            required_short_conditions = [
                ...
                "volume_bearish_1m",
                # LRC forecast conditions for all timeframes
                "lrc_forecast_below_current_1m",
                "lrc_forecast_below_current_3m",
                "lrc_forecast_below_current_5m",
            ]
            ...
            long_true_count = sum(1 for cond in required_long_conditions if conditions_long.get(cond, False))
            short_true_count = sum(1 for cond in required_short_conditions if conditions_short.get(cond, False))
            
            total_required_long = len(required_long_conditions)
            total_required_short = len(required_short_conditions)
            
            print(f"\nSummary: LONG Required Conditions - True: {long_true_count}, False: {total_required_long - long_true_count}")
            print(f"Summary: SHORT Required Conditions - True: {short_true_count}, False: {total_required_short - short_true_count}")
            
            logging.info(f"Summary: LONG Required Conditions - True: {long_true_count}, False: {total_required_long - long_true_count}")
            logging.info(f"Summary: SHORT Required Conditions - True: {short_true_count}, False: {total_required_short - short_true_count}")
            
            # Check if all required conditions are met
            long_signal = (long_true_count == total_required_long)
            short_signal = (short_true_count == total_required_short)
            
            print(f"\nTrade Signal Status:")
            print(f"LONG Signal: {'Active' if long_signal else 'Inactive'} (Conditions: {long_true_count}/{total_required_long})")
            print(f"SHORT Signal: {'Active' if short_signal else 'Inactive'} (Conditions: {short_true_count}/{total_required_short})")
            
            logging.info(f"LONG Signal: {'Active' if long_signal else 'Inactive'} (Conditions: {long_true_count}/{total_required_long})")
            logging.info(f"SHORT Signal: {'Active' if short_signal else 'Inactive'} (Conditions: {short_true_count}/{total_required_short})")
            ...
            current_long_condition_met = (long_true_count == total_required_long)
            current_short_condition_met = (short_true_count == total_required_short)
            
            # Get current time for cooldown check
            current_time = time.time()
            
            # FIXED: Always send an alert if all conditions are met, with cooldown to prevent spam
            ...
            if should_send_alert:
                ...
                telegram_analysis_message = (
                    ...
                    f"SHORT Conditions: {short_true_count}/{total_required_short}\n\n"
                )
                
                # Add LRC status for all timeframes
                for tf in TIMEFRAMES:
                    details = analysis_details.get(tf, {})
                    lrc_valid = details.get('lrc_valid', False)
                    telegram_analysis_message += (
                        f"--- {tf} LRC Analysis ---\n"
                    )
                    if lrc_valid:
                        telegram_analysis_message += (
                            f"Regression: {details.get('lrc_regression', 0):.2f}\n"
                            f"Upper Channel: {details.get('lrc_upper', 0):.2f}\n"
                            f"Lower Channel: {details.get('lrc_lower', 0):.2f}\n"
                            f"Forecast: {details.get('lrc_forecast', 0):.2f}\n"
                            f"Above Upper: {details.get('lrc_above_upper', False)}\n"
                            f"Below Lower: {details.get('lrc_below_lower', False)}\n"
                            f"Forecast Above Current: {details.get(f'lrc_forecast_above_current_{tf}', False)}\n"
                            f"Forecast Below Current: {details.get(f'lrc_forecast_below_current_{tf}', False)}\n\n"
                        )
                    else:
                        telegram_analysis_message += (
                            f"Status: Invalid calculation\n\n"
                        )
            ...
            if signal in ["LONG", "SHORT"] and position["side"] == "NONE":
                ...
                if signal == "LONG":
                    true_conditions = [cond for cond in required_long_conditions if conditions_long.get(cond, False)]
                    total_conditions = total_required_long
                    condition_count = long_true_count
                else:
