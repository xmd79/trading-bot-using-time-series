...
LEVERAGE = 20
MIN_NOTIONAL = Decimal('10.00')  # Minimum notional value for trades
...
def get_target(closes, n_components, last_major_reversal_type, buy_volume, sell_volume):
    ...
    fft = scipy.fft.rfft(closes_np)
    frequencies = scipy.fft.rfftfreq(len(closes_np))
...
def main():
    ...
    while True:
        try:
            ...
            # Initialize conditions (all non-volume conditions on 1m only)
            conditions_long = {
            ...
            # 1m-specific analysis for non-volume conditions
            min_threshold_1m, max_threshold_1m, momentum_signal_1m, adjusted_forecasted_price = None, None, None, None
            ...
            if "1m" in candle_map and candle_map["1m"]:
                print("\n--- 1m Timeframe Analysis (Primary for Trigger Conditions) ---")
                closes = [candle["close"] for candle in candle_map["1m"]]
                current_close = Decimal(str(closes[-1]))
                high_tf = Decimal(str(max([float(x) for x in closes])))
                low_tf = Decimal(str(min([float(x) for x in closes])))
                
                # Backtest and forecast
                model, _, _, _ = backtest_model(candle_map["1m"])
                forecasted_price = forecast_next_price(model, num_steps=1)[0]
                print(f"Forecasted Price: {forecasted_price:.25f}")
                min_threshold_1m, max_threshold_1m, _, momentum_signal_1m, range_price, _, _ = calculate_thresholds(closes)
                adjusted_forecasted_price = min(max(forecasted_price, min_threshold_1m), max_threshold_1m) if min_threshold_1m is not None and max_threshold_1m is not None else forecasted_price
                print(f"Adjusted Forecasted Price: {adjusted_forecasted_price:.25f}")

                # Support and resistance (used across all analyses)
                support_levels, resistance_levels = find_specific_support_resistance(candle_map, min_threshold_1m or current_price * Decimal('0.95'), max_threshold_1m or current_price * Decimal('1.05'), current_price)

                # Reversal conditions
                last_bottom, last_top, closest_reversal, closest_type = find_major_reversals(candle_map["1m"], current_price, min_threshold_1m, max_threshold_1m)
                conditions_long["dip_confirmed_1m"] = closest_type == 'DIP'
                conditions_short["top_confirmed_1m"] = closest_type == 'TOP'
                print(f"Reversal Type: {closest_type}, Closest Reversal: {closest_reversal:.25f}")

                # Distance conditions
                dist_to_min_price = ((current_close - min_threshold_1m) / (max_threshold_1m - min_threshold_1m)) * Decimal('100') if (max_threshold_1m - min_threshold_1m) != Decimal('0') else Decimal('50')
                dist_to_max_price = ((max_threshold_1m - current_close) / (max_threshold_1m - min_threshold_1m)) * Decimal('100') if (max_threshold_1m - min_threshold_1m) != Decimal('0') else Decimal('50')
                conditions_long["dist_to_min_closer_1m"] = dist_to_min_price < dist_to_max_price
                conditions_short["dist_to_max_closer_1m"] = dist_to_min_price >= dist_to_max_price
                print(f"Distance to Min Threshold (1m): {dist_to_min_price:.25f}%")
                ...
                # Volume conditions
                buy_vol = buy_volume.get("1m", [Decimal('0')])[-1]
                sell_vol = sell_volume.get("1m", [Decimal('0')])[-1]
                conditions_long["volume_bullish_1m"] = buy_vol > sell_vol
                conditions_short["volume_bearish_1m"] = sell_vol > buy_vol
                print(f"Volume Bullish (1m): {buy_vol:.25f}, Bearish: {sell_vol:.25f}, Bullish Condition: {conditions_long['volume_bullish_1m']}, Bearish Condition: {conditions_short['volume_bearish_1m']}")

                # Moving averages (second filter)
                ...
                print(f"Sine Wave Distance to Min (1m): {sine_dist_to_min:.25f}%")
                print(f"Sine Wave Distance to Max: {sine_dist_to_max:.25f}%")
                stoch_k, _ = calculate_stochastic_rsi(closes)
                ...
                avg = (min_threshold_1m + max_threshold_1m) / Decimal('2') if min_threshold_1m is not None and max_threshold_1m is not None else current_price
                calculate_wave_price(len(closes), avg, min_threshold_1m or Decimal('0'), max_threshold_1m or Decimal('Infinity'), Decimal('0.1'), Decimal('0'))
                calculate_independent_wave_price(current_price, avg, min_threshold_1m or Decimal('0'), max_threshold_1m or Decimal('Infinity'), Decimal('0.1'))
                ...
                fib_levels = calculate_fibonacci_levels_from_reversal(closest_reversal, max_threshold_1m, min_threshold_1m, closest_type, current_price)
                fib_info["1m"] = fib_levels
                print(f"Fibonacci Levels for 1m:")
                for level, price in fib_levels.items():
                    print(f"Level {level}: {price:.25f}")
                forecast_fibo_target_price(fib_info["1m"])
                ...
                print(f"Normalized Distance to Max Price: {symmetrical_min_distance:.25f}%")
                time_window = 1
                forecast_price, price_rate = forecast_price_per_time_pythagorean("1m", candle_map["1m"], min_threshold_1m, max_threshold_1m, current_price, time_window, closest_reversal, closest_type)
                pythagorean_forecasts["1m"] = {'price': forecast_price, 'rate': price_rate}
                print(f"Pythagorean Forecast Price for 1m: {pythagorean_forecasts['1m']['price']:.25f}")
                print(f"Pythagorean Price Rate for 1m: {pythagorean_forecasts['1m']['rate']:.25f} USDC/min")
            else:
                print("--- 1m --- No data available.")
                support_levels, resistance_levels = [], []
                fib_levels = calculate_fibonacci_levels_from_reversal(None, None, None, 'DIP', current_price)
                fib_info["1m"] = fib_levels
                print(f"Fibonacci Levels for 1m (Fallback):")
                for level, price in fib_levels.items():
                    print(f"Level {level}: {price:.25f}")
                time_window = 1
                forecast_price, price_rate = forecast_price_per_time_pythagorean("1m", [], None, None, current_price, time_window, None, 'DIP')
                pythagorean_forecasts["1m"] = {'price': forecast_price, 'rate': price_rate}
                print(f"Pythagorean Forecast Price for 1m: {pythagorean_forecasts['1m']['price']:.25f}")
                print(f"Pythagorean Price Rate for 1m: {pythagorean_forecasts['1m']['rate']:.25f} USDC/min")

            # Other timeframes (3m, 5m) for volume and supplementary analysis
            for timeframe in ["3m", "5m"]:
                if timeframe in candle_map and candle_map[timeframe]:
                    print(f"\n--- {timeframe} (Volume and Supplementary Analysis) ---")
                    closes = [candle['close'] for candle in candle_map[timeframe]]
                    current_close = Decimal(str(closes[-1]))
                    high_tf = Decimal(str(max([float(x) for x in closes])))
                    low_tf = Decimal(str(min([float(x) for x in closes])))
                    min_threshold_tf, max_threshold_tf, avg_mtf_tf, momentum_signal_tf, _, _, _ = calculate_thresholds(closes)
                    ...
                    calculate_wave_price(len(closes), avg, min_threshold_tf or Decimal('0'), max_threshold_tf or Decimal('Infinity'), Decimal('0.1'), Decimal('0'))
                    calculate_independent_wave_price(current_price, avg, min_threshold_tf or Decimal('0'), max_threshold_tf or Decimal('Infinity'), Decimal('0.1'))
                    ...
                    print(f"Normalized Distance to Min Price (Symmetrical): {symmetrical_max_distance:.25f}%")
                    print(f"Normalized Distance to Max Price: {symmetrical_min_distance:.25f}%")
                    time_window = {'3m': 3, '5m': 5}[timeframe]
            ...
            forecast_volume_based_on_conditions(volume_ratio, min_threshold_1m or current_price * Decimal('0.95'), current_price)
            check_market_conditions_and_forecast(support_levels, resistance_levels, current_price)

            # Evaluate forecast condition
            if adjusted_forecasted_price is not None:
                ...
                elif adjusted_forecasted_price < current_price:
            ...
            for long_cond, short_cond in condition_pairs:
                ...
                if long_val and short_val:
                ...
                elif not long_val and not short_val and long_cond != "forecast_price_above_close":
                    if "momentum" in long_cond:
                        # Already handled in 1m analysis
                        pass
                    elif "dist_to" in long_cond:
                        dist_to_min = ((current_price - min_threshold_1m) / (max_threshold_1m - min_threshold_1m)) * Decimal('100') if (max_threshold_1m - min_threshold_1m) != Decimal('0') else Decimal('50')
                        dist_to_max = ((max_threshold_1m - current_price) / (max_threshold_1m - min_threshold_1m)) * Decimal('100') if (max_threshold_1m - min_threshold_1m) != Decimal('0') else Decimal('50')
                        conditions_long["dist_to_min_closer_1m"] = dist_to_min < dist_to_max
                        conditions_short["dist_to_max_closer_1m"] = dist_to_min >= dist_to_max
                    elif "volume" in long_cond:
                    ...
                    elif "dip_confirmed" in long_cond:
                        _, _, _, closest_type = find_major_reversals(candle_map["1m"], current_price, min_threshold_1m, max_threshold_1m)
                        conditions_long["dip_confirmed_1m"] = closest_type == 'DIP'
                        conditions_short["top_confirmed_1m"] = closest_type == 'TOP'

            # Evaluate first filter signals
            first_filter_long = all(conditions_long.values())
            first_filter_short = all(conditions_short.values())

            # Apply second filter (SMA stack)
            long_signal = False
            short_signal = False
            if first_filter_long:
                if sma_bullish_1m:
                    long_signal = True
                    print("LONG Signal: All first filter conditions and SMA bullish alignment satisfied.")
                else:
                    print("LONG Signal blocked: SMA bullish alignment not satisfied.")
            if first_filter_short:
                if sma_bearish_1m:
                    short_signal = True
                    print("SHORT Signal: All first filter conditions and SMA bearish alignment satisfied.")
                else:
                    print("SHORT Signal blocked: SMA bearish alignment not satisfied.")
            ...
            print("\nLong Conditions Status (First Filter):")
            for condition, status in conditions_long.items():
                print(f"{condition}: {'True' if status else 'False'}")
            print("\nShort Conditions Status (First Filter):")
            for condition, status in conditions_short.items():
                print(f"{condition}: {'True' if status else 'False'}")
            print(f"\nSecond Filter (1m SMA Stack):")
            print(f"SMA Bullish Alignment: {'True' if sma_bullish_1m else 'False'}")
            print(f"SMA Bearish Alignment: {'True' if sma_bearish_1m else 'False'}")
