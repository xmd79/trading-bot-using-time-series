...
SIGNAL_CONDITION_THRESHOLD = 12  # Number of conditions that must be true to generate a signal
    ...
    try:
        ...
        while True:
            try:
                ...
                # NEW SIGNAL LOGIC
                # Check for LONG signal: 4 specific conditions + at least 12 total conditions
                long_signal = (
                    conditions_long.get('volume_bullish_1m', False) and
                    conditions_long.get('momentum_positive_1m', False) and
                    conditions_long.get('dip_reversal_1m', False) and
                    conditions_long.get('lrc_forecast_above_current_1m', False) and
                    long_condition_count >= SIGNAL_CONDITION_THRESHOLD
                )
                
                # Check for SHORT signal: 4 specific conditions + at least 12 total conditions
                short_signal = (
                    conditions_short.get('volume_bearish_1m', False) and
                    conditions_short.get('momentum_negative_1m', False) and
                    conditions_short.get('top_reversal_1m', False) and
                    conditions_short.get('lrc_forecast_below_current_1m', False) and
                    short_condition_count >= SIGNAL_CONDITION_THRESHOLD
                )
