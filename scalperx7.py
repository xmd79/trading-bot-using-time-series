...
class BinanceSineAnalyzer:
    def __init__(self, webhook_url=None):
        ...
        self.min_request_interval = 0.05  # Reduced for faster processing
    ...
    def find_recent_extremes(self, prices, window_size=1200):
        """Find the most recent minimum and maximum in the specified window"""
        if len(prices) < 2:
            return None, None, None, None
        
        # Use the last window_size values or all if less than window_size
        recent_prices = prices[-window_size:] if len(prices) > window_size else prices
        
        # Find minima and maxima using scipy's find_peaks
        minima_indices, _ = find_peaks(-np.array(recent_prices), distance=5)
        maxima_indices, _ = find_peaks(np.array(recent_prices), distance=5)
        ...
        if recent_min_idx > recent_max_idx:
            # Most recent extreme is a minimum (dip)
            extreme_type = "DIP"
            extreme_price = recent_prices[recent_min_idx]
            counterpart_type = "TOP"
            # Find the most recent maximum before this minimum
            valid_maxima = [idx for idx in maxima_indices if idx < recent_min_idx]
            counterpart_price = recent_prices[valid_maxima[-1]] if valid_maxima else max(recent_prices)
        else:
            # Most recent extreme is a maximum (top)
            extreme_type = "TOP"
            extreme_price = recent_prices[recent_max_idx]
            counterpart_type = "DIP"
            # Find the most recent minimum before this maximum
            valid_minima = [idx for idx in minima_indices if idx < recent_max_idx]
            counterpart_price = recent_prices[valid_minima[-1]] if valid_minima else min(recent_prices)
        
        return extreme_type, extreme_price, counterpart_type, counterpart_price
    ...
    def calculate_sine_wave_with_fft(self, prices, forecast_periods=5):
        ...
        # Find the most recent minima and maxima in the last 1200 values (or all if less than 1200)
        extreme_type, extreme_price, counterpart_type, counterpart_price = self.find_recent_extremes(prices_array)
        ...
        total_range = counterpart_price - extreme_price
        if total_range > 0:
            position_from_extreme = current_close - extreme_price
            pct_from_extreme = (position_from_extreme / total_range) * 100
            pct_from_counterpart = 100 - pct_from_extreme
        else:
            pct_from_extreme = 50  # Default to middle if range is zero
            pct_from_counterpart = 50
        ...
        # Calculate symmetric distance percentages for forecast
        forecast_range = forecast_max - forecast_min
        if forecast_range > 0:
            position_from_forecast_min = current_close - forecast_min
            pct_from_forecast_min = (position_from_forecast_min / forecast_range) * 100
            pct_from_forecast_max = 100 - pct_from_forecast_min
        else:
            pct_from_forecast_min = 50  # Default to middle if range is zero
            pct_from_forecast_max = 50
        
        # Only return if forecast shows at least 2% movement
        if abs(pct_from_forecast_min - 50) < 2 and abs(pct_from_forecast_max - 50) < 2:
            return None
        ...
        return {
            ...
            'pct_from_forecast_min': pct_from_forecast_min,
            'pct_from_forecast_max': pct_from_forecast_max,
            'pct_from_extreme': pct_from_extreme,
            'pct_from_counterpart': pct_from_counterpart
        }
    ...
    def analyze_pair_on_tf(self, symbol, tf_name, tf_interval):
        ...
        try:
            ...
            if sine_values is None:
                return None
            
            # Extract values from sine_values dictionary
            extreme_type = sine_values['extreme_type']
            extreme_price = sine_values['extreme_price']
            counterpart_type = sine_values['counterpart_type']
            counterpart_price = sine_values['counterpart_price']
            current_sine_value = sine_values['current_sine_value']
            forecast_min = sine_values['forecast_min']
            forecast_max = sine_values['forecast_max']
            forecast_middle = sine_values['forecast_middle']
            pct_from_forecast_min = sine_values['pct_from_forecast_min']
            pct_from_forecast_max = sine_values['pct_from_forecast_max']
            pct_from_extreme = sine_values['pct_from_extreme']
            pct_from_counterpart = sine_values['pct_from_counterpart']
            ...
            # Determine if we're in a dip based on extreme type
            is_dip = (extreme_type == "DIP")
            ...
            fib_range = counterpart_price - extreme_price
            fib_23_6 = counterpart_price - 0.236 * fib_range
            fib_38_2 = counterpart_price - 0.382 * fib_range
            fib_50 = counterpart_price - 0.5 * fib_range
            fib_61_8 = counterpart_price - 0.618 * fib_range
            fib_78_6 = counterpart_price - 0.786 * fib_range
            
            # Calculate support and resistance levels
            if is_dip:
                support_levels = [extreme_price, fib_38_2, fib_61_8]
                resistance_levels = [fib_23_6, fib_50, fib_78_6, counterpart_price]
            else:
                support_levels = [counterpart_price, fib_38_2, fib_61_8]
                resistance_levels = [fib_23_6, fib_50, fib_78_6, extreme_price]
    ...
    def scan_assets_on_tf(self, symbols, tf_name, tf_interval):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            # Process results as they complete with instant progress feedback
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    # Print progress for every asset
                    sys.stdout.write(f"\rProgress: {completed_count}/{total_count} ({int(completed_count/total_count*100)}%)")
                    sys.stdout.flush()
                    
                    if result and result['is_dip']:
                        dip_assets.append(result)
                        # Print immediate result for this asset
                        print(f"\n✓ {result['symbol']} - DIP found on {tf_name} ({len(dip_assets)}/{total_count})")
                except Exception as e:
    ...
    def gradual_mtf_scan(self):
        ...
        # Sort by extreme price (lower is better for a dip) and then by position from extreme
        if final_results and '24h' in final_results[0]['timeframes']:
            final_results.sort(key=lambda x: (
                x['timeframes']['24h']['extreme_price'],  # Lower extreme price is better
                x['timeframes']['24h']['pct_from_extreme']  # Lower percentage from extreme is better
            ))
    ...
    def print_technical_data(self, asset_data):
        ...
        for tf_name, tf_data in asset_data['timeframes'].items():
            ...
            print("\nExtreme Analysis:")
            print(f"  Most Recent Extreme:    {tf_data['extreme_type']} at {self.format_decimal(tf_data['extreme_price'])}")
            print(f"  Counterpart Extreme:    {tf_data['counterpart_type']} at {self.format_decimal(tf_data['counterpart_price'])}")
            print(f"  Total Range:            {self.format_decimal(tf_data['total_range'])}")
            print(f"  Position from Extreme:  {self.format_decimal(tf_data['pct_from_extreme'])}%")
            print(f"  Position from Counterpart: {self.format_decimal(tf_data['pct_from_counterpart'])}%")
            ...
            print(f"  Distance to Min (%):    {self.format_decimal(tf_data['pct_from_forecast_min'])}%")
            print(f"  Distance to Max (%):    {self.format_decimal(tf_data['pct_from_forecast_max'])}%")
            
            # Fibonacci Retracement Table
            print("\nFibonacci Retracement Levels:")
            print(f"  0% ({tf_data['counterpart_type']}): {self.format_decimal(tf_data['counterpart_price'])}")
            print(f"  23.6%:                  {self.format_decimal(tf_data['fib_23_6'])}")
            print(f"  38.2%:                  {self.format_decimal(tf_data['fib_38_2'])}")
            print(f"  50%:                    {self.format_decimal(tf_data['fib_50'])}")
            print(f"  61.8%:                  {self.format_decimal(tf_data['fib_61_8'])}")
            print(f"  78.6%:                  {self.format_decimal(tf_data['fib_78_6'])}")
            print(f"  100% ({tf_data['extreme_type']}): {self.format_decimal(tf_data['extreme_price'])}")
            
            # Support and Resistance Table
            print("\nSupport and Resistance Levels:")
            print("  Support Levels:")
            for i, level in enumerate(tf_data['support_levels'], 1):
                print(f"    S{i}: {self.format_decimal(level)}")
            print("  Resistance Levels:")
            for i, level in enumerate(tf_data['resistance_levels'], 1):
                print(f"    R{i}: {self.format_decimal(level)}")
    ...
    def run_trading_bot(self):
        ...
        print("Bot will scan for MTF dip assets every 10 seconds")
        ...
        while True:
            try:
                ...
                print("\nWaiting 10 seconds for next iteration...")
                time.sleep(10)
                
                # Reduced frequency of garbage collection
                if int(time.time()) % 60 == 0:  # Only run GC every 60 seconds
                    gc.collect()
