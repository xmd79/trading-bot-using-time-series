...
def get_target(closes, n_components, reversal_type="NONE", min_threshold=Decimal('0'), middle_threshold=Decimal('0'), max_threshold=Decimal('0')):
    ...
    # Validate predominant frequency sign
...
def calculate_support_resistance(candles, timeframe):
    ...
    # Calculate support levels (bullish price areas)
    ...
    for price in lows:
        ...
        price_key = (price // price_range) * price_range  # Bucket prices within tolerance
        price_counts[price_key] += 1
    
    # Get top 3 most frequent price levels as support
    for price, count in price_counts.most_common(3):
        if count >= 3:  # Require at least 3 touches for consistency
            support_levels.append({"price": price, "touches": count})
    
    # Calculate resistance levels (bearish volume areas)
    resistance_levels = []
    ...
    )[:10]  # Top 10 high-volume bearish candles
    ...
    # Get top 3 most significant volume resistance levels
            ...
            for timeframe in timeframes:
                ...
                if len(closes) >= 14:
                    ...
                    if len(momentum) > 0 and not np.isnan(momentum[-1]):
                        ...
                        logging.info(f"{timeframe} Momentum: {current_momentum:.25f} - {'Positive' if conditions_long[f'momentum_positive_{timeframe}'] else 'Negative'}")
                        print(f"{timeframe} Momentum: {current_momentum:.25f} - {'Positive' if conditions_long[f'momentum_positive_{timeframe}'] else 'Negative'}")
                else:
