    ...
    try:
        ...
        while True:
            try:
                ...
                if position["side"] != "NONE" and position["sl_price"] > Decimal('0'):
                    if position["side"] == "LONG" and current_price <= position["sl_price"]:
                        ...
                        try:
                            ...
                            logging.error(f"Error sending stop-loss message: {e}")
                        ...
                        try:
                            ...
                            logging.error(f"Error sending take-profit message: {e}")
                        close_position(position, current_price)
                        position = get_position()
                ...
                # Initialize conditions with new DIP/TOP and middle threshold conditions
                conditions_long = {
                ...
                # Calculate percentages of true conditions
                long_percentage = long_true_count / total_required_long if total_required_long > 0 else 0
                short_percentage = short_true_count / total_required_short if total_required_short > 0 else 0
                
                print(f"\nLONG Conditions: {long_true_count}/{total_required_long} ({long_percentage:.1%})")
                print(f"SHORT Conditions: {short_true_count}/{total_required_short} ({short_percentage:.1%})")
                
                # Determine signal based on which side has more true conditions
                signal = "NO_SIGNAL"
                # Only trigger signal if at least SIGNAL_CONDITION_THRESHOLD conditions are true
                if long_true_count >= SIGNAL_CONDITION_THRESHOLD and long_true_count > short_true_count:
                    signal = "LONG"
                elif short_true_count >= SIGNAL_CONDITION_THRESHOLD and short_true_count > long_true_count:
                    signal = "SHORT"
                ...
                # Only send alert if at least SIGNAL_CONDITION_THRESHOLD conditions are true
                should_send_alert = (long_true_count >= SIGNAL_CONDITION_THRESHOLD) or (short_true_count >= SIGNAL_CONDITION_THRESHOLD)
                ...
                if should_send_alert:
                    ...
                    telegram_analysis_message = (
                        ...
                        f"LONG Conditions: {long_true_count}/{total_required_long} ({long_percentage:.1%})\n"
                ...
                try:
                    telegram_loop.run_until_complete(send_telegram_message(f"Bot encountered an error: {str(e)}"))
        ...
        try:
            ...
            logging.error(f"Failed to send fatal error message to Telegram: {telegram_error}")
