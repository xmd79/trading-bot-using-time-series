...
TIMEFRAMES = ["1m", "3m"]  # Use 1m and 3m timeframes
LOOKBACK_PERIODS = {"1m": 1500, "3m": 1500}
VOLUME_CONFIRMATION_RATIO = Decimal('1.5')  # Buy/Sell volume ratio for analysis
...
RECENT_LOOKBACK = {"1m": 100, "3m": 50}  # Recent candles for short-term reversal
TIMEFRAME_INTERVALS = {"1m": 60, "3m": 180}  # Timeframe intervals in seconds
TOLERANCE_FACTORS = {"1m": Decimal('0.10'), "3m": Decimal('0.08')}  # Dynamic tolerance
...
try:
    ...
    if len(lines) != 4:
        logging.error(f"credentials.txt must contain 4 non-empty lines, found {len(lines)}.")
        print(f"Error: credentials.txt must contain 4 non-empty lines, found {len(lines)}.")
        exit(1)
...
async def validate_telegram_chat_id(app, chat_id):
    try:
        ...
        if chat.type in ["private", "group", "supergroup", "channel"]:
            ...
            logging.error(f"Invalid Telegram chat ID: {chat_id}. Type '{chat.type}' unsupported.")
            print(f"Error: Invalid Telegram chat ID: {chat_id}. Type '{chat.type}' unsupported.")
            return False
    except Exception as e:
        if "Forbidden" in str(e):
            logging.error(f"Telegram chat ID {chat_id} invalid or belongs to a bot.")
            print(f"Error: Telegram chat ID {chat_id} invalid or belongs to a bot.")
        else:
...
async def test_telegram():
    test_message = f"*Test Message*\nSent from {TRADE_SYMBOL} trading bot at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def run_telegram_test():
    try:
        telegram_loop.run_until_complete(test_telegram())
    except Exception as e:
        logging.error(f"Error in Telegram test: {e}")
...
def calculate_quantity(balance, price):
    if price <= Decimal('0') or balance < MINIMUM_BALANCE:
        ...
        logging.info(f"Insufficient balance {balance:.25f} USDC or invalid price {price:.25f}")
        return Decimal('0.0')
...
def main():
    try:
        ...
        while True:
            ...
            conditions_long = {f"{k}_{tf}": False for tf in TIMEFRAMES for k in ["momentum_positive", "fft_bullish", "dip_confirmed", "below_middle", "hurst_up"]}
            conditions_short = {f"{k}_{tf}": False for tf in TIMEFRAMES for k in ["momentum_negative", "fft_bearish", "top_confirmed", "above_middle", "hurst_down"]}
            ...
            for cond in ["momentum_positive", "fft_bullish", "dip_confirmed", "below_middle", "hurst_up"]:
                for tf in TIMEFRAMES:
                    print(f"{cond}_{tf}: {conditions_long.get(f'{cond}_{tf}', False)}")
            
            print("\nShort Conditions Status:")
            for cond in ["momentum_negative", "fft_bearish", "top_confirmed", "above_middle", "hurst_down"]:
                for tf in TIMEFRAMES:
