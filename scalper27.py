...
# Generate a simple trading signal based on EMA crossovers.
...
def generate_report(timeframe, candles, investment, forecast_minutes=12):
    ...
    print(f"Trading Signal: {generate_signal(candles)}")  # Added Trading Signal
    ...
    if predictions is not None:
        ...
        pl_amount, pl_percentage = calculate_profit_loss(current_close, minor_reversal_target)
        print(f"Potential Profit/Loss for Minor Target: Amount = {pl_amount:.2f}, Percentage = {pl_percentage:.2f}%")

        pl_amount, pl_percentage = calculate_profit_loss(current_close, major_reversal_target)
