...
def scale_to_sine_freq(timeframe):
    ...
    print(f"Current Momentum: {current_close - close_prices[-2]}")
    print(f"Current Harmonics: {sine_wave}")
    print(f"Current Octave: {leverage_octave_calculation(sine_wave)}")
    print(
        f"Current Rate to Eman Ratio Percentage: {calculate_rate_to_eman_ratio_percentage(sine_wave)}"
    )
    print(f"Current Frequency Spectrum: {calculate_frequency_spectrum(sine_wave)}")
    print(
        f"Average Distance to Min: {avg_dist_min:.2f}%"
        f" and Average Distance to Max: {avg_dist_max:.2f}%"
    )
    print()

# Placeholder function for octave calculation
def leverage_octave_calculation(sine_wave):
    # Example: Calculate octave based on the range of sine values
    return np.max(sine_wave) - np.min(sine_wave)

# Placeholder function for rate to eman ratio percentage calculation
...
# Placeholder function for frequency spectrum calculation
def calculate_frequency_spectrum(sine_wave):
    # Example: Calculate frequency spectrum
    return np.fft.fft(sine_wave)

# Call function
for timeframe in timeframes:
    scale_to_sine_freq(timeframe)
            ...
            with open("signals.txt", "a") as f:
                ...
                if un_realized_profit == 0:  
                    ...
                    if normalized_distance_to_min < normalized_distance_to_max:
                        ...
                        if pct_diff_to_min < 30:
                            print("LONG condition 2: pct_diff_to_min < 30")
                            if price < fastest_target:
                                print("LONG condition 3: price < fastest_target")
                                if price < incoming_reversal_keypoint:
                                    print("LONG condition 4: price < incoming_reversal_keypoint")
                                    if price < avg_mtf:
                                        print("LONG condition 5: price < avg_mtf") 
                                        if closest_threshold < price:
                                            print("LONG condition 6: closest_threshold < price")  
                    ...
                    if normalized_distance_to_max < normalized_distance_to_min:
                        ...
                        if pct_diff_to_max < 30:
                            print("SHORT condition 2: pct_diff_to_max < 30")
                            if price > fastest_target:
                                print("SHORT condition 3: price > fastest_target")
                                if price > incoming_reversal_keypoint:
                                    print("SHORT condition 4: price > incoming_reversal_keypoint")
                                    if price > avg_mtf:
                                        print("SHORT condition 5: price > avg_mtf") 
                                        if closest_threshold > price:
                                            print("SHORT condition 6: closest_threshold > price")  
