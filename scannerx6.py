...
class TradingBot:
            ...
            if analysis.get('1d', {}).get('Trend Status') == "Uptrend":
                ...
                if small_tf_dips > 0:  # If there are dips on smaller timeframes
                    uptrend_assets[symbol] = {
                        ...
                        'avg_forecast_price': np.mean([
                            analysis[interval].get("FFT Forecast Price") for interval in ["4h", "1h", "15m"]
                        ])
                    }

        # Create a DataFrame for displaying
        uptrend_df = pd.DataFrame.from_dict(uptrend_assets, orient='index')
        print("\nDaily Uptrend Assets with Dips on Smaller Timeframes:")
        print(uptrend_df.to_string(index=True))
            ...
            if analysis.get('1d', {}).get('Trend Status') == "Dip":
                ...
                if dip_count > 0:
                    dip_assets[symbol] = {
                        ...
                        'last_close_price': analysis['1d']['Current Close'] if '1d' in analysis else None
                    }

        # Create a DataFrame for displaying
        dip_df = pd.DataFrame.from_dict(dip_assets, orient='index')
        print("\nDaily Dip Assets with Lowest Bullish Momentum:")
        print(dip_df.to_string(index=True))
    ...
    def scan_assets(self):
        ...
        self.get_top_uptrend_assets(overall_analysis)  # Call the modified method here
        self.get_top_dip_assets(overall_analysis)      # Call the modified method here
