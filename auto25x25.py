...
def _physical_side_for_unified(unified_idx):
    ...
    return "UP" if unified_idx <= 3 else "DOWN"


def _cycle_tag_at_progress(unified_idx, quad_progress_pct):
    """
...
def compute_harmonic_oscillator(candles_5m, close_arr_5m, live_price,
    ...
    cycle_tag = _cycle_tag_at_progress(confirmed_unified, quad_progress_pct)
    if cycle_tag == "UP":
        ho_long, ho_short, trend_str = True, False, "UP"
    else:
        ho_long, ho_short, trend_str = False, True, "DOWN"

    # Active/current tag uses the midpoint-shifted cycle rule above (agrees
    # with the header's "Active: X-CYCLE" line). Last/Incoming use each
    # stage's own NATURAL physical side instead — Distribution/Falling are
    # always tagged DOWN, never flipped to UP just because the currently
    # active stage's cycle happens to read UP.
    active_label = f"{cycle_tag}:{UNIFIED_STAGE_LABELS[confirmed_unified]}"
...
def print_conditions(sig):
    ...
    ql       = sig["quad_label"]          # e.g. "UP:Q2u-ACCUMULATION" or "DOWN:Q3d-DISTRIBUTION"
    cycle_tag = ql.split(":")[0]          # "UP" or "DOWN"
    ...
    print(f"  │ Active: {cycle_tag}-CYCLE   Phase: {sig['theta_deg']:>6.1f}°                       │")
