...
for timeframe in timeframes:
    ...
    # Find the most recent significant dip and top 
    ...
    # Determine which approach (DIP or TOP) is closer to the current close
    ...
    # Set price forecast target based on last major reversal
    ...
    # Print major reversals with pricing
    print(f"\nTimeframe: {timeframe}")
    ...
    print(f"Price Forecast Target: {price_forecast_target:.2f}")
    ...
    scaled_pct_to_min = (current_close - min_threshold) / (max_threshold - min_threshold) * 100 if max_threshold != min_threshold else 0
    scaled_pct_to_max = (max_threshold - current_close) / (max_threshold - min_threshold) * 100 if max_threshold != min_threshold else 0
    print(f"Scaled Percentage to Min: {scaled_pct_to_min:.2f}%, Scaled Percentage to Max: {scaled_pct_to_max:.2f}%")
    ...
    summary_data.append({
        ...
        "Scaled to Min %": scaled_pct_to_min,
        "Scaled to Max %": scaled_pct_to_max
    })

# Create Summary Table
print("\nSummary of All Timeframes:")
print("=" * 120)
print(f"{'Timeframe':<10} {'Last Reversal':<15} {'Reversal Price':<15} {'Cycle Direction':<15} {'FFT Target':<15} {'Min Threshold':<15} {'Max Threshold':<15} {'Support':<15} {'Resistance':<15} {'Bullish Volume':<15} {'Bearish Volume':<15} {'Total Volume':<15} {'Bullish Ratio %':<15} {'Bearish Ratio %':<15} {'Scaled to Min %':<15} {'Scaled to Max %':<15}")
print("=" * 120)
for data in summary_data:
    print(f"{data['Timeframe']:10} {data['Last Major Reversal Type']:15} {data['Last Major Reversal Price']:15.2f} {data['Cycle Direction']:15} {data['FFT Forecast Price Target']:15.2f} {data['Min Threshold']:15.2f} {data['Max Threshold']:15.2f} {data['Support']:15.2f} {data['Resistance']:15.2f} {data['Bullish Volume']:15.2f} {data['Bearish Volume']:15.2f} {data['Total Volume']:15.2f} {data['Bullish Ratio %']:15.2f} {data['Bearish Ratio %']:15.2f} {data['Scaled to Min %']:15.2f} {data['Scaled to Max %']:15.2f}")
