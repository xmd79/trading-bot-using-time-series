...
def calculate_spectral_analysis(prices):
    ...
    # Filter to identify frequencies
...
def calculate_45_degree_projection(last_bottom, last_top):
    ...
    if last_bottom is not None and last_top is not None:
        # Calculate the distance and slope for a 45-degree projection
        distance = last_top - last_bottom
        projection = last_top + distance  # Matching the distance for a 45-degree rise
        return projection
    return None
...
def find_specific_support_resistance(candle_map, min_threshold, max_threshold, current_close):
    ...
    for timeframe in candle_map:
        ...
        # Calculate bullish volumes for support levels
        for candle in candles:
            if candle["close"] < current_close and candle["close"] >= min_threshold:  # Support must be below current close and above min threshold
                support_levels.append((candle["close"], candle["volume"]))

        # Calculate bearish volumes for resistance levels
        for candle in candles:
            if candle["close"] > current_close and candle["close"] <= max_threshold:  # Resistance must be above current close and below max threshold
    ...
    # Consolidation and distribution analysis
    if support_levels:
        avg_support_volume = np.mean([lvl[1] for lvl in support_levels])
        print(f"Average Support Volume: {avg_support_volume:.2f}")
    ...
    if resistance_levels:
        ...
        print(f"Average Resistance Volume: {avg_resistance_volume:.2f}")
...
while True:  
    ...
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map:
            ...
            conditions_status["forecast_price_condition_met"] = (adjusted_forecasted_price is not None) and (current_close < adjusted_forecasted_price)
            ...
            print(f"Forecast Price: {adjusted_forecasted_price:.2f}" if adjusted_forecasted_price is not None else "No Forecast Price")
