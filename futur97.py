...
def calculate_momentum_trend(candles, timeframe, period=MOMENTUM_PERIOD, lookback=MOMENTUM_LOOKBACK):
    ...
    if len(momentum) >= lookback:
        ...
        # Check if momentum is increasing (last value > first value in lookback)
        momentum_increasing = recent_momentum[-1] > recent_momentum[0]
        
        # Check if momentum is decreasing (last value < first value in lookback)
        momentum_decreasing = recent_momentum[-1] < recent_momentum[0]
...
def main():
    try:
        ...
        while True:
            ...
            # Updated conditions for momentum analysis
            conditions_long = {
                "momentum_negative_1m": False,  # Momentum is negative
                "momentum_increasing_1m": False,  # Momentum is increasing
            ...
            conditions_short = {
                "momentum_positive_1m": False,  # Momentum is positive
                "momentum_decreasing_1m": False,  # Momentum is decreasing
            ...
            for timeframe in TIMEFRAMES:
                ...
                # Update momentum conditions specifically for 1m timeframe
                if timeframe == "1m" and len(momentum_values) > 0:
                    # For LONG: momentum is negative
                    conditions_long["momentum_negative_1m"] = momentum_values[-1] < 0
                    
                    # For LONG: momentum is increasing
                    conditions_long["momentum_increasing_1m"] = momentum_increasing
                    
                    # For SHORT: momentum is positive
                    conditions_short["momentum_positive_1m"] = momentum_values[-1] > 0
                    
                    # For SHORT: momentum is decreasing
                ...
                # Print new momentum conditions for 1m timeframe
                if timeframe == "1m":
                    print(f"Momentum Value: {momentum_values[-1] if len(momentum_values) > 0 else 'N/A'}")
                    print(f"Momentum Increasing: {momentum_increasing}")
                    print(f"Momentum Decreasing: {momentum_decreasing}")
                    print(f"Momentum Negative (for LONG): {conditions_long['momentum_negative_1m']}")
                    print(f"Momentum Increasing (for LONG): {conditions_long['momentum_increasing_1m']}")
                    print(f"Momentum Positive (for SHORT): {conditions_short['momentum_positive_1m']}")
                    print(f"Momentum Decreasing (for SHORT): {conditions_short['momentum_decreasing_1m']}")
            ...
            for cond_pair in [
                ("momentum_negative_1m", "momentum_positive_1m"),
