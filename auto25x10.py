...
====================================================================
SIGNAL LOGIC:
  - MAJORITY + VOLUME RULE: more conditions Long than Short AND volume bullish -> LONG
                             more conditions Short than Long AND volume bearish -> SHORT
  - 1. Sine Scale (5m)        — dist_to_min < dist_to_max -> LONG
  - 2. Extrema Cycle (1m)     — Strictly Lowest Low vs Highest High (last 500)
  - 3. MOM Extrema (1m)       — talib.MOM last 500: if most-recent extreme was lowest-low
                                 and MOM rising last 5 bars -> LONG; if most-recent was
                                 highest-high and MOM falling last 5 bars -> SHORT
  - 4. Volume (1m)            — bullish% > bearish% -> LONG (also used as tie-breaker)
  - 5. FFT Spectrum (1m)      — HT_SINE cycle type "Up" -> LONG, "Down" -> SHORT
  - 6. ML Forecast (1m)       — Random Forest forecast_price > live_price -> LONG

  Conditions 7 (FFT Mood), 8 (SMA 1m), 9 (SMA 5m), 10 (LinReg) removed.
...
print("HFT KuCoin Bot (25x / ALL 7 MANDATORY) initialising...")
if _cleaned:
...
def compute_signals(buffer_5m, buffer_1m, live_price):
    ...
    # 1. Sine Scale (1m)
    last_500_1m = close_arr_1m[-ANALYSIS_WINDOW_1M:]
    argmin_idx_sine_1m = int(np.argmin(last_500_1m))
    argmax_idx_sine_1m = int(np.argmax(last_500_1m))
    dist_to_min, dist_to_max = scale_to_sine(close_arr_1m, argmin_idx_sine_1m, argmax_idx_sine_1m)
    ...
    # 3a. MOM Extrema Recency (1m) — which extreme occurred more recently
    #     argmin more recent → LONG | argmax more recent → SHORT — always binary
    # 3b. MOM Slope last-5 (1m) — is MOM rising or falling over last 5 bars — always binary
    mom_period = 14
    if len(close_arr_1m) >= 500 + mom_period:
        mom_500_full = talib.MOM(close_arr_1m, timeperiod=mom_period)
        mom_500 = mom_500_full[-500:]
    else:
        mom_500 = talib.MOM(close_arr_1m, timeperiod=mom_period)
    mom_500 = np.nan_to_num(mom_500, nan=0.0)
    mom_1m = float(mom_500[-1])  # current MOM value for display

    mom_argmin = int(np.argmin(mom_500))
    mom_argmax = int(np.argmax(mom_500))
    mom_len = len(mom_500)

    # bars_ago: higher = older
    mom_bars_ago_min = (mom_len - 1) - mom_argmin
    mom_bars_ago_max = (mom_len - 1) - mom_argmax

    # 3a: recency — small bars_ago = more recent
    #   most-negative MOM extreme most recent → LONG
    #   most-positive MOM extreme most recent → SHORT
    #   tie → Long
    cond_mom_recency_long  = (mom_bars_ago_min <= mom_bars_ago_max)
    cond_mom_recency_short = (mom_bars_ago_max <  mom_bars_ago_min)

    # 3b: slope of last 5 MOM values — strictly binary, flat → Long
    mom_tail = mom_500[-5:]
    cond_mom_slope_long  = bool(mom_tail[-1] >= mom_tail[0])
    cond_mom_slope_short = bool(mom_tail[-1] <  mom_tail[0])

    mom_extreme_label = ("RECENT=MIN" if mom_bars_ago_min < mom_bars_ago_max
                         else "RECENT=MAX" if mom_bars_ago_max < mom_bars_ago_min
                         else "TIE→L")
    mom_rising  = cond_mom_slope_long
    mom_falling = cond_mom_slope_short
    ...
    # ══════════════════════════════════════════════════════════════════
    # CONDITIONS 7-10 REMOVED (FFT Mood, SMA 1m, SMA 5m, LinReg)
    # ══════════════════════════════════════════════════════════════════

    # ══════════════════════════════════════════════════════════════════
    # ENTRY LOGIC: MAJORITY + VOLUME TIE-BREAK (6 conditions)
    # ALL 7 CONDITIONS MANDATORY — every condition must agree for signal to fire
    long_votes  = sum([cond_sine_long,  cond_cycle_long,
                       cond_mom_recency_long,  cond_mom_slope_long,
                       cond_vol_long,   cond_fft_long,    cond_ml_long])
    short_votes = sum([cond_sine_short, cond_cycle_short,
                       cond_mom_recency_short, cond_mom_slope_short,
                       cond_vol_short,  cond_fft_short,   cond_ml_short])

    is_long  = (long_votes  == 7)
    is_short = (short_votes == 7)
    ...
    return {
        ...
        "cond_flags": {
            ...
            "mom_recency_long": cond_mom_recency_long, "mom_recency_short": cond_mom_recency_short,
            "mom_slope_long": cond_mom_slope_long, "mom_slope_short": cond_mom_slope_short,
        ...
        "mom_1m": mom_1m,
        "mom_extreme_label": mom_extreme_label,
        "mom_rising": mom_rising, "mom_falling": mom_falling,
        "mom_bars_ago_min": mom_bars_ago_min, "mom_bars_ago_max": mom_bars_ago_max,
...
def write_trade_to_journal(trade_result):
    ...
    line = (f"{'='*60}\nMODE: {mode}\nTYPE: {trade_result.get('type', 'N/A').upper()}\n"
            ...
            f"LEVERAGE: {LEVERAGE}x\nSTRATEGY: ALL 7 MANDATORY | SizeAlloc: {TRADE_BALANCE_PCT*100:.0f}%\n{'='*60}\n\n")
    try:
...
def print_conditions(sig):
    ...
    mom_lbl  = sig.get('mom_extreme_label', 'N/A')
    mom_rise = sig.get('mom_rising', False)
    mom_fall = sig.get('mom_falling', False)
    bago_min = sig.get('mom_bars_ago_min', 0)
    bago_max = sig.get('mom_bars_ago_max', 0)
    print(f"  3a.MOM Recency(1m):Val:{sig['mom_1m']:.2f} {mom_lbl} | MinAgo:{bago_min} MaxAgo:{bago_max} L:{f['mom_recency_long']} S:{f['mom_recency_short']} [MANDATORY]")
    print(f"  3b.MOM Slope (1m): Rising:{mom_rise} Falling:{mom_fall} L:{f['mom_slope_long']} S:{f['mom_slope_short']} [MANDATORY]")

    print(f"  4. Vol (1m):       Bull:{sig['bullish_perc']:.1f}% Bear:{sig['bearish_perc']:.1f}% L:{f['vol_long']} S:{f['vol_short']} [MANDATORY+TIE-BREAK]")
    ...
    lv = sig.get('long_votes', 0)
    sv = sig.get('short_votes', 0)
    print(f"  ═══ LONG:{lv}/7 SHORT:{sv}/7 (ALL 7 MANDATORY) -> LONG:{sig['is_long']} SHORT:{sig['is_short']}")
...
def main():
    ...
    while True:
        try:
            ...
            # ── ORPHANED LIVE STATE CLEANUP ──
            if saved_state and saved_state.get("mode") == "LIVE" and not pos["is_open"]:
                print(f"  [recovery] Orphaned LIVE state - clearing.")
                clear_trade_state()
                saved_state = None

            # ── ORPHANED OTHER STATE CLEANUP ──
            ...
            if sig:
                ...
                if sig["is_long"] or sig["is_short"]:
                    ...
                    if API_CONNECTED and has_sufficient_balance:
                        ...
                        # Simulation: either no API connection or insufficient balance
                        reason_str = "no API" if not API_CONNECTED else f"low balance ({balance:.2f} < {MIN_BALANCE_USDT})"
                        sl_price, tp_price = calculate_sl_tp(current_price, "long" if sig["is_long"] else "short")
                        new_state = {
                        ...
                        save_trade_state(new_state)
                        saved_state = new_state
                        print(f"  [sim] SIMULATION {side_label} @ {current_price:.2f} ({reason_str})")
