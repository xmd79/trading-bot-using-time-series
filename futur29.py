...
def get_target(closes, n_components, reversal_type="NONE", min_threshold=Decimal('0'), middle_threshold=Decimal('0'), max_threshold=Decimal('0'), timeframe="1m"):
    ...
    # Adjust fastest_target for 1m timeframe based on market mood
    if timeframe == "1m":
        if is_bearish_target:
            fastest_target = min_threshold * (Decimal('1') + PROXIMITY_THRESHOLD)  # Near min_threshold, below current_close
            if fastest_target >= current_close:
                fastest_target = current_close * Decimal('0.995')  # Ensure below current_close
        else:  # Bullish
            fastest_target = max_threshold * (Decimal('1') - PROXIMITY_THRESHOLD)  # Near max_threshold, above current_close
            if fastest_target <= current_close:
    ...
    try:
        while True:
            ...
            conditions_long = {
                ...
                "dip_confirmation_1m": False, "dip_confirmation_3m": False, "dip_confirmation_5m": False,
                "below_middle_1m": False, "below_middle_3m": False, "below_middle_5m": False,
            ...
            conditions_short = {
                ...
                "top_confirmation_1m": False, "top_confirmation_3m": False, "top_confirmation_5m": False,
                "above_middle_1m": False, "above_middle_3m": False, "above_middle_5m": False,
            ...
            for timeframe in timeframes:
                ...
                logging.info(f"{timeframe} - Current Close: {current_close:.25f}, Below Middle: {conditions_long[f'below_middle_{timeframe}']}")
                print(f"{timeframe} - Current Close: {current_close:.25f}")
                ...
                logging.info(f"Volume Bullish ({timeframe}): {buy_vol:.25f}, Bearish: {sell_vol:.25f}")
                ...
                logging.info(f"{timeframe} - Dip Confirmation: {conditions_long[f'dip_confirmation_{timeframe}']}")
