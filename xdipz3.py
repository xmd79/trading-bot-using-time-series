...
def analyze_wave(t, wave, frequency, sampling_rate, df):
    ...
    print(f"Current Close Real Price: {df['Close'].values[-1]}")
    ...
    angle_45_value = np.sin(np.pi / 4)
    angle_distance = np.abs(current_close_sine_value - angle_45_value)
    max_distance = np.abs(np.max(wave) - angle_45_value)
    min_distance = np.abs(np.min(wave) - angle_45_value)
    total_angle_distance = max_distance + min_distance
    percentage_to_angle = (angle_distance / total_angle_distance) * 100

    angle_status = "above" if current_close_sine_value > angle_45_value else "below"

    print(f"Current Close Value on Sine is {angle_status} 45-degree angle.")
    print(f"Percentage Distance to 45-degree angle: {percentage_to_angle}%")
