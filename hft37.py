...
def main():
    ...
    sine_wave = em_amp * np.sin(2 * math.pi * wave_freq * t + em_phase)
        ...
        try:
            ...
            if '1m' in signals:
                ...
                if 'ht_sine_percent_to_min' in signals['1m'] and 'ht_sine_percent_to_max' in signals['1m']:
                    ...
                    # Calculate the EM field populated on Talib HT_SINE
                    em_field = np.array(talib.HT_SINE(close_prices)[-len(sine_wave):])
                    em_field[np.isnan(em_field)] = 0  # set NaN values to 0
                    em_field = em_amp * em_field

                    print("EM field:", em_field)

                    # Calculate the frequencies spectrum range index based on the EM field
                    frequencies = np.fft.fftfreq(len(close_prices), d=60)
                    spectrum = np.fft.fft(close_prices)
                    spectrum_abs = np.abs(spectrum)
                    f_range = np.where(frequencies > 0, frequencies, np.inf)
                    em_field_freq_index = np.where(f_range == np.min(f_range[spectrum_abs > em_field[-1]]))[0][0]

                    print("EM field frequency index:", em_field_freq_index)
                    print()

                    # Calculate the percentages from close to min/max of sine
                    percent_to_min_sine = (close_prices[-1] - np.min(close_prices)) / (np.max(close_prices) - np.min(close_prices)) * 100
                    percent_to_max_sine = (np.max(close_prices) - close_prices[-1]) / (np.max(close_prices) - np.min(close_prices)) * 100

                    print("Percent to min of sine:", percent_to_min_sine)
                    print("Percent to max of sine:", percent_to_max_sine)
                    print()

                    # Check if the 'mtf_average' key exists in the '1m' dictionary
                    if 'mtf_average' in signals['1m']:
                        # Calculate the momentum value from the MTF signals
                        mtf_average = signals['1m']['mtf_average']
                        close_price = close_prices[-1]
                        momentum = close_price - mtf_average

                        print("Momentum value:", momentum)
                        print()

                        # Check if the current close price is below the MTF signal and below the min threshold of sine wave
                        if close_prices[-1] < mtf_average and close_prices[-1] < np.min(sine_wave):
                            print("Short trigger!")

                        # Check if the current close price is above the max threshold of sine wave and above the MTF signal or in the range of sine wave
                        elif close_prices[-1] > np.max(sine_wave) and (close_prices[-1] > mtf_average or close_prices[-1] in sine_wave):
                            print("Long trigger!")

                        # Check if the current close price is below the MTF signal and above the min threshold of sine wave
                        elif close_prices[-1] < mtf_average and close_prices[-1] > np.min(sine_wave):
                            print("Seeking local reversal.")

                        # Check if the current close price is above the MTF signal and below the max threshold of sine wave
                        elif close_prices[-1] > mtf_average and close_prices[-1] < np.max(sine_wave):
                            print("Seeking local reversal.")

                        # Check if the current price is below the EM field and the momentum is negative
                        elif close_prices[-1] < em_field[-1] and momentum < 0:
                            print("EM field short trigger!")

                        # Check if the current price is above the EM field and the momentum is positive
                        elif close_prices[-1] > em_field[-1] and momentum > 0:
                            print("EM field long trigger!")

                        # Check if the current price is within the range of the EM field frequency
                        elif em_field_freq_index - 2 <= np.where(frequencies > wave_freq)[0][0] <= em_field_freq_index + 2:
                            print("Frequency range trigger!")

                        else:
                            print("No trigger.")

                    else:
                        print("Error: 'mtf_average' not found in '1m' signals.")

                else:
                    print("Error: 'ht_sine_percent_to_min' or 'ht_sine_percent_to_max' not found in '1m' signals.")

            else:
                print("Error: '1m' signals not found.")
