...
while True:
    ...
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map and candle_map[timeframe]:
            ...
            print(f"Distance to Min Threshold ({low_tf:.25f}): {dist_to_min:.25f}% {'(Triggers LONG)' if long_conditions[f'dist_to_min_{timeframe}_less_10pct'] else ''}")
            print(f"Distance to Max Threshold ({high_tf:.25f}): {dist_to_max:.25f}% {'(Triggers SHORT)' if short_conditions[f'dist_to_max_{timeframe}_less_10pct'] else ''}")
            if timeframe in ['3m', '5m']:
                print(f"Volume Bullish {timeframe}: {'True' if long_conditions[f'volume_bullish_{timeframe}'] else 'False'}")
                print(f"Volume Bearish {timeframe}: {'True' if short_conditions[f'volume_bearish_{timeframe}'] else 'False'}")
    ...
    if position_open:
        ...
        print(f"Stop Loss Value (-25.50% Loss): {stop_loss_value:.25f} USDC")
