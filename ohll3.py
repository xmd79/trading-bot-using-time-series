...
def calculate_fear_and_greed_index(value):
    """Calculate and return the sentiment based on the fear and greed index."""
    if value < 25:
        return "Extreme Fear"
    elif value < 45:
        return "Fear"
...
def linear_regression_with_extensions(close_prices, dip_index, top_index):
    """Calculate a linear regression line and its upper and lower extensions."""
    # Define the range for regression
    if dip_index < top_index:
        x = np.array(range(dip_index, top_index + 1)).reshape(-1, 1)  # x-axis time index between dip and top
        y = np.array(close_prices[dip_index:top_index + 1]).reshape(-1, 1)  # closing prices in that range
    else:
        return None, None, None  # In case indices are wrong
    ...
    # Define the extensions (e.g., ±2 standard deviations)
    std_dev = np.std(y)
    upper_bound = y_pred + 2 * std_dev
    lower_bound = y_pred - 2 * std_dev

    return y_pred.flatten(), upper_bound.flatten(), lower_bound.flatten()

def plot_timeframe_analysis(close_prices, incoming_max_threshold, current_close, min_threshold, max_threshold,
                            middle_threshold, last_major_reversal_price, dip_index, top_index, timeframe):
    """Plot the close prices along with thresholds, regression lines, and reversal points for a given timeframe."""
    
    # Get regression lines and extensions
    y_pred, upper_bound, lower_bound = linear_regression_with_extensions(close_prices, dip_index, top_index)
    
    plt.figure(figsize=(12, 6))
    plt.plot(close_prices, label='Close Prices', color='blue', linewidth=1.5)
    ...
    plt.axhline(y=middle_threshold, color='orange', linestyle=':', label='Middle Threshold')
    plt.axhline(y=incoming_max_threshold, color='purple', linestyle='-.', label='Incoming Threshold')
    plt.axhline(y=current_close, color='black', linestyle=':', label='Current Close')

    # Plot the regression line and its bounds
    if y_pred is not None and upper_bound is not None and lower_bound is not None:
        plt.plot(np.arange(dip_index, top_index + 1), y_pred, color="cyan", linestyle='--', label='Regression Line')
        plt.plot(np.arange(dip_index, top_index + 1), upper_bound, color="magenta", linestyle='--', label='Upper Bound')
        plt.plot(np.arange(dip_index, top_index + 1), lower_bound, color="yellow", linestyle='--', label='Lower Bound')

    plt.axvline(x=len(close_prices) - 1, color='gray', linestyle=':', label='Current Point')
    plt.scatter(len(close_prices) - 1, current_close, color='purple', s=100)  # Current close point
    plt.scatter(len(close_prices) - 1, last_major_reversal_price, color='black', s=100, zorder=5,
                label='Last Major Reversal Price')

    plt.title(f'Timeframe: {timeframe} Analysis')
    plt.xlabel('Time Index')
    plt.ylabel('Price')
    plt.xticks(ticks=np.arange(len(close_prices)), labels=np.arange(len(close_prices)), rotation=45)
    plt.legend()
    plt.grid()

    # Display values at lines
    plt.text(len(close_prices) - 1, min_threshold, f'Min: {min_threshold:.2f}', color='green', fontsize=10, ha='center')
    plt.text(len(close_prices) - 1, max_threshold, f'Max: {max_threshold:.2f}', color='red', fontsize=10, ha='center')
    plt.text(len(close_prices) - 1, middle_threshold, f'Middle: {middle_threshold:.2f}', color='orange', fontsize=10, ha='center')
    plt.text(len(close_prices) - 1, incoming_max_threshold, f'Incoming: {incoming_max_threshold:.2f}', color='purple', fontsize=10, ha='center')
    plt.text(len(close_prices) - 1, current_close, f'Current: {current_close:.2f}', color='black', fontsize=10, ha='center')
...
for timeframe in timeframes:
    ...
    # Calculate volume ratios and threshold percentages
    ...
    # Fear and Greed Index Calculation (Dummy Value for demonstration)
    # You would replace the following variable with your actual calculation or data
    positive_frequencies = 12  # Number of positive frequencies
    negative_frequencies = 12  # Number of negative frequencies
    total_frequencies = positive_frequencies + negative_frequencies

    # Simulating a Fear & Greed value based on current state
    # You should replace this with actual metrics from your analysis
    simulated_value = 50  # This should be a dynamic value derived from your analysis logic
    sentiment = calculate_fear_and_greed_index(simulated_value)
    ...
    # Plot analysis for the current timeframe
    plot_timeframe_analysis(
        close, 
        incoming_max_threshold, 
        current_close, 
        min_threshold, 
        max_threshold, 
        middle_threshold, 
        last_major_reversal_price,
        last_reversal_index_dip, 
        last_reversal_index_top, 
        timeframe
    )
...
print("=" * 120)
print(f"{'Timeframe':<10} {'Last Reversal':<15} {'Reversal Price':<15} {'Cycle Direction':<15} {'Incoming Threshold':<15} {'FFT Target':<15} {'Min Threshold':<15} {'Max Threshold':<15} {'Middle Threshold':<15} {'Support':<15} {'Resistance':<15} {'Bullish Volume':<15} {'Bearish Volume':<15} {'Total Volume':<15} {'Bullish Ratio %':<15} {'Bearish Ratio %':<15} {'Dist to Min %':<15} {'Dist to Max %':<15} {'Fear & Greed':<20}")
print("=" * 120)
for data in summary_data:
    print(f"{data['Timeframe']:10} {data['Last Major Reversal Type']:15} {data['Last Major Reversal Price']:15.2f} {data['Cycle Direction']:15} {data['Incoming Threshold']:15.2f} {data['FFT Forecast Price Target']:15.2f} {data['Min Threshold']:15.2f} {data['Max Threshold']:15.2f} {data['Middle Threshold']:15.2f} {data['Support']:15.2f} {data['Resistance']:15.2f} {data['Bullish Volume']:15.2f} {data['Bearish Volume']:15.2f} {data['Total Volume']:15.2f} {data['Bullish Ratio %']:15.2f} {data['Bearish Ratio %']:15.2f} {data['Distance to Min Threshold %']:15.2f} {data['Distance to Max Threshold %']:15.2f} {data['Fear & Greed Index']:20}")