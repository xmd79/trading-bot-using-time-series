...
def check_exit_condition(initial_investment, asset_balance):
    ...
    target_value = Decimal(str(initial_investment)) * Decimal('1.0055')  # 0.55% profit target
...
while True:
    ...
    if position_open:
        ...
        target_value = initial_investment * Decimal('1.0055')  # Updated to 0.55% profit target
        ...
        print(f"Price for 0.55% Profit Target: {target_price:.25f}")

        # Percentage Distance to 0.55% Profit Target
        ...
        print(f"Percentage Distance to 0.55% Profit Target: {percentage_to_target:.25f}%")

        # Percentage Progress to 0.55% Profit Target
        percentage_progress = Decimal('100.0') - percentage_to_target
        print(f"Percentage Progress to 0.55% Profit Target: {percentage_progress:.25f}%")
        print()

        if check_exit_condition(float(initial_investment), float(asset_balance)):
            print("Target profit of 0.55% reached or exceeded. Initiating exit...")
