...
# Define a function to get the account balance in USDC (spot account)

def get_spot_account_balance():
    account = client.get_account()
    for asset in account['balances']:
        if asset['asset'] == 'USDC':
            usdc_balance = float(asset['free'])  # Get only the free balance
            return usdc_balance

# Get the USDT balance of the spot account
usdc_balance = get_spot_account_balance()

# Print account balance
print("USDC Spot balance:", usdc_balance)
...
def log_phi_square(x):
    phi = (1 + math.sqrt(5)) / 2
    return math.log(x, phi) ** 2

def inverse_powers_phi(i):
    phi_inverse = (1 - math.sqrt(5)) / 2
    return phi_inverse ** i

def detect_market_mood(close):
    if len(close) < 2:
        return None  # Not enough data to determine mood
    
    recent_reversal = None
    last_price, current_price = close[-2], close[-1]
    
    # Find the most recent major reversal (dip or top) in the close list
    if len(close) >= 2:
        close_np = np.array(close, dtype=float)  # Convert close prices to double
        min_val = talib.MIN(close_np[:-1], timeperiod=2)[-1]
        max_val = talib.MAX(close_np[:-1], timeperiod=2)[-1]
        
        if min_val is not None and max_val is not None:
            recent_reversal = min(min_val, max_val)
        elif min_val is not None:
            recent_reversal = min_val
        elif max_val is not None:
            recent_reversal = max_val

    # Determine market cycle type
    if recent_reversal is not None:
        if current_price > last_price and last_price > recent_reversal:  # Upward trend
            return "Up"
        elif current_price < last_price and last_price < recent_reversal:  # Downward trend
            return "Down"
        else:
            return None  # No significant change
    else:
        return None  # No recent major reversal detected

last_price, current_price = close[-2], close[-1]

# Find the market mood
market_mood = detect_market_mood(close)
if market_mood is not None:
    print("Market mood:", market_mood)
else:
    print("Unable to determine market mood.")

# Find the most recent major reversal
recent_reversal = None
if market_mood is not None:
    close_np = np.array(close, dtype=float)  # Convert close prices to double
    min_val = talib.MIN(close_np[:-1], timeperiod=2)[-1]
    if min_val is not None:
        print("Most recent major reversal price:", min_val)
        recent_reversal = min_val
    else:
        print("No recent major reversal detected")

# Calculate logarithmic phi square
log_phi_square_val = log_phi_square(9)
print("Logarithmic phi square of 9:", current_price + log_phi_square_val)

# Calculate inverse powers of phi as targets between current close and forecasted incoming reversal price
targets = []
if recent_reversal is not None:
    for i in range(1, 6):
        target_price = current_price - (current_price - recent_reversal) * inverse_powers_phi(i)
        targets.append(target_price)
        print(f"Inverse power of phi ({i}):", target_price)
else:
    print("No recent major reversal detected, cannot calculate inverse powers of phi.")

# Perform Fourier transform on the prices data for better forecasting
if market_mood is not None:
    forecast_direction = 1 if market_mood == "Up" else -1
    forecast_price = current_price + (current_price - last_price) * forecast_direction
    print("Forecasted price using Fourier transform:", forecast_price)
...
def main():
    ...
    while True:
        ...
        try:     
            ...
            if cycle_direction == "UP":
                print("LONG condition 5: cycle_direction == UP")
                long_conditions_met += 1
            ...
            if forecast_direction == "Up":
                print("LONG condition 8: forecast_direction == Up")
                long_conditions_met += 1
            ...
            if pivot_mood == "Bullish":
                print("LONG condition 10: pivot_mood == Bullish")
                long_conditions_met += 1

            if incoming_reversal == "Top":
                print("LONG condition 11: incoming_reversal == Top")
                long_conditions_met += 1
            ...
            if positive_count < negative_count:
                ...
                print("SHORT condition 14: positive_count = negative_count (assigned based on overall majority)")
                short_conditions_met += 1
            elif positive_count > negative_count and overall_majority == "LONG":
                print("LONG condition 14: positive_count > negative_count (assigned based on overall majority)")
                long_conditions_met += 1

            if positive_sine_count < negative_sine_count:
                print("SHORT condition 15: positive_sine_count < negative_sine_count") 
                short_conditions_met += 1
            elif positive_sine_count == negative_sine_count and overall_majority == "SHORT":
                print("SHORT condition 15: positive_sine_count = negative_sine_count (assigned based on overall majority)")
                short_conditions_met += 1
            elif positive_sine_count > negative_sine_count and overall_majority == "LONG":
                print("LONG condition 15: positive_sine_count > negative_sine_count (assigned based on overall majority)")
                long_conditions_met += 1
            ...
            if cycle_direction == "DOWN":
                print("SHORT condition 5: cycle_direction == DOWN")
                short_conditions_met += 1
            ...
            if forecast_direction == "Down":
                print("SHORT condition 8: forecast_direction == Down")
                short_conditions_met += 1
            ...
            if pivot_mood == "Bearish":
                print("SHORT condition 10: pivot_mood == Bearish")
                short_conditions_met += 1

            if incoming_reversal == "Dip":
                print("SHORT condition 11: incoming_reversal == Dip")
                short_conditions_met += 1
            ...
            if positive_sine_count > negative_sine_count:
                print("LONG condition 15: positive_sine_count > negative_sine_count")
                long_conditions_met += 1
            elif positive_sine_count == negative_sine_count and overall_majority == "LONG":
                print("LONG condition 15: positive_sine_count = negative_sine_count (assigned based on overall majority)")
                long_conditions_met += 1
