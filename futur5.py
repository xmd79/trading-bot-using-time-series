...
def calculate_fft_forecast_price(closes, n_components=5):
    ...
    filtered_signal = scipy.fft.irfft(filtered_fft, n=len(closes_np))
...
def main():
    ...
    while True:
        try:
            ...
            # 1m-specific analysis for momentum and FFT forecast
            fft_forecast_price = None
            current_close = None
            if "1m" in candle_map and candle_map["1m"]:
                ...
                # FFT forecast price
                if len(valid_closes) >= 5:  # Ensure enough data for FFT
                    fft_forecast_price = calculate_fft_forecast_price(closes, n_components=5)
                    print(f"FFT Forecast Price (1m): {fft_forecast_price:.25f}")
                    conditions_long["fft_forecast_above_close"] = fft_forecast_price > current_close
                    conditions_short["fft_forecast_below_close"] = fft_forecast_price < current_close
                    print(f"FFT Forecast Condition: LONG={conditions_long['fft_forecast_above_close']}, SHORT={conditions_short['fft_forecast_below_close']}")
                else:
                    print("Insufficient data for FFT forecast on 1m timeframe.")
            ...
            for long_cond, short_cond in condition_pairs:
                ...
                if long_val and short_val:
                    ...
                    tf = long_cond.split('_')[-1] if 'volume' in long_cond else None
                    if "momentum" in long_cond or "fft" in long_cond:
            ...
            if position and position["side"] != "NONE":
                ...
                print(f"Unrealized PNL: {position_players['unrealized_pnl']:.25f} USDC")
