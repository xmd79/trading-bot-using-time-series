...
def calculate_mtf_trend(candles, timeframe, min_threshold, max_threshold, buy_volume, sell_volume, lookback=50):
    ...
    # Volume confirmation with gradual scaling
    lookback = min(len(buy_volume), lookback)
    ...
    # Adjust cycle target to be above/below current close based on trend
    if trend == "BULLISH":
...
# FFT-Based Target Price Function with Corrected Targets
def get_target(closes, n_components, timeframe, min_threshold, max_threshold, buy_volume, sell_volume):
    ...
    # Gradual volume confirmation
    lookback = min(len(buy_volume), LOOKBACK_PERIODS[timeframe])
    buy_vol = sum(buy_volume[-lookback:]) / lookback if buy_volume else Decimal('0')
    sell_vol = sum(sell_volume[-lookback:]) / lookback if sell_volume else Decimal('0')
    volume_ratio = buy_vol / sell_vol if sell_vol > Decimal('0') else Decimal('1.0')
    
    # Volume exhaustion for reversal detection
    ...
    # Calculate targets, ensuring they align with trend direction
    # Fastest target (short-term, top 3 frequencies)
    filtered_fft = np.zeros_like(fft_result, dtype=complex)
    ...
    # Average target (medium-term, top 2 frequencies)
    filtered_fft = np.zeros_like(fft_result, dtype=complex)
    filtered_fft[idx[:2]] = fft_result[idx[:2]]
    filtered_signal = fft.irfft(filtered_fft, n=len(closes))
    ...
    # Reversal target (long-term, single dominant frequency)
    ...
    # Ensure targets are above current close for bullish, below for bearish
...
def calculate_support_resistance(candles, timeframe):
    ...
    # Use only lowest low and highest high
...
def detect_recent_reversal(candles, timeframe, min_threshold, max_threshold, buy_volume, sell_volume):
    ...
    for i, candle in enumerate(recent_candles):
        ...
        if min_diff <= tolerance and min_diff < closest_min_diff and buy_vol >= VOLUME_CONFIRMATION_RATIO * sell_vol:
        ...
        if max_diff <= tolerance and max_diff < closest_max_diff and sell_vol >= VOLUME_CONFIRMATION_RATIO * buy_vol:
    ...
    if not reversals:
        logging.info(f"{timeframe} - No valid reversals detected. Defaulting to PEAK.")
        print(f"{timeframe} - No valid reversals detected. Defaulting to PEAK.")
        return "PEAK"
    
    reversals.sort(key=lambda x: x["time"], reverse=True)
    reversal_type = reversals[0]["type"]
    logging.info(f"{timeframe} - Confirmed reversal: {reversal_type} at price {reversals[0]['price']:.2f}, time {datetime.datetime.fromtimestamp(reversals[0]['time'])}")
    print(f"{timeframe} - Confirmed reversal: {reversal_type} at price {reversals[0]['price']:.2f}, time {datetime.datetime.fromtimestamp(reversals[0]['time'])}")
...
def main():
    try:
        ...
        while True:
            ...
            buy_volume = {tf: [] for tf in TIMEFRAMES}
            sell_volume = {tf: [] for tf in TIMEFRAMES}
            for tf in TIMEFRAMES:
                for candle in candle_map.get(tf, []):
                    total_volume = Decimal(str(candle["volume"]))
                    close_price = Decimal(str(candle["close"]))
                    open_price = Decimal(str(candle["open"]))
                    high_price = Decimal(str(candle["high"]))
                    low_price = Decimal(str(candle["low"]))
                    if high_price == low_price or total_volume <= Decimal('0'):
                        buy_vol = sell_vol = total_volume / Decimal('2')
                    else:
                    ...
                    buy_volume[tf].append(max(Decimal('0'), buy_vol))
                    sell_volume[tf].append(max(Decimal('0'), sell_vol))
            ...
            for timeframe in TIMEFRAMES:
                ...
                buy_vol, sell_vol, volume_mood = calculate_buy_sell_volume(candles_tf, timeframe)
                ...
                if len(closes) >= 2:
                    fft_data = get_target(
                        ...
                        buy_volume=buy_volume[timeframe], sell_volume=sell_volume[timeframe]
                    )
                    fft_results[timeframe] = {
