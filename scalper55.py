...
def generate_report(timeframe, candles, investment, forecast_minutes=12, ml_model=None):
    ...
    market_mood, lower_bound, upper_bound, avg_mtf = harmonic_wave_analysis(candles)
    print(f"Harmonic Analysis - Mood: {market_mood}, Lower Bound: {lower_bound:.2f}, Upper Bound: {upper_bound:.2f}, Average MTF: {avg_mtf:.2f}")
        ...
        if len(candles) > 0:
            ...
            if timeframe == "30m":
                ...
                if true_count >= 4:
                    ...
                    if timeframe == "5m":
                        true_count_5m = 0  # Initialize for 5m checks
                        if dist_min_close < dist_max_close:
                            true_count_5m += 1
                        if closest_threshold == min_threshold:
                            true_count_5m += 1
                        ...
                        if true_count_5m >= 5:
                            ...
                            with open(signal_file, "a") as f:
                                f.write(f"{timestamp} - SIGNAL: MTF DIP found on both 30m & 5m at {current_close:.2f}\n")
                            print(f"MTF DIP found on both 30m & 5m at {current_close:.2f} - Recorded to {signal_file}")
