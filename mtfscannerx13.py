...
def calculate_distance_percentages(current_price, min_threshold, max_threshold):
    """Calculate the distance percentages from the current price to the min and max thresholds."""
...
def analyze_asset(symbol):
    ...
    for timeframe in timeframes:
        ...
        # Determine trends based on bullish volume
        market_mood = "N/A"
        distance_to_min, distance_to_max = None, None
        ...
        if bullish_volume > bearish_volume:
            ...
            distance_to_min, distance_to_max = calculate_distance_percentages(current_price, min_threshold, max_threshold)

            # Market mood determination
            market_mood = "Bullish" if (bullish_volume > bearish_volume) else "Bearish"
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"{'Symbol':<15}{'Timeframe':<10}{'Min Threshold':<45}{'Max Threshold':<45}{'Projected Price':<45}{'Upper Channel':<45}{'Lower Channel':<45}{'Market Mood':<15}{'Distance to Min (%)':<20}{'Distance to Max (%)':<20}")
    print("-" * 60)

    for symbol, results in overall_analysis.items():
        for timeframe, data in results.items():
            print(f"{symbol:<15}{timeframe:<10}{data['Min Threshold']:<45}{data['Max Threshold']:<45}{data['Projected Price']:<45}{data['Upper Channel']:<45}{data['Lower Channel']:<45}{data['Market Mood']:<15}{data['Distance to Min (%)']:<20}{data['Distance to Max (%)']:<20}")
