...
def generate_em_index_with_sma():
        ...
        for length in sma_lengths:
            ...
            for i, em_dict in enumerate(em_index):
                if em_dict['min'] <= phi <= em_dict['max']:
                    ...
                    em_index[i]['phi_values'].append(phi)
                    break  # exit loop after adding phi to first matching EM field

    # Calculate phi ratios
    for em_dict in em_index:
        phi_values = em_dict['phi_values']
        if phi_values:
            phi_ratios = [phi_values[i+1]/phi_values[i] for i in range(len(phi_values)-1)]
            em_dict['phi_ratios'] = phi_ratios
        else:
            em_dict['phi_ratios'] = []
    ...
    # Get frequency ranges with non-zero phi values
    phi_ranges = [em_dict['freq_range'] for em_dict in em_index if em_dict['phi_values']]
    
    # Print frequency ranges
    print("Frequency ranges with non-zero phi values:")
    for freq_range in phi_ranges:
        print(freq_range)
...
em_index = [    
    ...
    {'name': 'W boson', 'phi_ratios': [0.15, 0.20, 0.35]}, 
    {'name': 'Z boson', 'phi_ratios': [0.4, 0.5]},   
    {'name': 'Higgs boson', 'phi_ratios': [0.8]},
    {'name': 'electron', 'phi_ratios': [0.9, 1.2, 1.3]},    
    {'name': 'proton', 'phi_ratios': [0.6, 1.1]},     
    {'name': 'neutron', 'phi_ratios': [0.3, 0.7]}, 
    {'name': 'positron', 'phi_ratios': [0.5, 0.8]},    
    {'name': 'muon', 'phi_ratios': [0.2, 0.4]}   
]
...
def generate_solfeggio_frequencies(em_index):
    ...
    for em_dict in em_index:
        phi_avg = em_dict.get('phi_avg', 0)  # Default value of 0 if 'phi_avg' key is missing
        if phi_avg < 1:
