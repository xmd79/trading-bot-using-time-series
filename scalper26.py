...
# Remove NaN and zero entries from arrays
...
def calculate_profit_loss(current_price, target_price, investment):
    """
    Calculate potential profit or loss from a position.
    :param current_price: The current market price.
    :param target_price: The target price for selling.
    :param investment: The amount invested (not the actual shares).
    :return: profit or loss amount and percentage.
    """
    number_of_shares = investment / current_price
    profit_loss_amount = (target_price - current_price) * number_of_shares
    profit_loss_percentage = (profit_loss_amount / investment) * 100
    return profit_loss_amount, profit_loss_percentage

# Predict price reversals using Linear Regression
...
# Calculate a 45-degree angle price based on linear regression
...
# Enforce a price area around the calculated angle price
...
# Scaling momentum to sine wave values
...
# Calculate regression channels for price analysis
...
# ZigZag indicator for price forecasting
...
# Calculate ZigZag forecast values
...
# Scale close prices to sine wave values
...
# Calculate thresholds for price analysis
...
# Calculate EMAs for various lengths
...
# Evaluate EMA patterns in the current candle data
...
# Merging FFT with price forecast
def merged_fft_forecast(candles, n_components=5):
    ...
    # Reconstruct the price signal from filtered FFT
...
def generate_report(timeframe, candles, investment, forecast_minutes=12):
    ...
    if predictions is not None:
        ...
        # Corrected Profit & Loss calculation
        pl_amount, pl_percentage = calculate_profit_loss(current_close, minor_reversal_target, investment)
        print(f"Potential Profit/Loss for Minor Target: Amount = {pl_amount:.2f}, Percentage = {pl_percentage:.2f}%")

        pl_amount, pl_percentage = calculate_profit_loss(current_close, major_reversal_target, investment)
...
# Function to calculate volatility using FFT and report current amplitude and frequency
def calculate_volatility_with_fft(candles):
    ...
    # Calculate volatility as the standard deviation of the reconstructed signal
...
def calculate_garch_volatility(candles):
    ...
    # Rescale the closing prices to avoid warnings
    scaled_closes = closes * 0.01
    
    # Fit the GARCH model
    model = arch_model(scaled_closes, vol='Garch', p=1, q=1)  # GARCH(1, 1) model
    garch_fit = model.fit(disp="off")

    # Get the predicted volatility and rescale it back
    predicted_volatility = np.sqrt(garch_fit.conditional_volatility[-1]) / 0.01  # Standard deviation
