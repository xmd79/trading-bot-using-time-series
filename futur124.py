...
LEVERAGE = 25
STOP_LOSS_PERCENTAGE = Decimal('0.25')  # 25% stop-loss
...
LONG_SIGNAL_THRESHOLD = 12  # Require 12 of 15 LONG conditions to be true
SHORT_SIGNAL_THRESHOLD = 12  # Require 12 of 15 SHORT conditions to be true
    ...
    try:
        ...
        while True:
            try:
                ...
                # Count the number of true conditions for long and short
                long_condition_count = sum(1 for condition in conditions_long.values() if condition)
                short_condition_count = sum(1 for condition in conditions_short.values() if condition)
                ...
                print("\nNew Condition Pairs Status:")
                for tf in TIMEFRAMES:
                    print(f"dip_reversal_{tf}: {conditions_long.get(f'dip_reversal_{tf}', False)}, "
                          f"top_reversal_{tf}: {conditions_short.get(f'top_reversal_{tf}', False)}")
                    print(f"below_middle_{tf}: {conditions_long.get(f'below_middle_{tf}', False)}, "
                          f"above_middle_{tf}: {conditions_short.get(f'above_middle_{tf}', False)}")
                
                print("\nLRC Forecast Condition Pairs Status:")
                for tf in TIMEFRAMES:
                    print(f"lrc_forecast_above_current_{tf}: {conditions_long.get(f'lrc_forecast_above_current_{tf}', False)}, "
                          f"lrc_forecast_below_current_{tf}: {conditions_short.get(f'lrc_forecast_below_current_{tf}', False)}")
                
                print("\nVolume Condition Pairs Status:")
                # Only print volume conditions for 1m
                for tf in ["1m"]:
                    print(f"volume_bullish_{tf}: {conditions_long.get(f'volume_bullish_{tf}', False)}, "
                          f"volume_bearish_{tf}: {conditions_short.get(f'volume_bearish_{tf}', False)}")
                ...
                # NEW SIGNAL LOGIC - Trigger when volume condition is true AND 12 of 15 conditions are true
                volume_bullish_1m = conditions_long.get("volume_bullish_1m", False)
                volume_bearish_1m = conditions_short.get("volume_bearish_1m", False)
                
                long_signal = volume_bullish_1m and (long_condition_count >= LONG_SIGNAL_THRESHOLD)
                short_signal = volume_bearish_1m and (short_condition_count >= SHORT_SIGNAL_THRESHOLD)
                ...
                print(f"Volume Bullish (1m): {volume_bullish_1m}")
                print(f"Volume Bearish (1m): {volume_bearish_1m}")
                print(f"LONG Conditions Met: {long_condition_count}/{LONG_SIGNAL_THRESHOLD} - {'PASS' if long_condition_count >= LONG_SIGNAL_THRESHOLD else 'FAIL'}")
                print(f"SHORT Conditions Met: {short_condition_count}/{SHORT_SIGNAL_THRESHOLD} - {'PASS' if short_condition_count >= SHORT_SIGNAL_THRESHOLD else 'FAIL'}")
                print(f"Final Signal: {signal}")
                print("==============================\n")
                
                logging.info(f"Final Signal: {signal} (Volume Bullish: {volume_bullish_1m}, Volume Bearish: {volume_bearish_1m}, LONG: {long_condition_count}/{LONG_SIGNAL_THRESHOLD}, SHORT: {short_condition_count}/{SHORT_SIGNAL_THRESHOLD})")
                
                # Only print signal alert if signal is not NO_SIGNAL
                if signal != "NO_SIGNAL":
                    volume_condition = "Volume Bullish" if signal == "LONG" else "Volume Bearish"
                    print(f"\n🚨 STRONG SIGNAL ALERT: {signal} ({volume_condition} and {LONG_SIGNAL_THRESHOLD if signal == 'LONG' else SHORT_SIGNAL_THRESHOLD}+ conditions met) 🚨")
