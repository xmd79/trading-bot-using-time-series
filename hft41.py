...
def main():
    ...
    prev_close_position = 0  # initialize previous close position
    midline = 0  # initialize midline

    while True:
        try:
            # Define sine wave variables
            wave_len = random.randint(10, 60) * 60
            ...
            em_field = random.uniform(0.05, 0.15)
            em_amp = random.uniform(5, 15)
            em_phase = random.uniform(0, 2 * math.pi)
            ...
            # Calculate the close position as a percentage of the sine wave
            ...
            # Calculate difference between close position and frequency ranges positions
            diff_alpha = abs(close_position - alpha_signal[-1]) / 100
            diff_beta = abs(close_position - beta_signal[-1]) / 100
            diff_gamma = abs(close_position - gamma_signal[-1]) / 100
            diff_theta = abs(close_position - theta_signal[-1]) / 100
            diff_delta = abs(close_position - delta_signal[-1]) / 100

            # Calculate frequency ranges based on updated values
            freq_alpha_range = (close_position / 100 * alpha_freq * 0.8, close_position / 100 * alpha_freq * 1.2)
            freq_beta_range = (close_position / 100 * beta_freq * 0.8, close_position / 100 * beta_freq * 1.2)
            freq_gamma_range = (close_position / 100 * gamma_freq * 0.8, close_position / 100 * gamma_freq * 1.2)
            freq_theta_range = (close_position / 100 * theta_freq * 0.8, close_position / 100 * theta_freq * 1.2)
            freq_delta_range = (close_position / 100 * delta_freq * 0.8, close_position / 100 * delta_freq * 1.2)

            # Calculate percentage difference for each frequency range
            alpha_perc = round(diff_alpha / (freq_alpha_range[1] - freq_alpha_range[0]) * 100, 2)
            beta_perc = round(diff_beta / (freq_beta_range[1] - freq_beta_range[0]) * 100, 2)
            gamma_perc = round(diff_gamma / (freq_gamma_range[1] - freq_gamma_range[0]) * 100, 2)
            ...
            # Define the degrees of the unit circle
            degrees = np.arange(0, 360, 1)

            # Print the calculated values
            print(f"Close position: {close_position:.2f}%")
            print(f"Momentum: {momentum:.2f}")
            print(f"EMA Slow: {ema_slow:.2f}")
            print(f"EMA Fast: {ema_fast:.2f}")
            print(f"Alpha frequency range: {freq_alpha_range}")
            print(f"Beta frequency range: {freq_beta_range}")
            print(f"Gamma frequency range: {freq_gamma_range}")
            print(f"Theta frequency range: {freq_theta_range}")
            print(f"Delta frequency range: {freq_delta_range}")
            print(f"Alpha percentage: {alpha_perc:.2f}%")
            print(f"Beta percentage: {beta_perc:.2f}%")
            print(f"Gamma percentage: {gamma_perc:.2f}%")
            print(f"Theta percentage: {theta_perc:.2f}%")
            print(f"Delta percentage: {delta_perc:.2f}%")

            # Determine which quadrant the close position falls in
            if close_position >= quadrant1[0] and close_position < quadrant1[1]:
                print("Close position is in quadrant 1")
            elif close_position >= quadrant2[0] and close_position < quadrant2[1]:
                print("Close position is in quadrant 2")
            elif close_position >= quadrant3[0] and close_position < quadrant3[1]:
                print("Close position is in quadrant 3")
            else:
