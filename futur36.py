...
ENERGY_SCALING_FACTOR = Decimal('0.1')  # Scaling factor for FFT energy adjustment
EMA_FAST_PERIOD = 9  # Fast EMA period for trend analysis
EMA_SLOW_PERIOD = 21  # Slow EMA period for trend analysis
...
# MTF Trend Analysis Function
def calculate_mtf_trend(candles, timeframe):
    if not candles or len(candles) < EMA_SLOW_PERIOD:
    ...
    if len(closes) < EMA_SLOW_PERIOD:
        logging.warning(f"Insufficient valid close prices ({len(closes)}) for MTF trend analysis in {timeframe}.")
        print(f"Insufficient valid close prices ({len(closes)}) for MTF trend analysis in {timeframe}.")
        return "NEUTRAL"
    
    try:
        ema_fast = talib.EMA(closes, timeperiod=EMA_FAST_PERIOD)
        ema_slow = talib.EMA(closes, timeperiod=EMA_SLOW_PERIOD)
        if len(ema_fast) < 1 or len(ema_slow) < 1 or np.isnan(ema_fast[-1]) or np.isnan(ema_slow[-1]):
            logging.warning(f"Invalid EMA data for {timeframe}.")
        ...
        logging.error(f"Error in MTF trend analysis for {timeframe}: {e}")
        print(f"Error in MTF trend analysis for {timeframe}: {e}")
        return "NEUTRAL"

# FFT-Based Target Price Function
def get_target(closes, n_components, reversal_type, min_threshold, middle_threshold, max_threshold, timeframe, trend):
    ...
    is_dip = reversal_type == "DIP" and is_near_min
    is_peak = reversal_type == "TOP" and is_near_max
    phase_adjusted = -np.pi if is_dip and trend == "UP" else np.pi if is_peak and trend == "DOWN" else phase
    
    energy = Decimal(str(np.sum(amplitudes[idx]))) * ENERGY_SCALING_FACTOR
    filtered_fft = np.zeros_like(fft)
    if positive_freqs and (is_dip or trend == "UP" or not negative_freqs):
        for i, _ in positive_freqs[:n_components]:
            filtered_fft[i] = fft[i] * np.exp(1j * phase_adjusted)
        is_bullish_target = True
        is_bearish_target = False
    elif negative_freqs and (is_peak or trend == "DOWN" or not positive_freqs):
        for i, _ in negative_freqs[:n_components]:
            filtered_fft[i] = fft[i] * np.exp(1j * phase_adjusted)
        is_bullish_target = False
        is_bearish_target = True
    else:
        filtered_fft[idx[:n_components]] = fft[idx[:n_components]] * np.exp(1j * phase_adjusted)
        is_bullish_target = fft[idx_max].real > 0
        is_bearish_target = not is_bullish_target
    
    filtered_signal = fftpack.irfft(filtered_fft)
    projected_price = Decimal(str(filtered_signal[-1])) * (Decimal('1') + energy)
    
    if is_bullish_target:
        if is_dip and max_threshold > middle_threshold > current_close:
            ...
            target_price = min(target_price, max_threshold * (Decimal('1') - PROXIMITY_THRESHOLD))
        # Ensure target price is above current close for bullish
        if target_price <= current_close:
            target_price = current_close * Decimal('1.005')
    else:
        if is_peak and min_threshold < middle_threshold < current_close:
            ...
            target_price = max(target_price, min_threshold * (Decimal('1') + PROXIMITY_THRESHOLD))
        # Ensure target price is below current close for bearish
        if target_price >= current_close:
            target_price = current_close * Decimal('0.995')
    ...
    logging.info(f"FFT Analysis - Timeframe: {timeframe}, Market Mood: {market_mood}, Current Close: {current_close:.25f}, Fastest Target: {fastest_target:.25f}, Stop Loss: {stop_loss:.25f}, Predominant Freq: {predominant_freq:.6f}, Sign: {predominant_sign}, Phase: {phase_adjusted:.2f}, Trend: {trend}")
    print(f"FFT Analysis - Timeframe: {timeframe}, Market Mood: {market_mood}, Current Close: {current_close:.25f}, Fastest Target: {fastest_target:.25f}, Stop Loss: {stop_loss:.25f}, Predominant Freq: {predominant_freq:.6f}, Sign: {predominant_sign}, Phase: {phase_adjusted:.2f}, Trend: {trend}")
...
def get_position():
    try:
        ...
        if not positions or len(positions) == 0 or not isinstance(positions, list):
            logging.warning(f"No position data returned for {TRADE_SYMBOL}. Returning default position.")
            print(f"No position data returned for {TRADE_SYMBOL}. Returning default position.")
            return {
        ...
        logging.warning(f"No matching position found for {TRADE_SYMBOL}. Returning default position.")
        print(f"No matching position found for {TRADE_SYMBOL}. Returning default position.")
        return {
            "quantity": Decimal('0.0'),
            "entry_price": Decimal('0.0'),
            "side": "NONE",
        ...
        logging.error(f"Unexpected error fetching position info: {e}")
        print(f"Unexpected error fetching position info: {e}")
        return {
            "quantity": Decimal('0.0'),
            "entry_price": Decimal('0.0'),
            "side": "NONE",
    ...
    try:
        while True:
            ...
            conditions_long = {
                ...
                "trend_up_1m": False, "trend_up_3m": False, "trend_up_5m": False
            }
            conditions_short = {
                ...
                "trend_down_1m": False, "trend_down_3m": False, "trend_down_5m": False
            }
            ...
            for timeframe in timeframes:
                if not candle_map.get(timeframe):
                    ...
                    trend_results[timeframe] = "NEUTRAL"
                    continue
                ...
                trend = calculate_mtf_trend(candles, timeframe)
                trend_results[timeframe] = trend
                conditions_long[f"trend_up_{timeframe}"] = trend == "UP"
                conditions_short[f"trend_down_{timeframe}"] = trend == "DOWN"
                ...
                if len(closes) >= 2:
                    try:
                        current_time, entry_price, stop_loss, fastest_target, market_mood, is_bullish_target, is_bearish_target, predominant_freq, predominant_sign = get_target(
                            closes, n_components=5, reversal_type=reversal_type, min_threshold=min_threshold, middle_threshold=middle_threshold, max_threshold=max_threshold, timeframe=timeframe, trend=trend
                        )
                        fft_results[timeframe] = {
                        ...
                        print(f"{timeframe} - FFT Target: {fastest_target:.25f}")
                        ...
                        logging.error(f"Error in FFT analysis for {timeframe}: {e}")
                        print(f"Error in FFT analysis for {timeframe}: {e}")
                        fft_results[timeframe] = {
                            "time": current_local_time_str,
                            "current_price": Decimal('0'),
                            "fastest_target": Decimal('0'),
                ...
                if len(closes) >= 14:
                    ...
                    if len(momentum) > 0 and not np.isnan(momentum[-1]):
                        ...
                        logging.warning(f"{timeframe} Momentum: Invalid momentum data")
                        print(f"{timeframe} Momentum: Invalid momentum data")
                        conditions_long[f"momentum_positive_{timeframe}"] = True
                        conditions_short[f"momentum_negative_{timeframe}"] = False
                else:
            ...
            condition_pairs = [
                ...
                ("trend_up_1m", "trend_down_1m"),
                ("trend_up_3m", "trend_down_3m"),
                ("trend_up_5m", "trend_down_5m")
            ]
            ...
            if signal in ["LONG", "SHORT"]:
                ...
                for tf in timeframes:
                    trend = trend_results.get(tf, "NEUTRAL")
                    telegram_message += f"{tf} Timeframe Trend: {trend}\n"
                telegram_message += f"\n*FFT Analysis*\n"
                for tf in timeframes:
        ...
        logging.info("Telegram bot shutdown completed.")
