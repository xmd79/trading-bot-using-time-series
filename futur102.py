...
# Octagonal symmetry constants
OCTAGON_VERTICES = 8  # Number of vertices in octagon
OCTAGON_ANGLES = np.linspace(0, 2*np.pi, OCTAGON_VERTICES, endpoint=False)  # Angles for each vertex
OCTAGON_PHASE_BINS = np.linspace(-np.pi, np.pi, OCTAGON_VERTICES+1)  # Phase bins for quantization
...
def octagonal_forecast(candles, timeframe, forecast_periods=5):
    """
    Octagonal symmetry forecast using 8-pointed star pattern on unit circle.
    ...
    Returns:
    - forecast_price: Predicted price after forecast_periods
    - cycle_direction: "Up" or "Down" based on octagonal analysis
    - phase_angle: Phase angle in degrees (0-360)
    - harmonic_strength: Strength of harmonic components
    - price_target: Price target for trading based on octagonal forecast
    """
    if len(candles) < OCTAGON_VERTICES * 2:
        logging.warning(f"Insufficient data ({len(candles)}) for octagonal forecast in {timeframe}")
        print(f"Insufficient data ({len(candles)}) for octagonal forecast in {timeframe}")
        return Decimal('0.0'), "N/A", 0.0, 0.0, Decimal('0.0')
    ...
    try:
        ...
        # Quantize phases to octagonal symmetry
        adjusted_fft = np.zeros_like(fft_result, dtype=complex)
        total_power = 0.0
        harmonic_power = 0.0
        
        for k in indices:
            # Get original complex value
            orig = fft_result[k]
            magnitude = np.abs(orig)
            phase = np.angle(orig)
            
            # Quantize phase to nearest octagonal vertex
            # Find the closest angle in OCTAGON_ANGLES
            phase_diffs = np.abs(phase - OCTAGON_ANGLES)
            closest_idx = np.argmin(phase_diffs)
            quantized_phase = OCTAGON_ANGLES[closest_idx]
            
            # Reconstruct complex value with quantized phase
            adjusted_fft[k] = magnitude * np.exp(1j * quantized_phase)
            
            # Track power
            total_power += magnitude ** 2
            if k in indices[:OCTAGON_VERTICES]:  # Top 8 harmonics
                harmonic_power += magnitude ** 2
        
        # Calculate harmonic strength
        harmonic_strength = harmonic_power / total_power if total_power > 0 else 0.0
        
        # Reconstruct time-domain signal with quantized phases
        adjusted_signal = np.real(np.fft.ifft(adjusted_fft))
        
        # Generate forecast
        forecast = np.zeros(forecast_periods)
        future_t = np.arange(n, n + forecast_periods)
        
        # Project using dominant frequencies
        for k in indices[:OCTAGON_VERTICES]:  # Use top 8 harmonics
            magnitude = np.abs(adjusted_fft[k])
            phase = np.angle(adjusted_fft[k])
            freq = freqs[k]
            
            for t_idx, t in enumerate(future_t):
                forecast[t_idx] += magnitude * np.cos(2 * np.pi * freq * t + phase)
        
        # Convert forecast back to price scale
        forecast_price = Decimal(str(forecast[-1] + mean_close))
        
        # Determine cycle direction from phase of dominant frequency
        dominant_idx = indices[np.argmax(power[indices])]
        dominant_phase = np.angle(adjusted_fft[dominant_idx])
        phase_angle = (np.degrees(dominant_phase) + 360) % 360
        
        # Determine cycle direction based on phase quadrant
        ...
        current_close = Decimal(str(closes[-1]))
        if cycle_direction == "Up":
            # For up cycle, ensure forecast is above current price
            forecast_price = max(forecast_price, current_close * Decimal('1.001'))
        else:
            # For down cycle, ensure forecast is below current price
            forecast_price = min(forecast_price, current_close * Decimal('0.999'))
        
        # Calculate price target based on cycle direction and harmonic strength
        # The price target is where we expect the price to move based on octagonal analysis
        if harmonic_strength > 0.7:  # Strong harmonic signal
            # Use a more aggressive target
            target_factor = Decimal('0.02')  # 2% target
        elif harmonic_strength > 0.4:  # Medium harmonic signal
            # Use a moderate target
            target_factor = Decimal('0.015')  # 1.5% target
        else:  # Weak harmonic signal
            # Use a conservative target
            target_factor = Decimal('0.01')  # 1% target
        
        if cycle_direction == "Up":
            price_target = current_close * (Decimal('1') + target_factor)
        else:
            price_target = current_close * (Decimal('1') - target_factor)
        
        logging.info(
            f"{timeframe} - Octagonal Forecast: {forecast_price:.25f}, "
            f"Direction: {cycle_direction}, Phase: {phase_angle:.2f}°, "
            f"Harmonic Strength: {harmonic_strength:.25f}, Price Target: {price_target:.25f}"
        )
        print(
            f"{timeframe} - Octagonal Forecast: {forecast_price:.25f}, "
            f"Direction: {cycle_direction}, Phase: {phase_angle:.2f}°, "
        ...
        return forecast_price, cycle_direction, phase_angle, harmonic_strength, price_target
    
    except Exception as e:
        logging.error(f"Error in octagonal forecast for {timeframe}: {e}")
        print(f"Error in octagonal forecast for {timeframe}: {e}")
        return Decimal('0.0'), "N/A", 0.0, 0.0, Decimal('0.0')
    ...
    for attempt in range(retries):
        try:
            ...
            for tf in TIMEFRAMES:
                ...
                octagonal_price_target = details.get('octagonal_price_target', Decimal('0'))
                ...
                octagonal_price_target = octagonal_price_target if isinstance(octagonal_price_target, Decimal) else Decimal('0')
                ...
                message += (
                    ...
                    f"Octagonal Price Target: {octagonal_price_target:.25f} USDC\n"
                    ...
                    f"Octagonal Forecast: {details.get('octagonal_forecast', Decimal('0')):.25f} USDC\n"
                    f"Octagonal Direction: {details.get('octagonal_direction', 'N/A')}\n"
                    f"Octagonal Phase: {details.get('octagonal_phase', 0.0):.2f}°\n"
                    f"Octagonal Harmonic Strength: {details.get('octagonal_harmonic_strength', 0.0):.25f}\n"
                ...
                # Add octagonal trigger information for 1m timeframe
                if tf == "1m":
                    message += (
                        f"Octagonal Trigger Active: {details.get('octagonal_trigger_active', False)}\n"
                        f"Octagonal Target Distance: {details.get('octagonal_target_distance', Decimal('0')):.25f} USDC\n"
                    )
    ...
    try:
        ...
        while True:
            ...
            for timeframe in TIMEFRAMES:
                ...
                # Octagonal Forecast
                octagonal_forecast_price, octagonal_direction, octagonal_phase, octagonal_harmonic_strength, octagonal_price_target = octagonal_forecast(
                    candles_tf, timeframe
                )
                ...
                if timeframe == "1m":
                    ...
                    print(f"Octagonal Target Distance (1m): {octagonal_target_distance:.25f} USDC")
                ...
                analysis_details[timeframe] = {
                    ...
                    "octagonal_forecast": octagonal_forecast_price,
                    "octagonal_direction": octagonal_direction,
                    "octagonal_phase": octagonal_phase,
                    "octagonal_harmonic_strength": octagonal_harmonic_strength,
                    "octagonal_price_target": octagonal_price_target,
                }
                ...
                if timeframe == "1m":
                    analysis_details[timeframe].update({
                        "octagonal_trigger_active": conditions_long["octagonal_trigger_1m"] or conditions_short["octagonal_trigger_1m"],
                        "octagonal_target_distance": octagonal_target_distance,
                    })
                
                print(f"Trend: {trend}")
                print(f"Cycle Status: {cycle_status}")
                print(f"Cycle Target: {cycle_target:.25f}")
                print(f"Octagonal Price Target: {octagonal_price_target:.25f}")
                ...
                print(f"Octagonal Forecast: {octagonal_forecast_price:.25f}")
                print(f"Octagonal Direction: {octagonal_direction}")
                print(f"Octagonal Phase: {octagonal_phase:.2f}°")
                print(f"Octagonal Harmonic Strength: {octagonal_harmonic_strength:.25f}")
            ...
            if signal in ["LONG", "SHORT"] and position["side"] == "NONE":
                signal_message = (
                    ...
                    f"1m Octagonal Price Target: {analysis_details.get('1m', {}).get('octagonal_price_target', Decimal('0')):.25f} USDC\n"
