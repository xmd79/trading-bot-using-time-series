...
# Fibonacci levels for Elliott Wave analysis
FIB_LEVELS = {
    'retracement': [Decimal('0.236'), Decimal('0.382'), Decimal('0.5'), Decimal('0.618'), Decimal('0.786')],
    'extension': [Decimal('1.618'), Decimal('2.618'), Decimal('4.236')]
}
...
def identify_elliott_wave(candles):
    """
    Identifies potential Elliott Wave patterns in the given candles.
    Returns wave structure, current wave, and confidence score.
    """
    if len(candles) < 8:  # Need at least 8 candles for a full 5+3 wave cycle
    ...
    wave_points = []
    direction = None  # 'up' for bullish, 'down' for bearish
    last_extreme = closes[0]
    last_extreme_type = 'low' if closes[1] > closes[0] else 'high'
    
    # Detect pivot points (simplified high/low detection)
    for i in range(1, len(closes)-1):
        if last_extreme_type == 'low' and highs[i] > highs[i-1] and highs[i] > highs[i+1]:
            wave_points.append(('high', highs[i], i))
            last_extreme = highs[i]
            last_extreme_type = 'high'
        elif last_extreme_type == 'high' and lows[i] < lows[i-1] and lows[i] < lows[i+1]:
            wave_points.append(('low', lows[i], i))
            last_extreme = lows[i]
            last_extreme_type = 'low'
    
    if len(wave_points) < 5:  # Need at least 5 points to form a wave
        return None, None, Decimal('0.0')
    
    # Determine trend direction
    if wave_points[0][1] < wave_points[1][1]:
        direction = 'up'
    else:
        direction = 'down'
    
    # Attempt to map to Elliott Wave structure
    wave_structure = []
    confidence = Decimal('1.0')
    current_wave = None
    
    for i in range(len(wave_points)):
        point_type, price, idx = wave_points[i]
        if direction == 'up':
            if len(wave_structure) == 0 and point_type == 'low':  # Wave 1 start
                wave_structure.append(('Wave 1 Start', price, idx))
            elif len(wave_structure) == 1 and point_type == 'high':  # Wave 1 end
                wave_structure.append(('Wave 1 End', price, idx))
            elif len(wave_structure) == 2 and point_type == 'low':  # Wave 2 end
                wave_structure.append(('Wave 2 End', price, idx))
                # Rule: Wave 2 cannot retrace beyond Wave 1 start
                if price < wave_structure[0][1]:
                    return None, None, Decimal('0.0')
                # Check Fibonacci retracement
                retrace = (wave_structure[1][1] - price) / (wave_structure[1][1] - wave_structure[0][1])
                if not any(abs(retrace - fib) < Decimal('0.05') for fib in FIB_LEVELS['retracement']):
                    confidence *= Decimal('0.8')
            elif len(wave_structure) == 3 and point_type == 'high':  # Wave 3 end
                wave_structure.append(('Wave 3 End', price, idx))
                # Rule: Wave 3 cannot be shortest
                w1_len = wave_structure[1][1] - wave_structure[0][1]
                w3_len = price - wave_structure[2][1]
                if w3_len < w1_len:
                    confidence *= Decimal('0.7')
                # Check Fibonacci extension
                w3_ext = w3_len / w1_len
                if not any(abs(w3_ext - fib) < Decimal('0.1') for fib in FIB_LEVELS['extension']):
                    confidence *= Decimal('0.9')
            elif len(wave_structure) == 4 and point_type == 'low':  # Wave 4 end
                wave_structure.append(('Wave 4 End', price, idx))
                # Rule: Wave 4 cannot overlap Wave 1
                if price < wave_structure[1][1]:
                    confidence *= Decimal('0.6')
                # Check Fibonacci retracement
                retrace = (wave_structure[3][1] - price) / (wave_structure[3][1] - wave_structure[2][1])
                if not any(abs(retrace - fib) < Decimal('0.05') for fib in FIB_LEVELS['retracement']):
                    confidence *= Decimal('0.8')
            elif len(wave_structure) == 5 and point_type == 'high':  # Wave 5 end
                wave_structure.append(('Wave 5 End', price, idx))
                # Check volume (Wave 5 often has lower volume)
                if volumes[idx] > volumes[wave_structure[3][2]]:
                    confidence *= Decimal('0.9')
            elif len(wave_structure) == 6 and point_type == 'low':  # Wave A end
                wave_structure.append(('Wave A End', price, idx))
            elif len(wave_structure) == 7 and point_type == 'high':  # Wave B end
                wave_structure.append(('Wave B End', price, idx))
            elif len(wave_structure) == 8 and point_type == 'low':  # Wave C end
                wave_structure.append(('Wave C End', price, idx))
                # Check Fibonacci retracement for Wave C
                wave_a_len = wave_structure[6][1] - wave_structure[5][1]
                wave_c_len = wave_structure[7][1] - price
                if wave_c_len / wave_a_len < Decimal('0.618') or wave_c_len / wave_a_len > Decimal('1.618'):
                    confidence *= Decimal('0.8')
        else:  # Bearish direction
            if len(wave_structure) == 0 and point_type == 'high':  # Wave 1 start
                wave_structure.append(('Wave 1 Start', price, idx))
            elif len(wave_structure) == 1 and point_type == 'low':  # Wave 1 end
                wave_structure.append(('Wave 1 End', price, idx))
            elif len(wave_structure) == 2 and point_type == 'high':  # Wave 2 end
                wave_structure.append(('Wave 2 End', price, idx))
                if price > wave_structure[0][1]:
                    return None, None, Decimal('0.0')
                retrace = (price - wave_structure[1][1]) / (wave_structure[0][1] - wave_structure[1][1])
                if not any(abs(retrace - fib) < Decimal('0.05') for fib in FIB_LEVELS['retracement']):
                    confidence *= Decimal('0.8')
            elif len(wave_structure) == 3 and point_type == 'low':  # Wave 3 end
                wave_structure.append(('Wave 3 End', price, idx))
                w1_len = wave_structure[0][1] - wave_structure[1][1]
                w3_len = wave_structure[2][1] - price
                if w3_len < w1_len:
                    confidence *= Decimal('0.7')
                w3_ext = w3_len / w1_len
                if not any(abs(w3_ext - fib) < Decimal('0.1') for fib in FIB_LEVELS['extension']):
                    confidence *= Decimal('0.9')
            elif len(wave_structure) == 4 and point_type == 'high':  # Wave 4 end
                wave_structure.append(('Wave 4 End', price, idx))
                if price > wave_structure[1][1]:
                    confidence *= Decimal('0.6')
                retrace = (price - wave_structure[3][1]) / (wave_structure[2][1] - wave_structure[3][1])
                if not any(abs(retrace - fib) < Decimal('0.05') for fib in FIB_LEVELS['retracement']):
                    confidence *= Decimal('0.8')
            elif len(wave_structure) == 5 and point_type == 'low':  # Wave 5 end
                wave_structure.append(('Wave 5 End', price, idx))
                if volumes[idx] > volumes[wave_structure[3][2]]:
                    confidence *= Decimal('0.9')
            elif len(wave_structure) == 6 and point_type == 'high':  # Wave A end
                wave_structure.append(('Wave A End', price, idx))
            elif len(wave_structure) == 7 and point_type == 'low':  # Wave B end
                wave_structure.append(('Wave B End', price, idx))
            elif len(wave_structure) == 8 and point_type == 'high':  # Wave C end
                wave_structure.append(('Wave C End', price, idx))
                wave_a_len = wave_structure[5][1] - wave_structure[6][1]
                wave_c_len = price - wave_structure[7][1]
                if wave_c_len / wave_a_len < Decimal('0.618') or wave_c_len / wave_a_len > Decimal('1.618'):
                    confidence *= Decimal('0.8')
    
    # Determine current wave
    if len(wave_structure) >= 2:
        last_wave = wave_structure[-1][0]
        if 'Wave 1 End' in last_wave:
            current_wave = 'Wave 2'
        elif 'Wave 2 End' in last_wave:
            current_wave = 'Wave 3'
        elif 'Wave 3 End' in last_wave:
            current_wave = 'Wave 4'
        elif 'Wave 4 End' in last_wave:
            current_wave = 'Wave 5'
        elif 'Wave 5 End' in last_wave:
            current_wave = 'Wave A'
        elif 'Wave A End' in last_wave:
            current_wave = 'Wave B'
        elif 'Wave B End' in last_wave:
            current_wave = 'Wave C'
        elif 'Wave C End' in last_wave:
            current_wave = 'New Impulse'
        else:
            current_wave = 'Wave 1'
    else:
        current_wave = 'Unknown'
    
    return wave_structure, current_wave, confidence

def calculate_elliott_fibonacci_targets(wave_structure, current_price, direction):
    """
    Calculates Fibonacci-based price targets for the current wave.
    Returns retracement and extension levels.
    """
    if not wave_structure or len(wave_structure) < 2:
        return {}, {}
    ...
    if direction == 'up':
        ...
        if len(wave_structure) >= 4:  # At least Wave 1 and Wave 2 completed
            wave_1_start = wave_structure[0][1]
            wave_1_end = wave_structure[1][1]
            wave_2_end = wave_structure[2][1]
            wave_1_length = wave_1_end - wave_1_start
            # Wave 3 extension targets
            for fib in FIB_LEVELS['extension']:
                extension_levels[fib] = wave_2_end + wave_1_length * fib
            # Wave 4 retracement targets
            if len(wave_structure) >= 4:
                wave_3_end = wave_structure[3][1]
                wave_3_length = wave_3_end - wave_2_end
                for fib in FIB_LEVELS['retracement']:
                    retracement_levels[fib] = wave_3_end - wave_3_length * fib
        # Corrective wave targets
        if len(wave_structure) >= 6:  # Wave A completed
            wave_5_end = wave_structure[5][1]
            wave_a_end = wave_structure[6][1]
            wave_a_length = wave_5_end - wave_a_end
            for fib in FIB_LEVELS['retracement']:
                retracement_levels[fib] = wave_a_end + wave_a_length * fib  # Wave B
            if len(wave_structure) >= 8:  # Wave C
                for fib in FIB_LEVELS['extension']:
                    extension_levels[fib] = wave_a_end - wave_a_length * fib
    else:
        # Bearish impulse wave targets
        if len(wave_structure) >= 4:
            wave_1_start = wave_structure[0][1]
            wave_1_end = wave_structure[1][1]
            wave_2_end = wave_structure[2][1]
            wave_1_length = wave_1_start - wave_1_end
            for fib in FIB_LEVELS['extension']:
                extension_levels[fib] = wave_2_end - wave_1_length * fib
            if len(wave_structure) >= 4:
                wave_3_end = wave_structure[3][1]
                wave_3_length = wave_2_end - wave_3_end
                for fib in FIB_LEVELS['retracement']:
                    retracement_levels[fib] = wave_3_end + wave_3_length * fib
        # Corrective wave targets
        if len(wave_structure) >= 6:
            wave_5_end = wave_structure[5][1]
            wave_a_end = wave_structure[6][1]
            wave_a_length = wave_a_end - wave_5_end
            for fib in FIB_LEVELS['retracement']:
                retracement_levels[fib] = wave_a_end - wave_a_length * fib  # Wave B
            if len(wave_structure) >= 8:
                for fib in FIB_LEVELS['extension']:
                    extension_levels[fib] = wave_a_end + wave_a_length * fib
    
    return retracement_levels, extension_levels

def generate_wave_signals(wave_structure, current_wave, current_price, retracement_levels, extension_levels):
    """
    Generates buy/sell signals based on Elliott Wave position.
    """
    signals = {'buy': False, 'sell': False, 'target': None, 'stop_loss': None}
    current_price_dec = Decimal(str(current_price))
    
    if current_wave in ['Wave 3', 'Wave 5']:
        signals['buy'] = True
        # Set target to nearest Fibonacci extension
        if extension_levels:
            signals['target'] = min(extension_levels.values(), key=lambda x: abs(x - current_price_dec))
        # Set stop-loss below Wave 2 or Wave 4
        if wave_structure and len(wave_structure) >= 3:
            signals['stop_loss'] = wave_structure[2][1] if current_wave == 'Wave 3' else wave_structure[4][1]
    elif current_wave == 'Wave C':
        signals['sell'] = True
        if extension_levels:
            signals['target'] = min(extension_levels.values(), key=lambda x: abs(x - current_price_dec))
        if wave_structure and len(wave_structure) >= 7:
            signals['stop_loss'] = wave_structure[7][1]  # Wave B high
    elif current_wave == 'Wave 2' or current_wave == 'Wave 4':
        # Buy near Fibonacci retracement levels
        for fib, level in retracement_levels.items():
            if abs(current_price_dec - level) / current_price_dec < Decimal('0.01'):  # Within 1% of level
                signals['buy'] = True
                signals['target'] = wave_structure[-1][1] + (wave_structure[-1][1] - wave_structure[-2][1]) * Decimal('1.618')
                signals['stop_loss'] = level - (wave_structure[-1][1] - level) * Decimal('0.1')
...
def forecast_price_per_time_pythagorean(timeframe, candles, min_threshold, max_threshold, current_price, time_window_minutes, last_reversal, last_reversal_type, wave_target=None):
    current_price_dec = Decimal(str(current_price))
    ...
    # Adjust forecast based on Elliott Wave target
    if wave_target is not None:
        wave_target_dec = Decimal(str(wave_target))
        forecast_price = (forecast_price + wave_target_dec) / Decimal('2')
        print(f"Adjusted Pythagorean forecast with Elliott Wave target: {forecast_price:.25f}")
...
while True:
    ...
    conditions_status = {
        ...
        "elliott_wave_buy_signal_1m": False,
        "elliott_wave_buy_signal_3m": False,
        "elliott_wave_buy_signal_5m": False
    }
    ...
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map and candle_map[timeframe]:
            ...
            wave_structure, current_wave, wave_confidence = identify_elliott_wave(candle_map[timeframe])
            wave_info[timeframe]['structure'] = wave_structure
            wave_info[timeframe]['current_wave'] = current_wave
            wave_info[timeframe]['confidence'] = wave_confidence
            print(f"Elliott Wave Analysis for {timeframe}:")
            print(f"Current Wave: {current_wave}")
            print(f"Wave Confidence: {wave_confidence:.2f}")
            if wave_structure:
                print("Wave Structure:")
                for wave_point in wave_structure:
                    print(f"{wave_point[0]} at Price: {wave_point[1]:.25f} (Index: {wave_point[2]})")
            direction = 'up' if wave_structure and wave_structure[0][1] < wave_structure[1][1] else 'down'
            retracement_levels, extension_levels = calculate_elliott_fibonacci_targets(wave_structure, current_price, direction)
            print("Elliott Wave Fibonacci Retracement Levels:")
            for fib, level in retracement_levels.items():
                print(f"Fib {fib:.3f}: {level:.25f}")
            print("Elliott Wave Fibonacci Extension Levels:")
            for fib, level in extension_levels.items():
                print(f"Fib {fib:.3f}: {level:.25f}")
            wave_signals = generate_wave_signals(wave_structure, current_wave, current_price, retracement_levels, extension_levels)
            print("Elliott Wave Signals:")
            print(f"Buy Signal: {wave_signals['buy']}")
            print(f"Sell Signal: {wave_signals['sell']}")
            print(f"Target Price: {wave_signals['target']:.25f}" if wave_signals['target'] else "Target Price: None")
            print(f"Stop Loss: {wave_signals['stop_loss']:.25f}" if wave_signals['stop_loss'] else "Stop Loss: None")
            conditions_status[f'elliott_wave_buy_signal_{timeframe}'] = wave_signals['buy']
            wave_info[timeframe]['signals'] = wave_signals
            time_window = {'1m': 1, '3m': 3, '5m': 5}[timeframe]
            # Pass wave target to Pythagorean forecast
            wave_target = wave_signals['target']
            forecast_price, price_rate = forecast_price_per_time_pythagorean(timeframe, candle_map[timeframe], min_threshold_tf, max_threshold_tf, current_price, time_window, closest_reversal, closest_type, wave_target)
    ...
    if position_open:
        ...
        target_value = initial_investment * Decimal('1.01')
        ...
        # Percentage Distance to 1% Profit Target
        ...
        # Percentage Progress to 1% Profit Target
        ...
        # Check for Elliott Wave sell signals
        sell_triggered = False
        for timeframe in ['1m', '3m', '5m']:
            if wave_info[timeframe].get('signals', {}).get('sell', False):
                print(f"Elliott Wave sell signal detected on {timeframe}. Initiating exit...")
                sell_triggered = True
                break
        if sell_triggered or check_exit_condition(float(initial_investment), float(asset_balance)):
            print("Target profit of 1% or Elliott Wave sell signal reached. Initiating exit...")
    ...
    # Print conditions status only when seeking entry (not in trade)
    if not position_open:
        ...
        # Require at least one Elliott Wave buy signal
        all_conditions_met = (
            conditions_status["ML_Forecasted_Price_over_Current_Close"] and
            conditions_status["volume_bullish_1m"] and
            any(conditions_status[f"elliott_wave_buy_signal_{tf}"] for tf in ['1m', '3m', '5m'])
        )
