...
def get_historical_candles(symbol, start_time, end_time, timeframe):
    ...
    if '1m' in candles_by_timeframe and candles_by_timeframe['1m'] is not None:
        close_prices = np.array([candle['close'] for candle in candles_by_timeframe['1m']])
        if len(close_prices) > 0:
            close_price = close_prices[-1]
            return candles_by_timeframe
        else:
            print("Error: No close prices found.")
            return None
    else:
        print("Error: No '1m' candles found.")
        return None
        ...
        try:
            ...
            if '1m' in signals:
                ...
                if 'ht_sine_percent_to_min' in signals['1m'] and 'ht_sine_percent_to_max' in signals['1m']:
                    ...
                    # Calculate the Fourier transform and extract the dominant frequency
                    fft_values = np.fft.fft(close_prices)
                    fft_freqs = np.fft.fftfreq(len(close_prices))
                    dominant_freq = np.abs(fft_freqs[np.argmax(np.abs(fft_values))])

                    print("Dominant frequency:", dominant_freq)

                    # Calculate the phase angle and quadrant based on the dominant frequency
                    phase_angle = np.angle(fft_values[np.argmax(np.abs(fft_values))]) * 180 / np.pi
                    quadrant = int((phase_angle + 180) / 90) % 4

                    print("Phase angle:", phase_angle)
                    print("Quadrant:", quadrant)

                    # Calculate the percentage change from the current close price to the min/max of the sine wave
                    em_field = talib.HT_TRENDMODE(close_prices)
                    sine_wave, _, = talib.HT_SINE(close_prices)
                    em_amp = max(em_field) # Dont know how to make use of it
                    interp_close = np.interp(close_prices[-1], sine_wave, close_prices) 
                    percent_to_min_sine = (interp_close - np.min(sine_wave)) / em_amp * 100
                    percent_to_max_sine = (np.max(sine_wave) - interp_close) / em_amp * 100

                    print("Percent to min sine:", percent_to_min_sine)
                    print("Percent to max sine:", percent_to_max_sine)

                    # Check if EMA periods have been defined
                    if EMA_SLOW_PERIOD is not None and EMA_FAST_PERIOD is not None:
                        # Calculate the EMAs
                    ...
                    # Determine the market mood based on the dominant frequency and quadrant
                    if dominant_freq > 0 and quadrant == 0:
                        market_mood = "Bullish"
                    elif dominant_freq > 0 and quadrant == 1:
                        market_mood = "Bearish"
                    elif dominant_freq < 0 and quadrant == 2:
                        market_mood = "Bearish"
                    elif dominant_freq < 0 and quadrant == 3:
                        market_mood = "Bullish"
                    else:
                        market_mood = "Neutral"
            ...
            # Flush all data after each iteration if an error occurs
            account_balance = None
            em_field = None
            em_amp = None
            em_phase = None
