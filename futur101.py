    ...
    try:
        ...
        while True:
            ...
            for timeframe in TIMEFRAMES:
                ...
                # Set conditions only for 1m timeframe (except DMI)
                if timeframe == "1m":
                    # Momentum conditions
                ...
                analysis_details[timeframe] = {
                    ...
                    "volume_bullish": conditions_long.get("volume_bullish_1m", False) if timeframe == "1m" else None,
                    "volume_bearish": conditions_short.get("volume_bearish_1m", False) if timeframe == "1m" else None,
                ...
                # Print conditions only for 1m timeframe (except DMI)
                if timeframe == "1m":
            ...
            # Print DMI conditions for all timeframes
            for tf in TIMEFRAMES:
                print(f"dmi_bullish_{tf}: {conditions_long.get(f'dmi_bullish_{tf}', False)}, dmi_bearish_{tf}: {conditions_short.get(f'dmi_bearish_{tf}', False)}")
            
            # Print 1m conditions
            print(f"negative_dominant_freq_1m: {conditions_long.get('negative_dominant_freq_1m', False)}, positive_dominant_freq_1m: {conditions_short.get('positive_dominant_freq_1m', False)}")
            print(f"volume_bullish_1m: {conditions_long.get('volume_bullish_1m', False)}, volume_bearish_1m: {conditions_short.get('volume_bearish_1m', False)}")
            print(f"wavelet_forecast_above_current_1m: {conditions_long.get('wavelet_forecast_above_current_1m', False)}, wavelet_forecast_below_current_1m: {conditions_short.get('wavelet_forecast_below_current_1m', False)}")
            ...
            print(f"ml_forecast_above_price_1m: {conditions_long.get('ml_forecast_above_price_1m', False)}, ml_forecast_below_price_1m: {conditions_short.get('ml_forecast_below_price_1m', False)}")
