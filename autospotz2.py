...
while True:
    ...
    if position_open:
        ...
        print(f"Initial USDC amount: {initial_investment_display:.25f}, Expected USDC amount after exit: {target_value:.25f}, Entry Price for last BTC converted: {entry_price:.25f}")
        ...
        if check_exit_condition(initial_investment, asset_balance, entry_price):
            ...
            if success:
                exit_usdc_balance = usdc_amount if usdc_amount else get_balance('USDC')
                ...
                print(f"Position closed. Converted BTC to USDC: {exit_usdc_balance:.25f}")
        ...
        if all_conditions_met:
            ...
            if usdc_balance > Decimal('0'):
                ...
                if success and btc_amount and conversion_price:
                    ...
                    quantity_bought = btc_amount
                    entry_datetime = current_local_time
                    print(f"Converted {usdc_balance:.25f} USDC to {quantity_bought:.25f} BTC at price {entry_price:.25f}")
                    print(f"Entry Datetime: {entry_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
                    position_open = True
                    print(f"New position opened with {usdc_balance:.25f} USDC converted to {quantity_bought:.25f} BTC at price {entry_price:.25f}.")
                    usdc_balance = get_balance('USDC')
                    asset_balance = get_balance(TRADE_SYMBOL.split('USDC')[0])
                else:
