...
import os
import scipy.fftpack as fftpack  # Importing fftpack for Fourier Transform
...
def get_liquidation_levels(symbol):
    ...
    for balance in balances:
        if balance['asset'] == 'USDT':  # Adjust to USDC if needed
...
def calculate_gann_levels(candles):
    """ Calculate Gann levels based on high, low, and last close. """
    highs = np.array([c['high'] for c in candles])
    lows = np.array([c['low'] for c in candles])
    closes = np.array([c['close'] for c in candles])

    gann_high = np.max(highs[-14:])  # Consider the last 14 periods for Gann calculation
    gann_low = np.min(lows[-14:])
    gann_close = closes[-1]

    range_gann = gann_high - gann_low
    approx_gann_levels = {
        "Max": gann_high,
        "Min": gann_low,
        "Avg": (gann_high + gann_low) / 2,
        "45° Angle Price": gann_low + range_gann * 0.5,  # 45 degree from low
    }

    return approx_gann_levels

def calculate_next_reversal_prices(candles):
    """ Calculate next reversal price forecasts and return key points. """
    highs = np.array([c['high'] for c in candles])
    lows = np.array([c['low'] for c in candles])
    closes = np.array([c['close'] for c in candles])
    ...
    # Gann Golden Square based next reversal calculations
    next_top_reversal = last_high * (1 + 0.618) # Using golden ratio
    next_bottom_reversal = last_low * (1 - 0.618)

    return next_top_reversal, next_bottom_reversal

def calculate_harmonic_wave(candles):
    """ Calculate harmonic waves for minima, maxima, and reversal key points. """
    close_prices = np.array([c["close"] for c in candles])
    highs = np.array([c["high"] for c in candles])
    lows = np.array([c["low"] for c in candles])
    ...
    # Safely handle local maxima and minima
    local_maxima = highs[np.r_[False, (highs[1:-1] > highs[:-2]) & (highs[1:-1] > highs[2:]), False]]
    local_minima = lows[np.r_[False, (lows[1:-1] < lows[:-2]) & (lows[1:-1] < lows[2:]), False]]

    max_point = np.max(local_maxima) if len(local_maxima) > 0 else None
    min_point = np.min(local_minima) if len(local_minima) > 0 else None

    return max_point, min_point, [local_maxima, local_minima]
...
def forecast_golden_ratio_reversal(last_reversal):
    """ Forecast incoming reversal prices based on the golden ratio for the 1-minute timeframe. """
    phi = (1 + np.sqrt(5)) / 2  # Golden ratio

    if last_reversal:
        last_reversal_value = last_reversal[1]  # Price of the last reversal

        predicted_price = last_reversal_value * (phi ** (1 / 5))  # Scale by Phi for 1 minute
        print(f"\n--- Incoming Reversal Forecast for 1m ---")
        print(f"Predicted Price for 1m: {predicted_price:.2f}")

        return predicted_price
    else:
        print("Insufficient data for golden ratio calculations.")
        return None

# Placeholder for analyzing energy fields
def analyze_energy_fields(prices, current_sine_close):
    print("Analyzing energy fields...")
    energy_fields = np.mean(prices) + current_sine_close  
...
def generate_report(timeframe, candles, investment, forecast_minutes=12, ml_model=None):
    ...
    # Calculate Gann levels and market mood
    gann_levels = calculate_gann_levels(candles)
    print("Gann Levels:")
    for key, value in gann_levels.items():
        print(f"{key}: {value:.2f}")

    # Set market mood based on the previous Gann levels
    if current_close > gann_levels['Max']:
        market_mood = "Up"
    elif current_close < gann_levels['Min']:
        market_mood = "Down"
    else:
        market_mood = "Stable (Within Gann Levels)"

    print(f"Market Mood: {market_mood}")

    # Calculate next reversal prices based on Gann geometry
    next_top_reversal, next_bottom_reversal = calculate_next_reversal_prices(candles)
    print(f"Next Top Reversal Price Forecast: {next_top_reversal:.2f}")
    print(f"Next Bottom Reversal Price Forecast: {next_bottom_reversal:.2f}")

    # Last reversals
    last_major_reversal, last_minor_reversal = calculate_reversals(candles)

    # Forecasting golden ratio reversals based on last found reversals
    if last_major_reversal:
        predicted_price = forecast_golden_ratio_reversal(last_major_reversal)
    else:
        print("Possible reversals not found.")
    ...
    target_price, market_mood_forecast = forecast_price(candles, n_components=5, target_distance=0.01)
    print(f"Forecasted Price: {target_price:.2f}")
    print(f"Forecast Market Mood: {market_mood_forecast}")
    ...
    harmonic_max, harmonic_min, harmonic_points = calculate_harmonic_wave(candles)
    print(f"Harmonic Analysis - Max: {harmonic_max:.2f}, Min: {harmonic_min:.2f}, Points: {harmonic_points}")
       
    # Calculate distances to harmonic threshold
    distance_to_lower = ((current_close - min(gann_levels['Min'], harmonic_min)) / (max(gann_levels['Max'], harmonic_max) - min(gann_levels['Min'], harmonic_min)) * 100) if (max(gann_levels['Max'], harmonic_max) - min(gann_levels['Min'], harmonic_min)) != 0 else 0
    distance_to_upper = ((max(gann_levels['Max'], harmonic_max) - current_close) / (max(gann_levels['Max'], harmonic_max) - min(gann_levels['Min'], harmonic_min)) * 100) if (max(gann_levels['Max'], harmonic_max) - min(gann_levels['Min'], harmonic_min)) != 0 else 0
    ...
    # Print last reversal found
    print(f"Last Reversal Found - Major: {last_major_reversal}, Minor: {last_minor_reversal}")

    # Analyze energy fields based on sine wave of close prices
    dist_min_close, dist_max_close, current_sine_close = scale_to_sine(timeframe)
    analyze_energy_fields(np.array([c["close"] for c in candles]), current_sine_close)
        ...
        if len(candles) > 0:
            ...
            if timeframe == "1h":
                ...
                if true_count >= 4:
                    ...
                    print(f"DIP found on 1h timeframe at {current_close:.2f} - Recorded to {signal_file1}")
            ...
            if timeframe == "15m":
                ...
                print("\nSummary of Conditions for 15min DIP:")
                print(f"Total TRUE conditions: {true_count}")

                # Check if the conditions meet the trigger requirements for 15m
                if true_count >= 4:
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
                    with open(signal_file2, "w") as f:  # Overwrites existing file content
                        f.write(f"{timestamp} - SIGNAL: DIP found on 15min timeframe at {current_close:.2f}\n")
                    print(f"DIP found on 15min timeframe at {current_close:.2f} - Recorded to {signal_file2}")
            ...
            if timeframe == "3m":
                ...
                if true_count >= 4:
                    ...
                    print(f"DIP found on 3min timeframe at {current_close:.2f} - Recorded to {signal_file3}")
            ...
            if timeframe == "1m":
                ...
                if true_count >= 4:
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
                    with open(signal_file4, "w") as f:  # Overwrites existing file content
                        f.write(f"{timestamp} - SIGNAL: DIP found on 1min timeframe at {current_close:.2f}\n")
                    print(f"DIP found on 1min timeframe at {current_close:.2f} - Recorded to {signal_file4}")
