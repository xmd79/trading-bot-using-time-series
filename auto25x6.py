"""
HFT Auto Trading Bot — KuCoin Futures Edition v6 (FFT TARGET PRICE LAYER)
=========================================================================
SIGNAL LOGIC:
...
NEW — FFT CYCLE TARGET PRICE LAYER (v6):
  - Decomposes 1m closes via real FFT into dominant frequency components.
  - Projects the dominant cycle N bars forward (N = half dominant period).
  - Computes a "cycle target price" = entry_price ± projected amplitude.
  - ENTRY FILTER: only enters if cycle_target >= TP price (long) or <= TP price (short).
    This ensures the dominant harmonic is already pointing toward your TP before entry.
  - TP is still the fixed ROE target; the FFT target is a pre-entry gate, not a replacement.
...
print("HFT KuCoin Bot (25x / 3-OUT-OF-5 + FFT TARGET LAYER v6) initialising...")
if _cleaned:
...
# ── FFT Target Layer config ────────────────────────────────────────────────────
# Number of dominant harmonics to reconstruct the cycle projection
FFT_TARGET_TOP_N = 3
# Minimum ratio: fft_target_distance / tp_distance must be >= this to allow entry.
# e.g. 1.0 means cycle target must reach AT LEAST the TP distance.
# Raise to 1.2 for extra confirmation that cycle overshoots TP.
FFT_TARGET_MIN_RATIO = 1.0
# Window of recent 1m bars used for FFT target projection
...
def analyze_fft_dominance_1m(close_prices_1m):
    """Original FFT dominance condition (condition #5 in the 5-condition signal system)."""
...
def compute_fft_cycle_target(close_prices_1m, entry_price, side):
    """
    NEW v6 FFT CYCLE TARGET PRICE LAYER
    =====================================
    Uses real FFT (rfft) on detrended 1m closes to:
      1. Identify the dominant cycle frequency and its period.
      2. Reconstruct a forward projection of the dominant N harmonics.
      3. Compute a "cycle target price" = projected price at (half-period) bars ahead.
      4. Return whether this target reaches or exceeds the TP price.

    This answers: "Is the dominant market cycle currently pointing toward my TP?"

    Returns:
        fft_target_ok   (bool)  : True if cycle target >= tp_price (long) or <= tp_price (short)
        fft_target_price(float) : the projected cycle price N bars forward
        dominant_period (float) : dominant cycle period in bars
        dominant_freq   (float) : dominant frequency (cycles/bar)
        cycle_amplitude (float) : amplitude of dominant harmonic as % of price
    """
    try:
        n_window = FFT_TARGET_WINDOW
        if len(close_prices_1m) < n_window:
            return False, 0.0, 0.0, 0.0, 0.0

        prices = np.array(close_prices_1m[-n_window:], dtype=np.float64)

        # ── 1. Detrend: remove linear trend so FFT sees the oscillation, not the drift
        ...
        freqs = rfftfreq(n_window)           # cycles per bar, 0..0.5
        magnitudes = np.abs(fft_coeffs)

        # Ignore DC (index 0) and very high frequencies (noise: period < 4 bars)
        MIN_PERIOD_BARS = 4
        MAX_FREQ = 1.0 / MIN_PERIOD_BARS     # 0.25
        valid_mask = (freqs > 0) & (freqs <= MAX_FREQ)
        ...
        # ── 4. Dominant frequency = peak magnitude in valid range
        dom_idx_in_valid = int(np.argmax(valid_mags))
        dom_freq = float(valid_freqs[dom_idx_in_valid])
        dom_period = 1.0 / dom_freq if dom_freq > 0 else n_window

        # ── 5. Reconstruct forward projection using top-N harmonics
        #       Project to half a dominant period ahead (the expected turning point)
        projection_bars = max(1, int(dom_period / 2))

        # Sort all valid bins by magnitude, take top N
        top_n_indices = valid_indices[np.argsort(valid_mags)[-FFT_TARGET_TOP_N:]]

        # Reconstruct the signal at t = n_window + projection_bars
        t_future = float(n_window + projection_bars)
        projected_osc = 0.0
        for idx in top_n_indices:
            freq_i = float(freqs[idx])
            coeff_i = fft_coeffs[idx]
            # Inverse contribution: Re(coeff * exp(2πi * freq * t)) / (n/2)
            phase_factor = np.exp(2j * np.pi * freq_i * t_future)
            contribution = np.real(coeff_i * phase_factor) / (n_window / 2)
            projected_osc += contribution

        # ── 6. Add back the trend extrapolated to future bar
        trend_at_future = np.polyval(trend_coeffs, t_future)
        # Undo window correction: window mean reduces amplitude; compensate approximately
        window_correction = np.mean(window_func) if np.mean(window_func) > 0 else 1.0
        fft_target_price = float(trend_at_future + projected_osc / window_correction)

        # ── 7. Amplitude as % of current price (informational)
        dom_amplitude_raw = float(valid_mags[dom_idx_in_valid]) / (n_window / 2) / window_correction
        cycle_amplitude_pct = (dom_amplitude_raw / (entry_price + 1e-12)) * 100.0

        # ── 8. TP price levels for comparison
        tp_long_price  = entry_price * (1.0 + TP_PRICE_PCT)
        tp_short_price = entry_price * (1.0 - TP_PRICE_PCT)

        # ── 9. Gate: does the FFT target reach the TP price?
        #    For LONG: fft_target_price must be >= tp_long_price * FFT_TARGET_MIN_RATIO factor
        #    For SHORT: fft_target_price must be <= tp_short_price
        if side == "long":
            required_move = entry_price + (tp_long_price - entry_price) * FFT_TARGET_MIN_RATIO
            fft_target_ok = fft_target_price >= required_move
        else:  # short
            required_move = entry_price - (entry_price - tp_short_price) * FFT_TARGET_MIN_RATIO
            fft_target_ok = fft_target_price <= required_move

        return fft_target_ok, fft_target_price, dom_period, dom_freq, cycle_amplitude_pct
...
def compute_signals(buffer_5m, buffer_1m, live_price):
    ...
    # ── NEW v6: FFT CYCLE TARGET LAYER ────────────────────────────────────────
    # Compute FFT target for whichever direction has a signal candidate.
    # This runs AFTER the 3/5 gate so it only fires when we might actually trade.
    fft_target_long_ok  = False
    fft_target_short_ok = False
    fft_target_price    = 0.0
    fft_dom_period      = 0.0
    fft_dom_freq        = 0.0
    fft_cycle_amp_pct   = 0.0

    if is_long or is_short:
        candidate_side = "long" if is_long else "short"
        (fft_target_ok_val,
         fft_target_price,
         fft_dom_period,
         fft_dom_freq,
         fft_cycle_amp_pct) = compute_fft_cycle_target(closes_1m_raw, live_price, candidate_side)

        if candidate_side == "long":
            fft_target_long_ok  = fft_target_ok_val
            is_long  = is_long  and fft_target_long_ok
        else:
            fft_target_short_ok = fft_target_ok_val
            is_short = is_short and fft_target_short_ok
    # ──────────────────────────────────────────────────────────────────────────
    ...
    return {
        ...
        "cond_flags": {
            ...
            "fft_target_long": fft_target_long_ok,
            "fft_target_short": fft_target_short_ok,
        },
        ...
        # FFT Target Layer output
        "fft_target_price": fft_target_price,
        "fft_dom_period": fft_dom_period,
        "fft_dom_freq": fft_dom_freq,
        "fft_cycle_amp_pct": fft_cycle_amp_pct,
    }
...
def write_trade_to_journal(trade_result):
    ...
    fft_tp = trade_result.get("fft_target_price", 0.0)
    fft_per = trade_result.get("fft_dom_period", 0.0)
    fft_amp = trade_result.get("fft_cycle_amp_pct", 0.0)
    line = (f"{'='*60}\nMODE: {mode}\nTYPE: {trade_result.get('type', 'N/A').upper()}\n"
            ...
            f"FFT TARGET: {fft_tp:.2f} | DOM PERIOD: {fft_per:.1f} bars | CYCLE AMP: {fft_amp:.4f}%\n"
            f"LEVERAGE: {LEVERAGE}x\nSTRATEGY: 3/5 Cond (Mom+Vol+ML Mandatory) + FFT Target Gate | SizeAlloc: {TRADE_BALANCE_PCT*100:.0f}%\n{'='*60}\n\n")
    try:
...
def print_conditions(sig):
    ...
    print(f"  5. FFT Dom (1m):    Neg:{sig['neg_ratio']:.2f}% Pos:{sig['pos_ratio']:.2f}% L:{f['fft_long']} S:{f['fft_short']}")
    ...
    # FFT Target Layer
    fft_tp  = sig.get("fft_target_price", 0.0)
    fft_per = sig.get("fft_dom_period", 0.0)
    fft_frq = sig.get("fft_dom_freq", 0.0)
    fft_amp = sig.get("fft_cycle_amp_pct", 0.0)
    tp_long_price  = sig["price"] * (1.0 + TP_PRICE_PCT)
    tp_short_price = sig["price"] * (1.0 - TP_PRICE_PCT)
    if fft_tp > 0:
        print(f"  7. FFT Target(v6):  CycleTarget:{fft_tp:.2f} | Dom Period:{fft_per:.1f}bars | Freq:{fft_frq:.4f} | Amp:{fft_amp:.4f}%")
        print(f"                      TP_Long:{tp_long_price:.2f} TP_Short:{tp_short_price:.2f}")
        print(f"                      FFT→TP_Long_Gate:{f['fft_target_long']} FFT→TP_Short_Gate:{f['fft_target_short']} [MANDATORY GATE]")
    else:
        print(f"  7. FFT Target(v6):  (not evaluated — waiting for 3/5 signal)")
    
    long_true  = sig["long_true_count"]
    short_true = sig["short_true_count"]
    
    print(f"  ═══ LONG:{long_true}/5 SHORT:{short_true}/5 (Rule: Mom+Vol+ML Mandatory + >=3/5 Total + FFT Target Gate) -> LONG:{sig['is_long']} SHORT:{sig['is_short']}")
...
def main():
    print(f"\n{'='*60}")
    print(f"HFT KuCoin Bot v6 - 3/5 CONDITIONS + FFT CYCLE TARGET GATE")
    print(f"{'='*60}")
    print(f"Symbol:              {TRADE_SYMBOL}")
    print(f"Leverage:            {LEVERAGE}x")
    print(f"TP: {TAKE_PROFIT_ROE}% ROE | SL: {STOP_LOSS_ROE}% ROE")
    print(f"Loop Sleep:          {LOOP_SLEEP}s")
    print(f"Entry Logic:         Mom & Vol & ML MANDATORY + At least 3/5 Total + FFT Target Gate")
    print(f"FFT Target Config:   Window:{FFT_TARGET_WINDOW} bars | Top-N:{FFT_TARGET_TOP_N} harmonics | Min Ratio:{FFT_TARGET_MIN_RATIO}x TP")
    ...
    while True:
        try:
            ...
            if pos["is_open"]:
                ...
                if reason:
                    ...
                    trade_result = {
                        ...
                        "fft_target_price": saved_state.get("fft_target_price", 0.0),
                        "fft_dom_period": saved_state.get("fft_dom_period", 0.0),
                        "fft_cycle_amp_pct": saved_state.get("fft_cycle_amp_pct", 0.0),
                    }
            ...
            if not pos["is_open"] and not saved_state:
                ...
                if sig["is_long"]:
                    print(f"\n  *** 3/5 + FFT TARGET GATE MET -> LONG ***")
                    print(f"      FFT Cycle Target: {sig.get('fft_target_price', 0):.2f} | TP Required: {current_price * (1 + TP_PRICE_PCT):.2f}")
                    if has_sufficient_balance:
                        ...
                        if execute_entry(TRADE_SYMBOL, "buy", balance, sig["price"]):
                            ...
                            saved_state = {
                                "active": True, "mode": "LIVE", "side": "long", 
                                "entry_price": sig["price"], "sl": sl_price, "tp": tp_price,
                                "entry_time": now_str,
                                "fft_target_price": float(sig.get("fft_target_price", 0.0)),
                                "fft_dom_period": float(sig.get("fft_dom_period", 0.0)),
                                "fft_cycle_amp_pct": float(sig.get("fft_cycle_amp_pct", 0.0)),
                            }
                        ...
                        sl_price, tp_price = calculate_sl_tp(sig["price"], "long")
                        saved_state = {
                            "active": True, "mode": "SIMULATION", "side": "long",
                            "entry_price": sig["price"], "sl": sl_price, "tp": tp_price,
                            "entry_time": now_str,
                            "fft_target_price": float(sig.get("fft_target_price", 0.0)),
                            "fft_dom_period": float(sig.get("fft_dom_period", 0.0)),
                    ...
                    if has_sufficient_balance:
                        ...
                        if execute_entry(TRADE_SYMBOL, "sell", balance, sig["price"]):
                            ...
                            saved_state = {
                                "active": True, "mode": "LIVE", "side": "short", 
                                "entry_price": sig["price"], "sl": sl_price, "tp": tp_price,
                                "entry_time": now_str,
                                "fft_target_price": float(sig.get("fft_target_price", 0.0)),
                                "fft_dom_period": float(sig.get("fft_dom_period", 0.0)),
                        ...
                        sl_price, tp_price = calculate_sl_tp(sig["price"], "short")
                        saved_state = {
                            "active": True, "mode": "SIMULATION", "side": "short",
                            "entry_price": sig["price"], "sl": sl_price, "tp": tp_price,
                            "entry_time": now_str,
                            "fft_target_price": float(sig.get("fft_target_price", 0.0)),
                            "fft_dom_period": float(sig.get("fft_dom_period", 0.0)),
