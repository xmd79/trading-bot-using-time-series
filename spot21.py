...
def calculate_enhanced_tsi(close_prices, long_length=25, short_length=5, signal_length=14, smoothing_length=5):
    """Calculate an enhanced TSI with smoothing."""
    close_prices = np.array(close_prices)
    ...
    smoothed_short_momentum = talib.EMA(momentum, timeperiod=short_length)
    smoothed_long_momentum = talib.EMA(momentum, timeperiod=long_length)
    smoothed_momentum = smoothed_short_momentum - smoothed_long_momentum
    ...
    # Apply smoothing to TSI
    tsi_smoothing = talib.EMA(tsi, timeperiod=smoothing_length)

    # Get the signal for TSI
    tsi_signal = talib.EMA(tsi_smoothing, timeperiod=signal_length)

    return tsi_smoothing, tsi_signal

def calculate_spectral_analysis(close_prices):
    """Calculate spectral analysis using FFT."""
    close_prices = np.array(close_prices)
    # Perform FFT
    freq_data = fft(close_prices)
    freq_magnitude = np.abs(freq_data)
    return freq_magnitude[:len(close_prices) // 2], freq_data[:len(close_prices) // 2]

def calculate_fear_greed_ratio(freq_magnitude):
    """Calculate the fear and greed ratio based on FFT results."""
    negative_freqs = freq_magnitude[freq_magnitude < 0]
    positive_freqs = freq_magnitude[freq_magnitude > 0]
    
    fear = np.sum(np.abs(negative_freqs))  # Total negative frequency magnitude
    greed = np.sum(np.abs(positive_freqs))  # Total positive frequency magnitude
    
    total = fear + greed
    if total == 0:
        return 0, 0  # Avoid division by zero
    return fear / total, greed / total
...
while True:  
    ...
    # Calculate enhanced TSI for all timeframes and print the requested values 
    tsi_values = {}
    tsi_signals = {}
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map:
            closes = [candle['close'] for candle in candle_map[timeframe]]
            tsi_values[timeframe], tsi_signals[timeframe] = calculate_enhanced_tsi(closes)
            ...
            if tsi_values[timeframe].size > 0:
                ...
                print(f"Current Enhanced TSI Value for {timeframe}: {current_tsi_value:.2f}")
                ...
                freq_magnitude, _ = calculate_spectral_analysis(closes)
                fear_ratio, greed_ratio = calculate_fear_greed_ratio(freq_magnitude)

                print(f"Fear Ratio for {timeframe}: {fear_ratio:.2f}, Greed Ratio: {greed_ratio:.2f}")
    ...
    conditions_status = {
        ...
        "mtf_bullish_signal": False,  
        "distance_to_mtf_condition": dist_to_min_mtf_tsi < dist_to_max_mtf_tsi,  
    }
    ...
    for timeframe in ['1m', '3m', '5m']:
        if timeframe in candle_map:
            ...
            conditions_status["dip_condition_met"] = current_close < min_threshold  
