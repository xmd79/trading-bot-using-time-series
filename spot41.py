...
def get_target(closes, n_components, target_distance=0.01):
    ...
    target_price = np.nanmax(filtered_signal)  # Use filtered signal to find the key target
    ...
    # Combine additional market logic
    order_book = get_order_book(TRADE_SYMBOL)
    bai = calculate_bai(order_book) if order_book else 0

    # Market mood logic based on BAI
    if bai > 0:
        market_mood = "Bullish"
    elif bai < 0:
        market_mood = "Bearish"
...
while True:
    ...
    for timeframe in ['1m', '3m', '5m']:  # Check 3m timeframe as well
        if timeframe in candle_map:
            ...
            current_time, entry_price, stop_loss, reversal_target, market_mood = get_target(closes, n_components=5, target_distance=0.01)
