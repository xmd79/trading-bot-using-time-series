    ...
    for attempt in range(retries):
        try:
            ...
            for tf in TIMEFRAMES:
                ...
                # Add LRC information for 1m timeframe
                if tf == "1m":
                    message += (
    ...
    try:
        ...
        while True:
            ...
            conditions_long = {
                ...
                # LRC condition for 1m (will be set as second filter)
                "lrc_below_lower_1m": False,
            }
            ...
            # Set LRC conditions for 1m timeframe (second filter)
            if lrc_data["1m"]["valid"]:
                conditions_long["lrc_below_lower_1m"] = lrc_data["1m"]["below_lower"]
                conditions_short["lrc_above_upper_1m"] = lrc_data["1m"]["above_upper"]
                
                print(f"LRC Below Lower (1m): {conditions_long['lrc_below_lower_1m']}")
                print(f"LRC Above Upper (1m): {conditions_short['lrc_above_upper_1m']}")
            else:
                # If LRC calculation failed, set conditions to False
                conditions_long["lrc_below_lower_1m"] = False
                conditions_short["lrc_above_upper_1m"] = False
            ...
            for timeframe in TIMEFRAMES:
                ...
                reversal_type, min_time, closest_min_price, _, max_time, closest_max_price, min_idx, max_idx = detect_recent_reversal(
                    candles_tf, timeframe, min_threshold, max_threshold, current_close_tf, higher_tf_tops if timeframe == "1m" else None
                )
            ...
            for cond_pair in [
                ...
                ("lrc_below_lower_1m", "lrc_above_upper_1m")
            ]:
            ...
            # Apply LRC as a second filter only when all other conditions are met
            long_signal = False
            short_signal = False
            
            # Check if all required conditions are met
            all_long_conditions_met = (long_true_count == total_required_long)
            all_short_conditions_met = (short_true_count == total_required_short)
            
            # Apply LRC second filter
            if all_long_conditions_met and conditions_long.get("lrc_below_lower_1m", False):
                long_signal = True
                print("\n🔍 LRC Second Filter: LONG signal confirmed (price below lower channel)")
                logging.info("LRC Second Filter: LONG signal confirmed (price below lower channel)")
            elif all_long_conditions_met and not conditions_long.get("lrc_below_lower_1m", False):
                print("\n🔍 LRC Second Filter: LONG signal rejected (price not below lower channel)")
                logging.info("LRC Second Filter: LONG signal rejected (price not below lower channel)")
            
            if all_short_conditions_met and conditions_short.get("lrc_above_upper_1m", False):
                short_signal = True
                print("\n🔍 LRC Second Filter: SHORT signal confirmed (price above upper channel)")
                logging.info("LRC Second Filter: SHORT signal confirmed (price above upper channel)")
            elif all_short_conditions_met and not conditions_short.get("lrc_above_upper_1m", False):
                print("\n🔍 LRC Second Filter: SHORT signal rejected (price not above upper channel)")
                logging.info("LRC Second Filter: SHORT signal rejected (price not above upper channel)")
            
            print(f"\nTrade Signal Status:")
            print(f"LONG Signal: {'Active' if long_signal else 'Inactive'} (Conditions: {long_true_count}/{total_required_long} + LRC filter)")
            print(f"SHORT Signal: {'Active' if short_signal else 'Inactive'} (Conditions: {short_true_count}/{total_required_short} + LRC filter)")
            
            logging.info(f"LONG Signal: {'Active' if long_signal else 'Inactive'} (Conditions: {long_true_count}/{total_required_long} + LRC filter)")
            logging.info(f"SHORT Signal: {'Active' if short_signal else 'Inactive'} (Conditions: {short_true_count}/{total_required_short} + LRC filter)")
            ...
            current_long_condition_met = (long_true_count == total_required_long) and conditions_long.get("lrc_below_lower_1m", False)
            current_short_condition_met = (short_true_count == total_required_short) and conditions_short.get("lrc_above_upper_1m", False)
            ...
            if signal in ["LONG", "SHORT"] and position["side"] == "NONE":
                ...
                if signal == "LONG":
                    ...
                    if conditions_long.get("lrc_below_lower_1m", False):
                        true_conditions.append("LRC: Price below lower channel (1m)")
                    total_conditions = total_required_long + 1  # +1 for LRC filter
                    condition_count = long_true_count + (1 if conditions_long.get("lrc_below_lower_1m", False) else 0)
                else:
                    true_conditions = [cond for cond in required_short_conditions if conditions_short.get(cond, False)]
                    if conditions_short.get("lrc_above_upper_1m", False):
                        true_conditions.append("LRC: Price above upper channel (1m)")
                    total_conditions = total_required_short + 1  # +1 for LRC filter
                    condition_count = short_true_count + (1 if conditions_short.get("lrc_above_upper_1m", False) else 0)
