...
def scale_to_sine_cycles(timeframe):  
    close_prices = np.array(get_close(timeframe))
    ...
    # Uptrend pattern logic
    if current_sine < 0:  # Assuming minimum point is at 0 radians phase
        print(f"{timeframe} Uptrend: A to B (AB)")
    elif 0 <= current_sine < np.pi/2:
        print(f"{timeframe} Uptrend: B to C (BC)")
    elif np.pi/2 <= current_sine < np.pi:
        print(f"{timeframe} Uptrend: C to D (CD)")
    elif np.pi <= current_sine <= 2*np.pi:
        print(f"{timeframe} Uptrend: D to E (DE)")

    # Downtrend pattern logic
    elif 2*np.pi > current_sine >= np.pi:
        print(f"{timeframe} Downtrend: E to D (ED)")
    elif np.pi >= current_sine > np.pi/2:
        print(f"{timeframe} Downtrend: D to C (DC)")
    elif np.pi/2 >= current_sine > 0:
        print(f"{timeframe} Downtrend: C to B (CB)")
    elif 0 >= current_sine >= -np.pi/2:
        print(f"{timeframe} Downtrend: B to A (BA)")

    return avg_dist_min, avg_dist_max, current_sine

# Call function           
for timeframe in timeframes:        
    scale_to_sine_cycles(timeframe)
...
# Complete the leverage_octave_calculation function
def leverage_octave_calculation(sine_wave):
    # Calculate octave based on the range of sine values
    octave = np.max(sine_wave) - np.min(sine_wave)
    return normalize_distance(octave, 0, 100)

def calculate_rate_to_eman_ratio_percentage(sine_wave):
    # Assuming rate is associated with frequency
    frequency = np.abs(np.fft.fftfreq(len(sine_wave)))
    rate = frequency[np.argmax(np.abs(np.fft.fft(sine_wave)))]

    # Assuming emanation is associated with amplitude
    amplitude = np.max(np.abs(sine_wave))

    # Calculate the ratio
    ratio = rate / amplitude
...
# Complete the calculate_frequency_spectrum function
def calculate_frequency_spectrum(sine_wave):
    # Calculate frequency spectrum
    spectrum = np.fft.fft(sine_wave)

    # Consider the last 3 most negative and last 3 most positive frequencies
    num_freqs = 3
    neg_freq_indices = np.argsort(spectrum)[:num_freqs]
    pos_freq_indices = np.argsort(spectrum)[-num_freqs:]

    # Calculate average frequencies for negative and positive frequencies
    avg_neg_freq = np.abs(np.mean(np.fft.fftfreq(len(sine_wave))[neg_freq_indices]))
    avg_pos_freq = np.abs(np.mean(np.fft.fftfreq(len(sine_wave))[pos_freq_indices]))

    # Calculate average between the last 3 most negative and last 3 most positive frequencies
    avg_frequency = (avg_neg_freq + avg_pos_freq) / 2

    # Provide forecast based on trend
    trend_forecast = "Up" if avg_pos_freq < avg_neg_freq else "Down"
...
# Function to scale current close price to sine wave
def scale_to_sine_freq(timeframe):
    close_prices = np.array(get_close(timeframe))
    ...
    for close in close_prices:
        # Calculate distances as percentages
        dist_from_close_to_min = (
            (current_sine - sine_wave_min) / (sine_wave_max - sine_wave_min)
        ) * 100
        dist_from_close_to_max = (
            (sine_wave_max - current_sine) / (sine_wave_max - sine_wave_min)
        ) * 100
    ...
    return {
        "timeframe": timeframe,
        "current_momentum": current_close - close_prices[-2],
        "trend_forecast": "Up" if current_close > close_prices[-2] else "Down",
        "current_harmonics": sine_wave,
        "current_octave": leverage_octave_calculation(sine_wave),
        "rate_to_eman_ratio_percentage": calculate_rate_to_eman_ratio_percentage(sine_wave),
        "current_frequency_spectrum": calculate_frequency_spectrum(sine_wave),
        "avg_distance_to_min": avg_dist_min,
        "avg_distance_to_max": avg_dist_max,
        "dist_from_harmonics_to_min": dist_from_close_to_min,
        "dist_from_harmonics_to_max": dist_from_close_to_max,
        "normalized_dist_octave_to_min": normalize_distance(dist_from_close_to_min, 0, 100),
        "normalized_dist_octave_to_max": normalize_distance(dist_from_close_to_max, 0, 100),
        "normalized_dist_frequency_to_min": normalize_distance(avg_dist_min, 0, 100),
        "normalized_dist_frequency_to_max": normalize_distance(avg_dist_max, 0, 100)
    }

# Placeholder function for normalizing distances
def normalize_distance(distance, min_val, max_val):
    return ((distance - min_val) / (max_val - min_val)) * 100

# Call function and print outside
for timeframe in timeframes:
    analysis_info = scale_to_sine_freq(timeframe)
    
    print(f"{analysis_info['timeframe']} Analysis:")
    print(f"Current Momentum: {analysis_info['current_momentum']}")
    print(f"Market Freq Trend Forecast: {analysis_info['trend_forecast']}")
    print(f"Current Harmonics: {analysis_info['current_harmonics']}")
    print(f"Current Octave: {analysis_info['current_octave']}")
    print(f"Current Rate to Eman Ratio Percentage: {analysis_info['rate_to_eman_ratio_percentage']}")
    print(f"Current Frequency Spectrum: {analysis_info['current_frequency_spectrum']['spectrum']}")
    print(f"Avg Frequency: {analysis_info['current_frequency_spectrum']['avg_frequency']}")
    print(f"Average Distance to Min: {analysis_info['avg_distance_to_min']:.2f}% and Average Distance to Max: {analysis_info['avg_distance_to_max']:.2f}%")
        ...
        try:     
            ...
            #for timeframe in timeframes:
                ...
                #print(f"On {timeframe} Distance from close to max perc. is now at: {dist_from_close_to_max})")
                    ...
                    #print(f"Close on {timeframe} tf closer to reversal dip area")

                #elif dist_from_close_to_min > dist_from_close_to_max:
                    #print(f"Close on {timeframe} tf closer to reversal top area")
            ...
            for timeframe in timeframes:
                ...
                # Extract information from analysis_info
                timeframe_label = analysis_info['timeframe']
                current_momentum = analysis_info['current_momentum']
                trend_forecast = analysis_info['trend_forecast']
                current_harmonics = analysis_info['current_harmonics']
                current_octave = analysis_info['current_octave']
                rate_to_eman_ratio_percentage = analysis_info['rate_to_eman_ratio_percentage']
                current_spectrum = analysis_info['current_frequency_spectrum']['spectrum']
                avg_frequency = analysis_info['current_frequency_spectrum']['avg_frequency']
                avg_dist_to_min = analysis_info['avg_distance_to_min']
                avg_dist_to_max = analysis_info['avg_distance_to_max']

                # Print detailed analysis
                print(f"{timeframe_label} Analysis:")
                print(f"Current Momentum: {current_momentum}")
                print(f"Market Freq Trend Forecast: {trend_forecast}")
                print(f"Current Harmonics: {current_harmonics}")
                print(f"Current Octave: {current_octave}")
                print(f"Current Rate to Eman Ratio Percentage: {rate_to_eman_ratio_percentage}")
                print(f"Current Frequency Spectrum: {current_spectrum}")
                print(f"Avg Frequency: {avg_frequency}")
                print(f"Average Distance to Min: {avg_dist_to_min:.2f}% and Average Distance to Max: {avg_dist_to_max:.2f}%")
                                    ...
                                    if closest_threshold < price:
                                        ...
                                        if forecast_direction == "Up":
                                            ...
                                            if market_mood_sine == "Uptrend":
                                                print("LONG condition 7: market_mood_sine == Uptrend")
                                                if momentum > 0:
                                                    print("LONG condition 8: momentum > 0")
                                                    trigger_long = True

                    if trend_forecast == "Down":
                                    ...
                                    if closest_threshold > price:
                                        ...
                                        if forecast_direction == "Down":
                                            ...
                                            if market_mood_sine == "Downtrend":
                                                print("SHORT condition 7: market_mood_sine == Downtrend")
                                                if momentum < 0:
