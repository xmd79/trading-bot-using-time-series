...
def generate_fft_forecast(candles, timeframe, forecast_periods=5):
    ...
    try:
        ...
        top_indices = np.argsort(magnitudes)[::-1][:5]  # Top 5 frequencies including DC if not zeroed
        ...
        for idx in top_indices:
            if freqs[idx] == 0:
                continue  # Skip DC since we subtracted mean
            amp = np.abs(fft_result[idx]) / len(closes) * 2
            phase = np.angle(fft_result[idx])
            forecast += amp * np.cos(2 * np.pi * freqs[idx] * future_t + phase)
        ...
        if forecast_price > current_close:  # Assuming upward forecast
        ...
        logging.info(f"{timeframe} - FFT Forecast: {forecast_price:.25f}")
        print(f"{timeframe} - FFT Forecast: {forecast_price:.25f}")
        return forecast_price
    except Exception as e:
