...
LEVERAGE = 25
STOP_LOSS_PERCENTAGE = Decimal('0.99')  # 99% stop-loss
...
MAGIC_CONSTANT = 34  # Sum of rows, columns, diagonals in a 4x4 magic square
MAGIC_SQUARE_STATES = 4  # Number of states in the magic square cycle
...
try:
    ...
    if len(lines) != 2:
        logging.error(f"credentials.txt must contain exactly 2 non-empty lines, found {len(lines)}.")
        print(f"Error: credentials.txt must contain exactly 2 non-empty lines, found {len(lines)}.")
...
def calculate_dmi(candles, timeframe, period=DMI_PERIOD):
    ...
    - candles: List of candle data
    - timeframe: Timeframe string (e.g., "1m", "5m")
    - period: Period for DMI calculation (default: 7 for 1min)
    
    Returns:
    - plus_di: +DI value
    - minus_di: -DI value
    - adx: ADX value
    - is_bullish: Boolean indicating if DMI is bullish (+DI > -DI and ADX > threshold)
    - is_bearish: Boolean indicating if DMI is bearish (-DI > +DI and ADX > threshold)
    """
    if len(candles) < period:
        logging.warning(f"Insufficient candles ({len(candles)}) for DMI calculation in {timeframe}.")
        print(f"Insufficient candles ({len(candles)}) for DMI calculation in {timeframe}.")
    ...
    try:
        ...
        # Calculate DMI indicators using TALib
        ...
        latest_plus_di = plus_di[-1] if not np.isnan(plus_di[-1]) else None
        ...
        logging.info(
            ...
            f"Bullish={is_bullish}, Bearish={is_bearish}"
        )
        print(
            f"{timeframe} - DMI: +DI={latest_plus_di:.4f}, -DI={latest_minus_di:.4f}, ADX={latest_adx:.4f}, "
            f"Bullish={is_bullish}, Bearish={is_bearish}"
        ...
        return latest_plus_di, latest_minus_di, latest_adx, is_bullish, is_bearish
    except Exception as e:
...
def detect_lrc_reversal(candles, timeframe, lrc_data, current_close):
    """
    Detect reversal based on Linear Regression Channel (LRC) extremes.
    
    Parameters:
    - candles: List of candle data
    - timeframe: Timeframe string (e.g., "1m", "5m")
    - lrc_data: Dictionary containing LRC values (regression, upper, lower, etc.)
    - current_close: Current closing price
    
    Returns:
    - reversal_type: Type of reversal ("DIP" or "TOP")
    - reversal_time: Timestamp of the reversal
    - reversal_price: Price of the reversal
    - min_idx: Index of the minimum point (for dip)
    - max_idx: Index of the maximum point (for top)
    """
    if not lrc_data or not lrc_data.get("valid", False):
        # Fall back to standard reversal detection if LRC is not valid
        return detect_standard_reversal(candles, timeframe, current_close)
    
    # Extract LRC values
    lrc_lower = lrc_data.get("lower", 0)
    lrc_upper = lrc_data.get("upper", 0)
    
    if lrc_lower == 0 or lrc_upper == 0:
        # Fall back to standard reversal detection if LRC values are invalid
        return detect_standard_reversal(candles, timeframe, current_close)
    
    # Find the most recent dip (close below lower LRC) and top (close above upper LRC)
    dip_idx = -1
    top_idx = -1
    dip_price = Decimal('0')
    top_price = Decimal('0')
    ...
    for i, candle in enumerate(candles):
        ...
        # Check for dip (close below lower LRC)
        if close_price < lrc_lower and dip_idx == -1:
            dip_idx = i
            dip_price = close_price
        
        # Check for top (close above upper LRC)
        if close_price > lrc_upper and top_idx == -1:
            top_idx = i
            top_price = close_price
    
    # Determine which reversal is more recent
    if dip_idx == -1 and top_idx == -1:
        # No LRC reversal found, fall back to standard
        return detect_standard_reversal(candles, timeframe, current_close)
    elif dip_idx == -1:
        # Only top found
        reversal_type = "TOP"
        reversal_time = candles[top_idx]['time']
        ...
        # Only dip found
        reversal_type = "DIP"
        reversal_time = candles[dip_idx]['time']
        reversal_price = dip_price
        min_idx = dip_idx
        max_idx = -1
    else:
        # Both found, determine which is more recent
        if dip_idx > top_idx:
            reversal_type = "DIP"
            reversal_time = candles[dip_idx]['time']
            reversal_price = dip_price
    ...
    if reversal_type == "DIP":
        dist_to_reversal = abs(current_close - dip_price)
    else:
        dist_to_reversal = abs(current_close - top_price)
    
    logging.info(
        f"{timeframe} - LRC Reversal: {reversal_type} at {reversal_price:.25f}, "
        f"Distance to current close: {dist_to_reversal:.25f}"
    )
    print(
        f"{timeframe} - LRC Reversal: {reversal_type} at {reversal_price:.25f}, "
        f"Distance to current close: {dist_to_reversal:.25f}"
    ...
    return reversal_type, reversal_time, reversal_price, min_idx, max_idx

def detect_standard_reversal(candles, timeframe, current_close):
    """
    Standard reversal detection based on min and max thresholds.
    ...
    if diff_to_high < diff_to_low:
        reversal_type = "TOP"
        reversal_time = max_time
        reversal_price = max_threshold
    else:
        reversal_type = "DIP"
        reversal_time = min_time
        reversal_price = min_threshold
    
    logging.info(
        f"{timeframe} - Standard Reversal: {reversal_type} at {reversal_price:.25f}, "
...
def wavelet_analysis(candles, timeframe, forecast_periods=1):
    ...
    try:
        ...
        logging.warning(f"PyWavelets not available for {timeframe} wavelet analysis.")
        return 0, "N/A", Decimal('0.0'), [], np.zeros(len(candles))
    
    if len(candles) < 10:
        logging.warning(f"Insufficient data ({len(candles)}) for wavelet analysis in {timeframe}.")
        return 0, "N/A", Decimal('0.0'), [], np.zeros(len(candles))
...
def calculate_linear_regression_channel(candles, period=500, deviation_multiplier=2.0, log_scale=False, forecast_periods=1):
    """
    Calculate Linear Regression Channel values for the most recent candle and forecast future prices.
    
    Parameters:
    - candles: List of candle dictionaries
    - period: Lookback period for regression (default: 500)
    - deviation_multiplier: Multiplier for standard deviation (default: 2.0)
    - log_scale: Whether to use log scale (default: False)
    - forecast_periods: Number of periods to forecast (default: 1)
    
    Returns:
    - Dictionary with regression values, position flags, and forecast
    """
    if len(candles) < period:
        period = len(candles)
    ...
    try:
        ...
        upper_channel = regression_line + deviation_multiplier * std_dev
        lower_channel = regression_line - deviation_multiplier * std_dev
        
        # Get values for the most recent point
        ...
        if log_scale:
            current_regression = np.exp(current_regression)
            current_upper = np.exp(current_upper)
            current_lower = np.exp(current_lower)
        
        # Get current close price (original scale)
        ...
        for i in range(1, forecast_periods + 1):
            # Calculate future x value
            future_x = period + i - 1
            # Calculate future price on regression line
        ...
        # If only forecasting one period, return a single value
        forecast = forecast_prices[0] if forecast_periods == 1 else forecast_prices
        ...
        logging.error(f"Error calculating Linear Regression Channel: {e}")
        return {
            "regression": None,
            "upper": None,
            "lower": None,
...
def magic_square_forecast(candles, timeframe='1m'):
    """
    Generate a price forecast using magic square diagonal reversals.
    ...
    Returns:
    - forecast_price: Predicted price for the next period
    - cycle_state: Current state in the magic square cycle (A, B, C, or D)
    - next_state: Predicted next state in the cycle
    """
    if len(candles) < 16:
        logging.warning(f"Insufficient data ({len(candles)}) for magic square forecast in {timeframe}")
        print(f"Insufficient data ({len(candles)}) for magic square forecast in {timeframe}")
        return Decimal('0.0'), "N/A", "N/A"
    
    try:
        # Extract close prices
        closes = np.array([float(c['close']) for c in candles[-16:]], dtype=np.float64)
        
        # Clean the data
        closes = clean_data(closes)
        
        # Create 4x4 grid (row-major order)
        grid = closes.reshape(4, 4)
        
        # Define the magic square states
        # State A: Original grid
        state_a = grid.copy()
        
        # State B: Main diagonal reversal (swap (0,0) with (3,3), (1,1) with (2,2))
        state_b = state_a.copy()
        state_b[0, 0], state_b[3, 3] = state_b[3, 3], state_b[0, 0]
        state_b[1, 1], state_b[2, 2] = state_b[2, 2], state_b[1, 1]
        
        # State C: Anti-diagonal reversal on State B (swap (0,3) with (3,0), (1,2) with (2,1))
        state_c = state_b.copy()
        state_c[0, 3], state_c[3, 0] = state_c[3, 0], state_c[0, 3]
        state_c[1, 2], state_c[2, 1] = state_c[2, 1], state_c[1, 2]
        
        # State D: Main diagonal reversal on State C
        state_d = state_c.copy()
        state_d[0, 0], state_d[3, 3] = state_d[3, 3], state_d[0, 0]
        state_d[1, 1], state_d[2, 2] = state_d[2, 2], state_d[1, 1]
        
        # Calculate sums for rows, columns, and diagonals for each state
        def calculate_state_sums(state):
            row_sums = np.sum(state, axis=1)
            col_sums = np.sum(state, axis=0)
            main_diag_sum = np.trace(state)
            anti_diag_sum = np.trace(np.fliplr(state))
            return row_sums, col_sums, main_diag_sum, anti_diag_sum
        
        sums_a = calculate_state_sums(state_a)
        sums_b = calculate_state_sums(state_b)
        sums_c = calculate_state_sums(state_c)
        sums_d = calculate_state_sums(state_d)
        
        # Determine current state by comparing sums to magic constant
        current_state = "A"
        min_diff = float('inf')
        
        for state_name, state_sums in [("A", sums_a), ("B", sums_b), ("C", sums_c), ("D", sums_d)]:
            row_sums, col_sums, main_diag_sum, anti_diag_sum = state_sums
            all_sums = np.concatenate([row_sums, col_sums, [main_diag_sum, anti_diag_sum]])
            diff = np.mean(np.abs(all_sums - MAGIC_CONSTANT))
        ...
        state_cycle = {"A": "B", "B": "C", "C": "D", "D": "A"}
        next_state = state_cycle[current_state]
        
        # Get the next state grid
        next_state_grid = {
            "A": state_a,
            "B": state_b,
            "C": state_c,
        ...
        # Extract forecast price from the next state grid
        # We use the top-left element as it represents the next value in the sequence
        forecast_price = Decimal(str(next_state_grid[0, 0]))
        
        logging.info(
            f"{timeframe} - Magic Square Forecast: {forecast_price:.25f}, "
        ...
        return forecast_price, current_state, next_state
    except Exception as e:
        logging.error(f"Error in magic square forecast for {timeframe}: {e}")
        print(f"Error in magic square forecast for {timeframe}: {e}")
        return Decimal('0.0'), "N/A", "N/A"
...
def get_position(retries=5, base_delay=5):
    for attempt in range(retries):
        try:
            ...
            # Calculate the correct unrealized PNL if the API returns 0
            if unrealized_pnl == Decimal('0') and quantity != Decimal('0') and entry_price != Decimal('0'):
                current_price = get_current_price()
                if current_price > Decimal('0'):
                    if quantity > Decimal('0'):  # LONG position
                        unrealized_pnl = (current_price - entry_price) * quantity
                    else:  # SHORT position
                        unrealized_pnl = (entry_price - current_price) * abs(quantity)
            
            # Calculate the margin used for the position
            margin = Decimal('0.0')
            if quantity != Decimal('0') and entry_price != Decimal('0'):
                position_size_usdc = abs(quantity) * entry_price
                margin = position_size_usdc / Decimal(str(LEVERAGE))
            ...
            return {
                ...
                "initial_balance": margin,  # Now using margin instead of wallet balance
...
def update_position_pnl(position, current_price):
    """Update the unrealized PNL of a position based on current price"""
...
def place_order(signal, quantity, price, initial_balance, analysis_details, retries=5, base_delay=5):
    ...
    for attempt in range(retries):
        try:
            ...
            # Get signal info from analysis_details
            signal_info = analysis_details.get("signal_info", {})
            condition_count = signal_info.get("condition_count", 0)
            total_conditions = signal_info.get("total_conditions", 0)
            true_conditions = signal_info.get("true_conditions", [])
            
            # Calculate the margin used for the position
            position_size_usdc = quantity * price
            margin = position_size_usdc / Decimal(str(LEVERAGE))
            ...
            if signal == "LONG":
                ...
                # Calculate stop-loss and take-profit based on ROI percentage
                # For LONG position:
                # Stop-loss triggers when PNL is -75% of initial balance
                # Take-profit triggers when PNL is +5% of initial balance
                sl_price = price * (Decimal('1') - (STOP_LOSS_PERCENTAGE / Decimal(str(LEVERAGE))))
                tp_price = price * (Decimal('1') + (TAKE_PROFIT_PERCENTAGE / Decimal(str(LEVERAGE))))
                ...
                position = {
                    ...
                    "initial_balance": margin,  # Use margin instead of wallet balance
                    "sl_price": sl_price, 
                    "tp_price": tp_price
                }
...
def display_trade_status(position, current_price):
    """
    Display detailed status of the current open trade.
    ...
    initial_balance = position["initial_balance"]  # This is now the margin used
    
    # Calculate PNL percentage relative to margin
    pnl_percentage = (unrealized_pnl / initial_balance * Decimal('100')) if initial_balance > Decimal('0') else Decimal('0')
    
    # Calculate ROI percentage (PNL as percentage of position size)
    position_size_usdc = quantity * current_price
    roi_percentage = (unrealized_pnl / position_size_usdc * Decimal('100')) if position_size_usdc > Decimal('0') else Decimal('0')
    
    # Calculate price differences and percentages
    if side == "LONG":
        price_diff = current_price - entry_price
        price_diff_pct = (price_diff / entry_price * Decimal('100')) if entry_price > Decimal('0') else Decimal('0')
        
        # Distance to SL (how much price can drop before hitting SL)
        dist_to_sl_price = current_price - sl_price if sl_price > Decimal('0') else Decimal('0')
        dist_to_sl_pct = (dist_to_sl_price / current_price * Decimal('100')) if current_price > Decimal('0') and sl_price > Decimal('0') else Decimal('0')
        
        # Distance to TP (how much price can rise before hitting TP)
        dist_to_tp_price = tp_price - current_price if tp_price > Decimal('0') else Decimal('0')
        dist_to_tp_pct = (dist_to_tp_price / current_price * Decimal('100')) if current_price > Decimal('0') and tp_price > Decimal('0') else Decimal('0')
        
        # Risk/Reward Ratio
        risk = entry_price - sl_price if sl_price > Decimal('0') else Decimal('0')
        reward = tp_price - entry_price if tp_price > Decimal('0') else Decimal('0')
        risk_reward_ratio = (reward / risk) if risk > Decimal('0') else Decimal('0')
        
    else:  # SHORT
        price_diff = entry_price - current_price
        price_diff_pct = (price_diff / entry_price * Decimal('100')) if entry_price > Decimal('0') else Decimal('0')
        
        # Distance to SL (how much price can rise before hitting SL)
        dist_to_sl_price = sl_price - current_price if sl_price > Decimal('0') else Decimal('0')
        dist_to_sl_pct = (dist_to_sl_price / current_price * Decimal('100')) if current_price > Decimal('0') and sl_price > Decimal('0') else Decimal('0')
        
        # Distance to TP (how much price can drop before hitting TP)
        dist_to_tp_price = current_price - tp_price if tp_price > Decimal('0') else Decimal('0')
        dist_to_tp_pct = (dist_to_tp_price / current_price * Decimal('100')) if current_price > Decimal('0') and tp_price > Decimal('0') else Decimal('0')
        
        # Risk/Reward Ratio
        risk = sl_price - entry_price if sl_price > Decimal('0') else Decimal('0')
        reward = entry_price - tp_price if tp_price > Decimal('0') else Decimal('0')
        risk_reward_ratio = (reward / risk) if risk > Decimal('0') else Decimal('0')
    ...
    effective_leverage = (position_size_usdc / initial_balance) if initial_balance > Decimal('0') else LEVERAGE
    
    # Print detailed trade status
    print("\n" + "="*60)
    print(f"CURRENT TRADE STATUS - {side} POSITION")
    print("="*60)
    print(f"Entry Price:      {entry_price:.25f} USDC")
    print(f"Current Price:    {current_price:.25f} USDC")
    print(f"Quantity:         {quantity:.6f} BTC")
    print(f"Position Size:    {position_size_usdc:.2f} USDC")
    print(f"Margin Used:      {initial_balance:.2f} USDC")
    print(f"Stop-Loss:        {sl_price:.25f} USDC")
    print(f"Take-Profit:      {tp_price:.25f} USDC")
    print("-"*60)
    print(f"Price Difference: {price_diff:+.25f} USDC ({price_diff_pct:+.4f}%)")
    print(f"Unrealized PNL:   {unrealized_pnl:+.25f} USDC")
    print(f"ROI:              {roi_percentage:+.4f}%")
    print(f"PNL % (Margin):   {pnl_percentage:+.4f}%")
    print("-"*60)
    print(f"Distance to SL:   {dist_to_sl_price:+.25f} USDC ({dist_to_sl_pct:+.4f}%)")
    print(f"Distance to TP:   {dist_to_tp_price:+.25f} USDC ({dist_to_tp_pct:+.4f}%)")
    ...
    try:
        ...
        while True:
            try:
                ...
                # Update position state based on exchange data
                if position["side"] != "NONE":
                    if exchange_position["side"] == "NONE":
                        # Position was closed externally
                        position = exchange_position
                    else:
                        # Update our local position state with the exchange data
                        # Preserve SL, TP, and initial_balance from our local state
                        sl_price = position["sl_price"]
                        tp_price = position["tp_price"]
                        ...
                        # If SL or TP are zero, recalculate them
                        if position["sl_price"] == Decimal('0') or position["tp_price"] == Decimal('0'):
                            logging.warning("SL or TP is zero, recalculating...")
                            if position["side"] == "LONG":
                                position["sl_price"] = position["entry_price"] * (Decimal('1') - (STOP_LOSS_PERCENTAGE / Decimal(str(LEVERAGE))))
                                position["tp_price"] = position["entry_price"] * (Decimal('1') + (TAKE_PROFIT_PERCENTAGE / Decimal(str(LEVERAGE))))
                            elif position["side"] == "SHORT":
                                position["sl_price"] = position["entry_price"] * (Decimal('1') + (STOP_LOSS_PERCENTAGE / Decimal(str(LEVERAGE))))
                                position["tp_price"] = position["entry_price"] * (Decimal('1') - (TAKE_PROFIT_PERCENTAGE / Decimal(str(LEVERAGE))))
                            logging.info(f"Recalculated SL: {position['sl_price']}, TP: {position['tp_price']}")
                        
                        # Update unrealized PNL based on current price
                        update_position_pnl(position, current_price)
                else:
                    # We don't have an open position in our local state, so use the exchange position
                    position = exchange_position
                ...
                if position["side"] != "NONE":
                    # Calculate PNL percentage difference from initial balance (margin)
                    if position["initial_balance"] > Decimal('0'):
                        ...
                        if position["side"] == "LONG" and pnl_percentage <= -STOP_LOSS_PERCENTAGE * Decimal('100'):
                            logging.info(f"Stop-Loss Triggered: LONG at {current_price:.25f} USDC with PNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)")
                            print(f"\n=== STOP-LOSS TRIGGERED ===\nSide: LONG\nExit Price: {current_price:.25f} USDC\nPNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)\n=========================\n")
                            close_position(position, current_price)
                            position = get_position()  # Update position after closing
                        elif position["side"] == "LONG" and pnl_percentage >= TAKE_PROFIT_PERCENTAGE * Decimal('100'):
                            logging.info(f"Take-Profit Triggered: LONG at {current_price:.25f} USDC with PNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)")
                            print(f"\n=== TAKE-PROFIT TRIGGERED ===\nSide: LONG\nExit Price: {current_price:.25f} USDC\nPNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)\n==========================\n")
                            close_position(position, current_price)
                            position = get_position()  # Update position after closing
                        elif position["side"] == "SHORT" and pnl_percentage <= -STOP_LOSS_PERCENTAGE * Decimal('100'):
                            logging.info(f"Stop-Loss Triggered: SHORT at {current_price:.25f} USDC with PNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)")
                            print(f"\n=== STOP-LOSS TRIGGERED ===\nSide: SHORT\nExit Price: {current_price:.25f} USDC\nPNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)\n=========================\n")
                            close_position(position, current_price)
                            position = get_position()  # Update position after closing
                        elif position["side"] == "SHORT" and pnl_percentage >= TAKE_PROFIT_PERCENTAGE * Decimal('100'):
                            logging.info(f"Take-Profit Triggered: SHORT at {current_price:.25f} USDC with PNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)")
                            print(f"\n=== TAKE-PROFIT TRIGGERED ===\nSide: SHORT\nExit Price: {current_price:.25f} USDC\nPNL: {position['unrealized_pnl']:.25f} USDC ({pnl_percentage:.2f}%)\n==========================\n")
                            close_position(position, current_price)
                            position = get_position()  # Update position after closing
                    
                    # Display current trade status
                    display_trade_status(position, current_price)
                    
                    # Skip the rest of the analysis if we have an open position
                    time.sleep(5)
                    continue
                
                # Only proceed with analysis if there's no open position
                ...
                # Initialize conditions with LRC conditions
                conditions_long = {
                    ...
                    # New conditions for all timeframes
                    "dip_reversal_1m": False,
                    "dip_reversal_3m": False,
                    "dip_reversal_5m": False,
                    "below_middle_1m": False,
                    "below_middle_3m": False,
                    "below_middle_5m": False,
                    # Volume conditions only for 1m
                    "volume_bullish_1m": False,
                    # LRC forecast conditions for all timeframes
                    "lrc_forecast_above_current_1m": False,
                    "lrc_forecast_above_current_3m": False,
                    "lrc_forecast_above_current_5m": False,
                    # Magic Square forecast condition
                    "magic_square_forecast_above_current_1m": False,
                }
                ...
                conditions_short = {
                    ...
                    # New conditions for all timeframes
                    "top_reversal_1m": False,
                    "top_reversal_3m": False,
                    "top_reversal_5m": False,
                    ...
                    # LRC forecast conditions for all timeframes
                    "lrc_forecast_below_current_1m": False,
                    "lrc_forecast_below_current_3m": False,
                    "lrc_forecast_below_current_5m": False,
                    # Magic Square forecast condition
                    "magic_square_forecast_below_current_1m": False,
                }
                ...
                for tf in ["3m", "5m"]:
                    if candle_map.get(tf):
                        ...
                        reversal_type, _, _, _, _ = detect_standard_reversal(candle_map[tf], tf, current_close)
                        higher_tf_tops[tf] = (reversal_type == "TOP")
                
                analysis_details = {}
                
                # Calculate LRC for all timeframes
                lrc_data = {}
                for tf in TIMEFRAMES:
                    lrc_data[tf] = calculate_linear_regression_channel(
                        candle_map.get(tf, []),
                        period=500,
                ...
                for timeframe in TIMEFRAMES:
                    ...
                    current_close_tf = Decimal(str(closes[-1])) if len(closes) > 0 else current_price
                    
                    # Use LRC-based reversal detection for 1m timeframe
                    if timeframe == "1m":
                        reversal_type, reversal_time, reversal_price, min_idx, max_idx = detect_lrc_reversal(
                            candles_tf, timeframe, lrc_data[timeframe], current_close_tf
                        )
                    else:
                        # Use standard reversal detection for other timeframes
                        reversal_type, reversal_time, reversal_price, min_idx, max_idx = detect_standard_reversal(
                            candles_tf, timeframe, current_close_tf
                        )
                    ...
                    # Set volume conditions only for 1m
                    if timeframe == "1m":
                    ...
                    if hurst_peaks and hurst_troughs:
                        ...
                        if hurst_cycle_type == "Up":
                            hurst_target = current_close_tf * (Decimal('1') + CYCLE_TARGET_PERCENTAGE)
                    ...
                    momentum_values, momentum_positive, momentum_negative, momentum_increasing, momentum_decreasing = calculate_momentum_trend(
                    ...
                    advanced_fft_forecast_price, fft_cycle_stage, fft_cycle_direction, fft_dominant_freq, fft_phase_angle, fft_harmonic_strength = advanced_fft_forecast(
                        candles_tf, timeframe, min_threshold, max_threshold, current_close_tf
                    )
                    
                    # Wavelet Analysis
                    dominant_cycle, wavelet_cycle_direction, wavelet_forecast, wavelet_cycles, wavelet_reconstruction = wavelet_analysis(
                        candles_tf, timeframe
                    )
                    
                    # Calculate DMI for 1m timeframe
                    plus_di, minus_di, adx, is_bullish, is_bearish = None, None, None, False, False
                    if timeframe == "1m":
                        plus_di, minus_di, adx, is_bullish, is_bearish = calculate_dmi(candles_tf, timeframe, period=DMI_PERIOD)
                        
                        # Set DMI conditions with perfect symmetry
                        conditions_long["dmi_bullish_1m"] = is_bullish
                        conditions_short["dmi_bearish_1m"] = is_bearish
                    ...
                    conditions_short[f"top_reversal_{timeframe}"] = (reversal_type == "TOP")
                    conditions_short[f"above_middle_{timeframe}"] = (current_close_tf > middle_threshold)
                    
                    # Set LRC forecast conditions for all timeframes
                    if lrc_data[timeframe]["valid"] and lrc_data[timeframe]["forecast"] is not None:
                        lrc_forecast = Decimal(str(lrc_data[timeframe]["forecast"]))
                        conditions_long[f"lrc_forecast_above_current_{timeframe}"] = lrc_forecast > current_close_tf
                        conditions_short[f"lrc_forecast_below_current_{timeframe}"] = lrc_forecast < current_close_tf
                        
                        print(f"LRC Forecast Above Current ({timeframe}): {conditions_long[f'lrc_forecast_above_current_{timeframe}']}")
                        print(f"LRC Forecast Below Current ({timeframe}): {conditions_short[f'lrc_forecast_below_current_{timeframe}']}")
                    else:
                        conditions_long[f"lrc_forecast_above_current_{timeframe}"] = False
                        conditions_short[f"lrc_forecast_below_current_{timeframe}"] = False
                    
                    # Set conditions for 1m timeframe with perfect symmetry
                    if timeframe == "1m":
                        # Momentum conditions
                        conditions_long["momentum_positive_1m"] = momentum_positive
                        conditions_short["momentum_negative_1m"] = momentum_negative
                        ...
                        # Magic Square forecast conditions
                        magic_square_forecast_price, magic_square_current_state, magic_square_next_state = magic_square_forecast(candles_tf, timeframe)
                        conditions_long["magic_square_forecast_above_current_1m"] = magic_square_forecast_price > current_close_tf
                        conditions_short["magic_square_forecast_below_current_1m"] = magic_square_forecast_price < current_close_tf
                        
                        print(f"Magic Square Forecast Above Current (1m): {conditions_long['magic_square_forecast_above_current_1m']}")
                        print(f"Magic Square Forecast Below Current (1m): {conditions_short['magic_square_forecast_below_current_1m']}")
                    ...
                    analysis_details[timeframe] = {
                        ...
                        "wavelet_dominant_cycle": dominant_cycle,
                        "wavelet_cycle_direction": wavelet_cycle_direction,
                        "wavelet_forecast": wavelet_forecast,
                        ...
                        "perc_distance": calculate_percentage_distance(current_close_tf, min_threshold, max_threshold),
                        "dist_to_min": current_close_tf - min_threshold,
                        "dist_to_max": max_threshold - current_close_tf,
                        ...
                        "reversal_time": datetime.datetime.fromtimestamp(reversal_time) if reversal_time else None,
                        ...
                        # Add new conditions
                        "dip_reversal": conditions_long.get(f"dip_reversal_{timeframe}", False),
                        "top_reversal": conditions_short.get(f"top_reversal_{timeframe}", False),
                        "below_middle": conditions_long.get(f"below_middle_{timeframe}", False),
                        "above_middle": conditions_short.get(f"above_middle_{timeframe}", False),
                        # Add volume conditions
                        f"volume_bullish_{timeframe}": conditions_long.get(f"volume_bullish_{timeframe}", False),
                        f"volume_bearish_{timeframe}": conditions_short.get(f"volume_bearish_{timeframe}", False),
                    }
                    
                    # Add LRC details for this timeframe
                    analysis_details[timeframe].update({
                        "lrc_regression": lrc_data[timeframe]["regression"],
                        "lrc_upper": lrc_data[timeframe]["upper"],
                        "lrc_lower": lrc_data[timeframe]["lower"],
                        "lrc_above_upper": lrc_data[timeframe]["above_upper"],
                        "lrc_below_lower": lrc_data[timeframe]["below_lower"],
                        "lrc_valid": lrc_data[timeframe]["valid"],
                        "lrc_forecast": lrc_data[timeframe]["forecast"]
                    })
                    
                    # Add LRC forecast conditions to analysis details
                    analysis_details[timeframe].update({
                        f"lrc_forecast_above_current_{timeframe}": conditions_long.get(f"lrc_forecast_above_current_{timeframe}", False),
                        f"lrc_forecast_below_current_{timeframe}": conditions_short.get(f"lrc_forecast_below_current_{timeframe}", False)
                    })
                    
                    # Add DMI to analysis details for 1m timeframe
                    if timeframe == "1m" and plus_di is not None and minus_di is not None and adx is not None:
                        analysis_details[timeframe].update({
                            "plus_di": plus_di,
                            "minus_di": minus_di,
                            "adx": adx,
                            "dmi_bullish": conditions_long["dmi_bullish_1m"],
                            "dmi_bearish": conditions_short["dmi_bearish_1m"]
                        })
                    
                    # Add Magic Square forecast to analysis details for 1m timeframe
                    if timeframe == "1m":
                        analysis_details[timeframe].update({
                            "magic_square_forecast": magic_square_forecast_price,
                            "magic_square_current_state": magic_square_current_state,
                            "magic_square_next_state": magic_square_next_state,
                            "magic_square_forecast_above_current": conditions_long["magic_square_forecast_above_current_1m"],
                            "magic_square_forecast_below_current": conditions_short["magic_square_forecast_below_current_1m"]
                        })
                    ...
                    print(f"Wavelet Dominant Cycle: {dominant_cycle}")
                    print(f"Wavelet Cycle Direction: {wavelet_cycle_direction}")
                    print(f"Wavelet Forecast: {wavelet_forecast:.25f}")
                    ...
                    # Print new conditions for all timeframes
                    print(f"DIP Reversal ({timeframe}): {conditions_long.get(f'dip_reversal_{timeframe}', False)}")
                    print(f"TOP Reversal ({timeframe}): {conditions_short.get(f'top_reversal_{timeframe}', False)}")
                    print(f"Below Middle ({timeframe}): {conditions_long.get(f'below_middle_{timeframe}', False)}")
                    print(f"Above Middle ({timeframe}): {conditions_short.get(f'above_middle_{timeframe}', False)}")
                    
                    # Print volume conditions only for 1m
                    if timeframe == "1m":
                        print(f"Volume Bullish ({timeframe}): {conditions_long.get(f'volume_bullish_{timeframe}', False)}")
                        print(f"Volume Bearish ({timeframe}): {conditions_short.get(f'volume_bearish_{timeframe}', False)}")
                        print(f"DMI +DI: {plus_di:.4f}")
                        print(f"DMI -DI: {minus_di:.4f}")
                        print(f"DMI ADX: {adx:.4f}")
                        print(f"DMI Bullish: {conditions_long['dmi_bullish_1m']}")
                        ...
                        print(f"Magic Square Forecast: {magic_square_forecast_price:.25f}")
                        print(f"Magic Square Current State: {magic_square_current_state}")
                        print(f"Magic Square Next State: {magic_square_next_state}")
                        print(f"Magic Square Forecast Above Current (1m): {conditions_long['magic_square_forecast_above_current_1m']}")
                        print(f"Magic Square Forecast Below Current (1m): {conditions_short['magic_square_forecast_below_current_1m']}")
                    
                    # Print LRC details for this timeframe
                    if lrc_data[timeframe]["valid"]:
                        print(f"LRC Regression: {lrc_data[timeframe]['regression']:.25f}")
                        print(f"LRC Upper: {lrc_data[timeframe]['upper']:.25f}")
                        print(f"LRC Lower: {lrc_data[timeframe]['lower']:.25f}")
                        print(f"LRC Forecast: {lrc_data[timeframe]['forecast']:.25f}")
                        print(f"LRC Above Upper: {lrc_data[timeframe]['above_upper']}")
                        print(f"LRC Below Lower: {lrc_data[timeframe]['below_lower']}")
                    else:
                        print("LRC: Invalid calculation")
                
                # FIXED SIGNAL LOGIC - Trigger when at least 13 conditions from 16 are true
                long_conditions_met = sum(conditions_long.values())
                short_conditions_met = sum(conditions_short.values())
                
                long_signal = long_conditions_met >= 13
                short_signal = short_conditions_met >= 13
                
                # Determine the final signal
                if long_signal and short_signal:
                    signal = "NO_SIGNAL"  # Conflict: both signals active
                ...
                print(f"Total Met: {long_conditions_met}/16 - {'PASS' if long_signal else 'FAIL'}")
                print("=====================")
                ...
                for condition, status in conditions_short.items():
                    print(f"{condition}: {'PASS' if status else 'FAIL'}")
                print(f"Total Met: {short_conditions_met}/16 - {'PASS' if short_signal else 'FAIL'}")
                print("======================")
                
                # Print final signal evaluation
                print(f"\n=== FINAL SIGNAL EVALUATION ===")
                print(f"LONG Conditions Met: {long_conditions_met}/16 - {'PASS' if long_signal else 'FAIL'}")
                print(f"SHORT Conditions Met: {short_conditions_met}/16 - {'PASS' if short_signal else 'FAIL'}")
                print(f"Final Signal: {signal}")
                print("==============================\n")
                
                logging.info(f"Final Signal: {signal} (LONG: {long_conditions_met}/16, SHORT: {short_conditions_met}/16)")
                
                # Only print signal alert if signal is not NO_SIGNAL
                if signal != "NO_SIGNAL":
                    print(f"\n🚨 STRONG SIGNAL ALERT: {signal} (at least 13/16 conditions met) 🚨")
                
                # If we have a signal and no open position, try to trade
                if signal in ["LONG", "SHORT"] and position["side"] == "NONE":
                    # Prepare the list of true conditions for the signal
                    if signal == "LONG":
                    ...
                    if quantity > Decimal('0'):
                        # Add signal info to analysis_details
                        signal_info = {
