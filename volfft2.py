...
def get_active_trading_pairs(client):
    ...
    # Fetch volume for each trading pair
    volume_dict = {ticker['symbol']: float(ticker['price']) for ticker in tickers if ticker['symbol'] in active_trading_pairs}
    
    # Sort pairs by volume descending
    sorted_pairs = sorted(volume_dict.keys(), key=lambda x: volume_dict[x], reverse=True)
    return sorted_pairs
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                if buy_volume > sell_volume:  # Check for bullish tendency
                    bullish_assets.append((symbol, df, buy_volume, sell_volume, total_volume))
            except Exception as e:
                print(f"Error fetching data for {symbol}: {e}")

    most_profitable_asset = None
    max_potential_profit = -np.inf

    for symbol, df, buy_volume, sell_volume, total_volume in bullish_assets:
        ...
        print(f"\nSymbol: {symbol}, Total Volume: {total_volume}, Buy Volume: {buy_volume}, Sell Volume: {sell_volume}, Trend: Bullish")
        ...
        if potential_profit > max_potential_profit:
            ...
            most_profitable_asset = (symbol, potential_profit, latest_close, forecast_close)

    if most_profitable_asset:
