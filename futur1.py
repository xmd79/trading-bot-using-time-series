...
def get_position():
    try:
        ...
        return {"quantity": Decimal('0.0'), "entry_price": Decimal('0.0'), "side": "NONE"}
...
def calculate_thresholds(close_prices, period=14, minimum_percentage=2, maximum_percentage=2, range_distance=Decimal('0.05')):
    ...
    percent_to_min_combined = (Decimal(str(minimum_percentage)) + Decimal(str(percent_to_min_momentum))) / Decimal('2')
    percent_to_max_combined = (Decimal(str(maximum_percentage)) + Decimal(str(percent_to_max_momentum))) / Decimal('2')
...
def main():
    ...
    while True:
        try:
            ...
            # Initialize conditions (first filter only)
            conditions_long = {
                "forecast_price_above_close": False,
                "dip_confirmed_1m": False,
                "dip_confirmed_3m": False,
                "dip_confirmed_5m": True,
                "dip_confirmed_15m": True,
                "dip_confirmed_30m": False,
                "volume_bullish_1m": False,
                "volume_bullish_3m": True,
                "volume_bullish_5m": True,
                "volume_bullish_15m": True,
                "volume_bullish_30m": True,
                "dist_to_min_closer_1m": False,
                "dist_to_min_closer_3m": True,
                "dist_to_min_closer_5m": True,
                "dist_to_min_closer_15m": True,
                "dist_to_min_closer_30m": True,
                "momentum_positive_1m": True
            }
            conditions_short = {
                "forecast_price_below_close": False,
                "top_confirmed_1m": False,
                "top_confirmed_3m": True,
                "top_confirmed_5m": False,
                "top_confirmed_15m": False,
                "top_confirmed_30m": True,
                "volume_bearish_1m": False,
                "volume_bearish_3m": True,
                "volume_bearish_5m": True,
                "volume_bearish_15m": True,
                "volume_bearish_30m": True,
                "dist_to_max_closer_1m": False,
                "dist_to_max_closer_3m": True,
                "dist_to_max_closer_5m": True,
                "dist_to_max_closer_15m": True,
                "dist_to_max_closer_30m": True,
            ...
            if "1m" in candle_map and candle_map["1m"]:
                ...
                min_threshold, max_threshold, _, momentum_signal, range_price, _, _ = calculate_thresholds(close_prices)
                ...
                min_threshold, max_threshold, _, adjusted_forecasted_price = None, None, None, None
            ...
            for timeframe in timeframes:
                if timeframe in candle_map and candle_map[timeframe]:
                    ...
                    dist_to_min_price = ((current_close - min_threshold_tf) / (max_threshold_tf - min_threshold_tf)) * Decimal('100') if (max_threshold_tf - min_threshold_tf) != Decimal('0') else Decimal('50')
                    dist_to_max_price = ((max_threshold_tf - current_close) / (max_threshold_tf - min_threshold_tf)) * Decimal('100') if (max_threshold_tf - min_threshold_tf) != Decimal('0') else Decimal('50')
                    ...
                    # Moving averages (for second filter)
                    valid_closes = np.array([float(c) for c in closes if not np.isnan(c) and c > 0], dtype=np.float64)
                    sma_lengths = [5, 7, 9, 12, 27, 56]
                    ...
                    # 1m-specific SMA alignment check (second filter)
                    if timeframe == "1m" and all(length in smas for length in [7, 9, 12, 27, 56]):
                        sma7 = smas[7]
                        sma9 = smas[9]
                        sma12 = smas[12]
                        sma27 = smas[27]
                        sma56 = smas[56]
                        if current_close < sma7 < sma9 < sma12 < sma27 < sma56:
                            sma_bullish_1m = True
                            print("1m SMA Bullish Alignment: close < SMA7 < SMA9 < SMA12 < SMA27 < SMA56")
                        elif current_close > sma7 > sma9 > sma12 > sma27 > sma56:
                            sma_bearish_1m = True
                            print("1m SMA Bearish Alignment: close > SMA7 > SMA9 > SMA12 > SMA27 > SMA56")
                        else:
                            print("1m SMA Alignment: No clear bullish or bearish pattern")
                    ...
                    fib_levels = calculate_fibonacci_levels_from_reversal(None, timeframe, None, None, 'DIP', current_price)
                    fib_info[timeframe] = fib_levels
            ...
            for long_cond, short_cond in condition_pairs:
                ...
                if long_val and short_val:
                    ...
                    # Neutral case: force symmetry by setting based on secondary logic
                    if "momentum" in long_cond:
                        # Already handled above
                        pass
                    elif "dist_to" in long_cond:
                        # Use distance comparison to decide
                        tf = long_cond.split('_')[-1]
                        dist_to_min = ((current_price - min_threshold_tf) / (max_threshold_tf - min_threshold_tf)) * Decimal('100') if (max_threshold_tf - min_threshold_tf) != Decimal('0') else Decimal('50')
                        dist_to_max = ((max_threshold_tf - current_price) / (max_threshold_tf - min_threshold_tf)) * Decimal('100') if (max_threshold_tf - min_threshold_tf) != Decimal('0') else Decimal('50')
                        if dist_to_min < dist_to_max:
                            conditions_long[f"dist_to_min_closer_{tf}"] = True
                            conditions_short[f"dist_to_max_closer_{tf}"] = False
                        else:
                            conditions_long[f"dist_to_min_closer_{tf}"] = False
                            conditions_short[f"dist_to_max_closer_{tf}"] = True
                    elif "volume" in long_cond:
                        tf = long_cond.split('_')[-1]
                        buy_vol = buy_volume.get(tf, [Decimal('0')])[-1]
                        sell_vol = sell_volume.get(tf, [Decimal('0')])[-1]
                        if buy_vol > sell_vol:
                            conditions_long[f"volume_bullish_{tf}"] = True
                            conditions_short[f"volume_bearish_{tf}"] = False
                        elif sell_vol > buy_vol:
                            conditions_long[f"volume_bullish_{tf}"] = False
                            conditions_short[f"volume_bearish_{tf}"] = True
                    elif "dip_confirmed" in long_cond:
                        tf = long_cond.split('_')[-1]
                        _, _, _, closest_type = find_major_reversals(candle_map[tf], current_price, min_threshold_tf, max_threshold_tf)
                        conditions_long[f"dip_confirmed_{tf}"] = closest_type == 'DIP'
                        conditions_short[f"top_confirmed_{tf}"] = closest_type == 'TOP'

            # Evaluate first filter signals
            first_filter_long = all(conditions_long.values())
            first_filter_short = all(conditions_short.values())

            # Apply second filter (SMA stack) only if first filter is fully satisfied
            long_signal = False
            short_signal = False
