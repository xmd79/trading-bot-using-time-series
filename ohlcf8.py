...
# Function to calculate Pythagorean price forecast
def calculate_pythagorean_forecast(current_close, last_major_reversal_price, current_cycle_direction):
    """Calculate forecast price using the Pythagorean theorem approach."""
    price_difference = abs(current_close - last_major_reversal_price)
    if current_cycle_direction == "UP":
        forecast_price = current_close + price_difference  # Ascending triangle
    else:
        forecast_price = current_close - price_difference  # Descending triangle
...
for timeframe in timeframes:
    ...
    min_threshold, max_threshold = calculate_thresholds(close)
    ...
    # Calculate Pythagorean forecast
    pythagorean_forecast = calculate_pythagorean_forecast(current_close, last_major_reversal_price, current_cycle_direction)
    ...
    print(f"Pythagorean Price Forecast: {pythagorean_forecast:.2f}")
    ...
    # Adding visualizations for each timeframe
    min_threshold, max_threshold = calculate_thresholds(close)
    middle_threshold = (min_threshold + max_threshold) / 2

    plt.figure(figsize=(12, 6))
    plt.plot(close, label='Closing Prices', color='blue')
    ...
    plt.axhline(y=current_close, color='purple', linestyle='--', label='Current Close')
    plt.axhline(y=pythagorean_forecast, color='cyan', linestyle='--', label='Pythagorean Forecast')

    plt.title(f'Price Analysis for {TRADE_SYMBOL} - {timeframe} Timeframe')
    plt.xlabel('Time (candles)')
    plt.ylabel('Price (USDT)')
    plt.legend()
    plt.grid()
    plt.show()  # Show plot for each timeframe

# Create Summary Table
print("\nSummary of All Timeframes:")
print("=" * 120)
print(f"{'Timeframe':<10} {'Last Reversal':<15} {'Reversal Price':<15} {'Cycle Direction':<15} {'Incoming Threshold':<15} {'FFT Target':<15} {'Min Threshold':<15} {'Max Threshold':<15} {'Support':<15} {'Resistance':<15} {'Bullish Volume':<15} {'Bearish Volume':<15} {'Total Volume':<15} {'Bullish Ratio %':<15} {'Bearish Ratio %':<15}")
print("=" * 120)
for data in summary_data:
    print(f"{data['Timeframe']:10} {data['Last Major Reversal Type']:15} {data['Last Major Reversal Price']:15.2f} {data['Cycle Direction']:15} {data['Incoming Threshold']:15.2f} {data['FFT Forecast Price Target']:15.2f} {data['Min Threshold']:15.2f} {data['Max Threshold']:15.2f} {data['Support']:15.2f} {data['Resistance']:15.2f} {data['Bullish Volume']:15.2f} {data['Bearish Volume']:15.2f} {data['Total Volume']:15.2f} {data['Bullish Ratio %']:15.2f} {data['Bearish Ratio %']:15.2f}")