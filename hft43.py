...
def main():
    ...
    quadrant1_range = (0, 90)
    quadrant2_range = (90, 180)
    quadrant3_range = (180, 270)
    quadrant4_range = (270, 360)
        ...
        try:
            ...
            if '1m' in signals:
                ...
                if 'ht_sine_percent_to_min' in signals['1m'] and 'ht_sine_percent_to_max' in signals['1m']:
                    ...
                    # Calculate the EM field for each quadrant
                    em_field_q1 = 0
                    em_field_q2 = 0
                    em_field_q3 = 0
                    em_field_q4 = 0
                    for i in range(len(sine_wave)):
                        ...
                        elif quadrant2_range[0] <= i < quadrant2_range[1]:
                            em_field_q2 += sine_wave[i]
                        elif quadrant3_range[0] <= i < quadrant3_range[1]:
                            em_field_q3 += sine_wave[i]
                        elif quadrant4_range[0] <= i < quadrant4_range[1]:
                            em_field_q4 += sine_wave[i]

                    # Calculate the EM amplitude for each quadrant
                    em_amp_q1 = math.sqrt(em_field_q1 ** 2 + em_field_q2 ** 2)
                    em_amp_q2 = math.sqrt(em_field_q2 ** 2 + em_field_q3 ** 2)
                    em_amp_q3 = math.sqrt(em_field_q3 ** 2 + em_field_q4 ** 2)
                    em_amp_q4 = math.sqrt(em_field_q4 ** 2 + em_field_q1 ** 2)

                    # Calculate the EM phase for each quadrant
                    em_phase_q1 = math.atan2(em_field_q2, em_field_q1)
                    em_phase_q2 = math.atan2(em_field_q3, em_field_q2)
                    ...
                    if em_phase_diff_q1_q2 > math.pi:
                        em_phase_diff_q1_q2 -= 2*math.pi
                    elif em_phase_diff_q1_q2 < -math.pi:
                        em_phase_diff_q1_q2 += 2*math.pi

                    if em_phase_diff_q2_q3 > math.pi:
                        em_phase_diff_q2_q3 -= 2*math.pi
                    elif em_phase_diff_q2_q3 < -math.pi:
                        em_phase_diff_q2_q3 += 2*math.pi

                    if em_phase_diff_q3_q4 > math.pi:
                        em_phase_diff_q3_q4 -= 2*math.pi
                    elif em_phase_diff_q3_q4 < -math.pi:
                        em_phase_diff_q3_q4 += 2*math.pi

                    if em_phase_diff_q4_q1 > math.pi:
                    ...
                    print("Error: Required signal keys not found in signals dictionary.")
                    continue
            else:
                print("Error: '1m' key not found in signals dictionary.")
                continue

            # Wait for 5 seconds before getting new data
            time.sleep(5)
