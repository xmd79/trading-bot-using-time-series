...
def get_planet_positions(current_time):
    ...
    for planet in planets:
        ...
        helio_longitude = get_heliocentric_longitude(planet['name'], current_time)

        # Calculate heliocentric aspects
        helio_aspects = calculate_heliocentric_aspects(planet['name'], current_time)

        # Calculate astrological cycle status
        astro_cycle_status = calculate_astrological_cycle_status(planet['name'], current_time)
        ...
        planet_data[planet['name']] = {
            ...
            'astro_cycle_status': astro_cycle_status,
            'helio_longitude': helio_longitude,
            'helio_aspects': helio_aspects
        }
...
def get_heliocentric_longitude(planet_name, current_time):
    planet = getattr(ephem, planet_name)()
    planet.compute(current_time)
    return math.degrees(planet.hlon)

def calculate_heliocentric_aspects(planet_name, current_time):
    ...
    for other_planet_name in ['Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto']:
        if other_planet_name == planet_name:
            continue
        other_planet = getattr(ephem, other_planet_name)()
        other_planet.compute(current_time)

        # Calculate separation in degrees
        separation_deg = math.degrees(ephem.separation((planet_ra, planet_dec), (other_planet.ra, other_planet.dec)))

        # Determine aspect type
        aspect_type = None
        if separation_deg <= 3:
            aspect_type = 'conjunction'
        elif separation_deg >= 177:
            aspect_type = 'opposition'
        elif 87 <= separation_deg <= 93:
            aspect_type = 'square'
        elif 117 <= separation_deg <= 123:
            aspect_type = 'trine'
        elif 57 <= separation_deg <= 63:
            aspect_type = 'sextile'

        if aspect_type:
            aspect_data.append({
                'planet': other_planet_name,
...
def calculate_astrological_cycle_status(planet_name, current_time):
    aspects = calculate_heliocentric_aspects(planet_name, current_time)
    aspect_types = [aspect['aspect_type'] for aspect in aspects]
    if 'conjunction' in aspect_types or 'opposition' in aspect_types:
        return 'High activity'
    elif 'square' in aspect_types or 'trine' in aspect_types or 'sextile' in aspect_types:
...
def main():
    ...
    for planet, data in planet_positions.items():
        ...
        print(f"{planet} Heliocentric Longitude: {data['helio_longitude']:.2f}")
        print(f"{planet} Heliocentric Aspects:")
        for aspect in data['helio_aspects']:
            print(f"   - {aspect['planet']} {aspect['aspect_type']} ({aspect['separation_deg']:.2f} degrees)")
        print()
