# Trading bot for Binance futures (BTCUSDC) with technical analysis, ML forecasting, Hurst cycle analysis, and Telegram notifications
# Dependencies: numpy, talib, scikit-learn, python-telegram-bot, python-binance, scipy, hurst
...
def hurst_cycle_analysis(series, timeframe, window_size=200, sampling_rate=20, preferred_kind='random_walk'):
    ...
    if len(series) < window_size:
        ...
        print(f"{timeframe} - Hurst cycle analysis skipped: Series length ({len(series)}) is less than window_size ({window_size})")
        return [], [], []
    ...
    for start in range(0, length - window_size + 1, sampling_rate):
        ...
        if len(window) < window_size:
            ...
            print(f"{timeframe} - Skipping window at index {start}: insufficient data points")
            continue
        ...
        if np.std(window) < 1e-10 or np.max(window) == np.min(window):
            ...
            print(f"{timeframe} - Skipping window at index {start}: zero range or standard deviation")
            continue
        ...
        if np.any(np.isnan(window)) or np.any(np.isinf(window)):
            ...
            print(f"{timeframe} - Skipping window at index {start}: contains NaN or Inf values")
            continue
        ...
        for kind in kinds:
            try:
                ...
                print(f"{timeframe} - Success at index {start} with kind='{kind}', Hurst={H:.3f}")
                hurst_computed = True
                break
            except Exception as e:
                logging.error(f"{timeframe} - Error computing Hurst in window at index {start} with kind='{kind}': {e}")
                print(f"{timeframe} - Error computing Hurst in window at index {start} with kind='{kind}': {e}")

        if not hurst_computed:
            logging.warning(f"{timeframe} - Failed to compute Hurst for window at index {start}")
            print(f"{timeframe} - Failed to compute Hurst for window at index {start}")
            continue
    ...
    if not cycles:
        ...
        print(f"{timeframe} - No valid windows processed for Hurst cycle analysis")
        return [], [], []
    ...
    for attempt in range(retries):
        try:
            ...
            for tf in TIMEFRAMES:
                ...
                # Validate cycle_target and hurst_target
