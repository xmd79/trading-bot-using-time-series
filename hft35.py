    ...
    while True:
        ...
        try:
            ...
            if '1m' in signals:
                ...
                if 'ht_sine_percent_to_min' in signals['1m'] and 'ht_sine_percent_to_max' in signals['1m'] and 'mtf_momentum' in signals['1m']:
                    ...
                    mtf_momentum = signals['1m']['mtf_momentum']
                    close_prices = np.array([candle['close'] for candle in candles['1m']])

                    print("Close price:", close_prices[-1])
                    print("MTF signal average:", mtf_average)
                    print("MTF signal momentum:", mtf_momentum)
                    print("Percent to min:", percent_to_min_val)
                    print("Percent to max:", percent_to_max_val)
                    print()

                    # Calculate the Fourier transform of the signal
                    signal_fft = np.fft.fft(close_prices)
                    signal_freq = np.fft.fftfreq(len(close_prices))
                    signal_fft_abs = np.abs(signal_fft)

                    # Get the maximum value of the signal within the min/max thresholds
                    signal_within_thresholds = close_prices[(close_prices >= em_amp * percent_to_min_val) & (close_prices <= em_amp * percent_to_max_val)]
                    max_signal = np.max(signal_within_thresholds)

                    # Calculate the current position of the signal on the sine wave
                    current_phase = (2 * math.pi * (close_prices[-1] - em_amp * percent_to_min_val)) / (em_amp * (percent_to_max_val - percent_to_min_val))
                    current_phase_deg = np.rad2deg(current_phase)

                    # Calculate the quadrant of the current position of the signal on the unit circle
                    if (current_phase_deg >= 0 and current_phase_deg < 90):
                        quadrant = "Quadrant 1"
                    elif (current_phase_deg >= 90 and current_phase_deg < 180):
                        quadrant = "Quadrant 2"
                    elif (current_phase_deg >= 180 and current_phase_deg < 270):
                        quadrant = "Quadrant 3"
                    else:
                        quadrant = "Quadrant 4"

                    # Calculate the frequency spectrum range index based on the electromagnetic field value
                    em_field_index = int(round(em_field * 100))

                    if em_field_index < 10:
                        freq_range_start = 0
                        freq_range_end = 20
                    elif em_field_index < 20:
                        freq_range_start = 20
                        freq_range_end = 40
                    elif em_field_index < 30:
                        freq_range_start = 40
                        freq_range_end = 60
                    elif em_field_index < 40:
                        freq_range_start = 60
                        freq_range_end = 80
                    elif em_field_index < 50:
                        freq_range_start = 80
                        freq_range_end = 100
                    else:
                        freq_range_start = 100
                        freq_range_end = 120

                    # Determine the market mood based on the current signal position on the sine wave and quadrant
                    if close_prices[-1] < em_amp * percent_to_min_val or close_prices[-1] >= max_signal:
                        market_mood = "Neutral"
                    elif quadrant == "Quadrant 1" and current_phase_deg < 70:
                        market_mood = "Short"
                    elif quadrant == "Quadrant 1" and current_phase_deg >= 70:
                        market_mood = "Long"
                    elif quadrant == "Quadrant 2" and current_phase_deg < 250:
                        market_mood = "Short"
                    elif quadrant == "Quadrant 2" and current_phase_deg >= 250:
                        market_mood = "Long"
                    elif quadrant == "Quadrant 3" and current_phase_deg < 190:
                        market_mood = "Long"
                    elif quadrant == "Quadrant 3" and current_phase_deg >= 190:
                        market_mood = "Short"
                    elif quadrant == "Quadrant 4" and current_phase_deg < 10:
                        market_mood = "Long"
                    else:
                        market_mood = "Short"

                    print("Max signal:", max_signal)
                    print("Current phase (radians):", current_phase)
                    print("Current phase (degrees):", current_phase_deg)
                    print("Quadrant:", quadrant)
                    print("EM field index:", em_field_index)
                    print("Frequency spectrum range start:", freq_range_start)
                    print("Frequency spectrum range end:", freq_range_end)
                    print("Market mood:", market_mood)

                    # Determine the forecast market mood for the next 15-30 minutes based on the frequency spectrum range index
                    forecast_mood = "Neutral"

                    if em_field_index >= 30 and freq_range_start <= 60 and freq_range_end >= 80:
                        forecast_mood = "Bullish"
                    elif em_field_index <= 20 and freq_range_start >= 80 and freq_range_end <= 100:
                        forecast_mood = "Bearish"

                    print("Forecast mood:", forecast_mood)
                    print()
